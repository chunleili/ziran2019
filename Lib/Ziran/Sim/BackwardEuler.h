 ...
 */
template <class Simulation> class BackwardEulerLagrangianForceObjective {
public:
  ...
  BackwardEulerLagrangianForceObjective(Simulation &simulation)
      : simulation(simulation), project([](TVStack &) {}),
        precondition([](const TVStack &x, TVStack &b) { b = x; }), gmres(20),
        linear_solver(std::make_unique<Minres<T, Objective, TVStack>>(20)),
        minres(*reinterpret_cast<Minres<T, Objective, TVStack> *>(
            linear_solver.get())),
        matrix_free(false), lu(false) {
    gmres.setTolerance(1);
    linear_solver->setTolerance(1);
  }

  BackwardEulerLagrangianForceObjective(
      const BackwardEulerLagrangianForceObjective &objective) = delete;
  ...
  void
  switchLinearSolver(SimulationBase::LinearSolverType p_linear_solver_type) {
    ...
    case SimulationBase::CG:
      linear_solver =
          std::make_unique<ConjugateGradient<T, Objective, TVStack>>(20);
      break;
    ...
    T norm_sq = tbb::parallel_reduce(
        ...
        [&](const tbb::blocked_range<int> &range, T ns) -> T {
          ...
          const auto &mass_block =
              simulation.mass_matrix.segment(start, length).transpose();
  ...
  T innerProduct(const TVStack &ddv, const TVStack &residual) const {
    ...
    T result = tbb::parallel_reduce(
        ...
        [&](const tbb::blocked_range<int> &range, T ns) -> T {
          ...
          const auto &r_block = residual.middleCols(start, length);
          const auto &mass_block =
              simulation.mass_matrix.segment(start, length);
          // ZIRAN_DEBUG(ddv_block);
          // ZIRAN_DEBUG(r_block);
          // ZIRAN_DEBUG(mass_block);

          T to_add = ((ddv_block.transpose() * r_block).diagonal().array() /
                      mass_block.array())
                         .sum();
          // ZIRAN_DEBUG(to_add);
          return ns += to_add;
  ...
  T totalEnergy() {
    ...
    result += tbb::parallel_reduce(
        tbb::blocked_range<int>(0, simulation.dv.cols(), 256), (T)0,
        [&](const tbb::blocked_range<int> &range, T ns) -> T {
          int start = range.begin();
          int length = (range.end() - range.begin());
          ...
          const auto &mass_block =
              simulation.mass_matrix.segment(start, length);
          return ns -
                 simulation.dt * ((simulation.gravity.transpose() * dv_block)
                                      .dot(mass_block));
        },
        [](T a, T b) -> T { return a + b; });
    return result;
  ...
  void computeStep(TVStack &ddv, const TVStack &residual,
                   const T linear_solve_relative_tolerance) {
    ...
    if (!matrix_free) {
      ...
      if (lu) {
        ...
        */
        eigenSparseLUSolve(implicit_system_matrix, EIGEN_EXT::vec(residual),
                           EIGEN_EXT::vec(ddv));
  ...
  T computeDescentStep(TVStack &ddv, const TVStack &residual,
                       const T linear_solve_relative_tolerance) {
    ...
    if (inner_prod < -tol) {
      ZIRAN_INFO("Newton step direction is of same direction as gradient. Look "
                 "backwards!!");
      ...
      ZIRAN_INFO("Newton step direciton is almost orthogonal to gradient. Look "
                 "towards negative gradient direction!");
  ...
  /**
     This sets up the sparsity pattern in the implicit time stepping system
     matrix. This sparsity pattern will not change over time, although it's
     entries will.
  */
  void initializeImplicitTimeSteppingMatrix() {
    ...
    implicit_system_matrix.resize(simulation.num_nodes * dim,
                                  simulation.num_nodes * dim);
    ...
    implicit_system_matrix.setFromTriplets(tripletList.begin(),
                                           tripletList.end());
  ...
  void buildMatrix() {
    ...
    for (int k = 0; k < implicit_system_matrix.outerSize(); ++k)
      for (typename SpMat::InnerIterator it(implicit_system_matrix, k); it;
           ++it) {
        it.valueRef() = (T)0;
      }
    if (!simulation.quasistatic)
      simulation.inertia->addScaledStiffnessEntries(sqr(simulation.dt),
                                                    implicit_system_matrix);
    // stiffness matrix
    for (auto &lf : simulation.forces) {
  ...
  void multiply(const TVStack &x, TVStack &b) const {
    ...
    if (matrix_free) {
      ...
      if (!simulation.quasistatic)
        simulation.inertia->addScaledForceDifferential(-sqr(simulation.dt), x,
                                                       b);
      ...
      tbb::parallel_for(
          tbb::blocked_range<int>(0, b.cols(), (b.cols() * dim) / 32),
          [&](const tbb::blocked_range<int> &range) {
            int start = range.begin() * dim;
            int length = (range.end() - range.begin()) * dim;
            auto vec_b = vec(b);
            vec_b.segment(start, length) =
                implicit_system_matrix.middleRows(start, length) * vec(x);
          });
    }
  }

  template <class Func> void setProjection(Func project_func) {
    project = project_func;
  }

  template <class Func> void setPreconditioner(Func preconditioner_func) {
