...
/**
  ...
  diff_test_perturbation_scale allows you to choose how to scale the
  perturbation from simple power of 2s
*/
template <class T, int dim, class Objective>
void runDiffTest(const int num_nodes,
                 const Eigen::Matrix<T, dim, Eigen::Dynamic> &dv,
                 Objective &objective,
                 double diff_test_perturbation_scale = 1) {
  ZIRAN_INFO("Running diff test with perturbation scale: ",
  ...
  for (int i = 1; i <= 10; ++i) {
    ...
    TVStack f2 = f0 + f1;
    T differential = tbb::parallel_reduce(
        tbb::blocked_range<int>(0, f2.cols(), 256), (T)0,
  ...
  std::cout << std::setprecision(12) << "energy["
            << "i"
            << "] = " << std::setw(20) << "difference" << std::setw(20)
            << "differential" << std::setw(20) << "err" << std::setw(20)
            << "log_err" << std::endl;
  for (int i = 0; i < DIFF_SIZE; ++i) {
    std::cout << std::setprecision(12) << "energy[" << i
              << "] = " << std::setw(20) << energy_difference[i]
              << std::setw(20) << energy_differential[i] << std::setw(20)
              << energy_err[i] << std::setw(20) << energy_log_err[i]
              << std::endl;
  }
  std::cout << std::endl;
  std::cout << std::setprecision(12) << "force["
            << "i"
            << "] = " << std::setw(20) << "difference_norm" << std::setw(20)
            << "differential_norm" << std::setw(20) << "err" << std::setw(20)
            << "log_err" << std::endl;
  for (int i = 0; i < DIFF_SIZE; ++i) {
    std::cout << std::setprecision(12) << "force[" << i
              << "] = " << std::setw(20) << force_difference_norm[i]
              << std::setw(20) << force_differential_norm[i] << std::setw(20)
              << force_err[i] << std::setw(20) << force_log_err[i] << std::endl;
  }
  std::cin.get();
}
} // namespace ZIRAN
