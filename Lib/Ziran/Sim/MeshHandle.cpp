...
MeshReader<T, dim, SimplexMesh<1>>::MeshReader(const std::string &filename,
                                               const T percentage_to_keep_in)
    : filename(filename), percentage_to_keep(percentage_to_keep_in) {}

// readSegMesh
template <class T, int dim>
void MeshReader<T, dim, SimplexMesh<1>>::operator()(SimplexMesh<1> &mesh,
                                                    StdVector<TV> &X0) {
  ...
  if (ext == "data") {
    ...
    ZIRAN_ASSERT(percentage_to_keep == (T)1,
                 "reading poly segmesh doesn't support downsample");
...
template <class T, int dim>
void MeshReader<T, dim, SimplexMesh<2>>::operator()(SimplexMesh<2> &mesh,
                                                    StdVector<TV> &X0) {
  std::string ext = filename.substr(filename.find_last_of('.') + 1);
  std::string absolute_path = DataDir().absolutePath(filename);
  ZIRAN_INFO("Reading mesh ", filename);
...
template <class T, int dim>
void MeshReader<T, dim, SimplexMesh<3>>::operator()(SimplexMesh<3> &mesh,
                                                    StdVector<TV> &X0) {
  std::string ext = filename.substr(filename.find_last_of('.') + 1);
  std::string absolute_path = DataDir().absolutePath(filename);
  ZIRAN_INFO("Reading mesh ", filename);
...
MeshHandle<T, dim, TMesh>::MeshHandle(Particles<T, dim> &particles,
    ...
    : particles(particles), mesh(std::move(mesh_in)),
...
template <class T, int dim, class TMesh>
MeshHandle<T, dim, TMesh>::MeshHandle(
    Particles<T, dim> &particles, std::shared_ptr<TMesh> mesh_in,
...
MeshHandle<T, dim, TMesh>::MeshHandle(
    Particles<T, dim> &particles, std::shared_ptr<TMesh> mesh_in,
    const std::function<void(TMesh &, StdVector<TV> &, StdVector<TV> &)>
        mesh_constructor)
    : particles(particles), mesh(std::move(mesh_in)),
      particle_index_offset(particles.count) {
  StdVector<TV> &X0 = particles.X.array;
...
MeshHandle<T, dim, TMesh> MeshHandle<T, dim, TMesh>::copy() {
  return MeshHandle(particles, mesh, [=](TMesh &, StdVector<TV> &X0) {
    X0.insert(X0.end(), X0.begin() + particle_range.lower,
              X0.begin() + particle_range.upper);
  });
}

template <class T, int dim, class TMesh>
void MeshHandle<T, dim, TMesh>::transform(
    const std::function<void(int, Ref<T>, TV &, TV &)> &mapping) {
...
template <class T, int dim>
MeshHandle<T, dim, SimplexMesh<1>> constructTrimeshBoundaryWithParticles(
    MeshHandle<T, dim, SimplexMesh<2>> &trimesh) {
  ...
  HashTable<int, int>
      vertex_hash; // map tri mesh point to seg mesh point (both global indices)

  {
    ...
    for (auto segment : trimesh.mesh->boundary_indices) {
      ...
      if (vertex_hash.get(A) == nullptr) {
        ...
        ap.append(/*mass*/ (T)0, trimesh.particles.X[A + trimesh_offset],
                  trimesh.particles.V[A + trimesh_offset]);
      }
      if (vertex_hash.get(B) == nullptr) {
        vertex_hash[B] = offset++;
        ap.append(/*mass*/ (T)0, trimesh.particles.X[B + trimesh_offset],
                  trimesh.particles.V[B + trimesh_offset]);
      }
      Vector<int, 2> seg;
  ...
  MeshHandle<T, dim, SimplexMesh<1>> handle(
      trimesh.particles, segmesh, Range{old_count, old_count + offset});
  return handle;
}
...
template MeshHandle<double, 2, SimplexMesh<1>>
constructTrimeshBoundaryWithParticles<double, 2>(
    MeshHandle<double, 2, SimplexMesh<2>> &);
template MeshHandle<double, 3, SimplexMesh<1>>
constructTrimeshBoundaryWithParticles<double, 3>(
    MeshHandle<double, 3, SimplexMesh<2>> &);
template MeshHandle<float, 2, SimplexMesh<1>>
constructTrimeshBoundaryWithParticles<float, 2>(
    MeshHandle<float, 2, SimplexMesh<2>> &);
template MeshHandle<float, 3, SimplexMesh<1>>
constructTrimeshBoundaryWithParticles<float, 3>(
    MeshHandle<float, 3, SimplexMesh<2>> &);

template void
correctInverted<double, 3>(MeshHandle<double, 3, SimplexMesh<3>> &);
