...
template <class T, int dim> class SceneInitialization {
public:
  ...
  SceneInitialization(Scene<T, dim> &scene_in) : scene(scene_in) {}

  void
  setBoundaryConditions(const std::function<bool(int, TV &)> &is_dirichlet) {
  ...
  MeshHandle<T, dim, TMesh>
  createMesh(std::function<void(TMesh &, StdVector<TV> &)> mesh_constructor) {
    return MeshHandle<T, dim, TMesh>(scene.particles, std::make_shared<TMesh>(),
                                     mesh_constructor);
  }

  template <class TMesh>
  MeshHandle<T, dim, TMesh>
  createMesh(std::function<void(TMesh &, StdVector<TV> &, StdVector<TV> &)>
                 mesh_constructor) {
    return MeshHandle<T, dim, TMesh>(scene.particles, std::make_shared<TMesh>(),
                                     mesh_constructor);
  }
