...
*/
template <class T, int dim> struct ElementManagerProxy {
...
public:
  ...
  T totalEnergy() {
    return tbb::parallel_reduce(
        tbb::blocked_range<int>(0, forces.size()), (double)0,
  ...
  void addScaledForces(const T scale, TVStack &f) {
    ...
    if (element_partitions) { // parallel
      ...
      // gather forces
      for (auto &em : element_managers)
        em->gatherFromPadsToTVStack(scale,
                                    f); // this is internally a parallel_for
    }
  ...
  void addScaledForceDifferentials(const T scale, const TVStack &dx,
                                   TVStack &df) {
    if (element_managers.size() == 0)
      return;
    int element_partitions = element_managers[0]->pads.size();
    ...
    if (element_partitions) { // parallel
      ...
      for (auto &em : element_managers)
        tbb::parallel_for(tbb::blocked_range<int>(0, em->pads.size()),
                          [&](const tbb::blocked_range<int> &r) {
                            for (int k = r.begin(); k < r.end();
                                 ++k) {              // k is pad id
                              em->pads[k].setZero(); // zero out pads

                              // build pad_dx
                              TVStack &pad_dx = em->pads_dx[k];
                              for (int t = 0; t < pad_dx.cols();
                                   t++) // t is pad k's local particle index
      ...
      for (auto &em : element_managers)
        em->gatherFromPadsToTVStack(scale,
                                    df); // this is internally a parallel_for
    }
  ...
  template <class Type> bool exist() {
    for (auto &emp : element_managers)
      if (Type *em = dynamic_cast<Type *>(emp.get()))
        return true;
    return false;
  ...
  /* binary ver 1 */
  void writeState(std::ostream &out, const std::string &boundary_filename,
                  const std::string &segment_filename, DataDir &output_dir,
                  int binary_ver, bool write_meshes = true) {
    if (write_meshes && (trimesh_to_write.indices.size() != 0)) {
      std::ofstream boundary_file =
          output_dir.openTextOutput(boundary_filename);
    ...
    if (write_meshes && segmesh_to_write.indices.size() != 0) {
      ...
      writeSegmeshPoly(segmesh_file, particles.X.array,
                       segmesh_to_write.indices);
    }
