...
public:
  ...
  RestartException(int frame)
      : std::runtime_error("Restart Simulation"), frame(frame) {}
};
...
public:
  ...
  StdVector<std::function<void(int, int, double, double)>>
      begin_time_step_callbacks;
  ...
  SimulationBase()
      : frame(0), substep(0), restarting(false), output_dir("output"),
        write_substeps(false), write_files(true), write_log(true),
        verbose(false), start_frame(0), end_frame(0), restart_frame(0),
        logger(nullptr), diff_test(false),
        diff_test_perturbation_scale((double)1), full_implicit(false),
  ...
  */
  virtual void read(const std::string &filename) {
    int binary_ver_data;
    try {
      std::ifstream binary_ver_file =
          output_dir.openTextInput("binary_ver.txt");
      binary_ver_file >> binary_ver_data;
    } catch (std::exception &e) {
      binary_ver_data = 0;
    }
    ZIRAN_ASSERT(binary_ver_data <= binary_ver,
                 "The data to read is of newer version. Update code base!");
    ...
    ZIRAN_WARN_IF(last_written_frame < frame,
                  "Frame is after the last written frame ", last_written_frame,
                  ". Could be reading stale data");
  ...
  */
  virtual std::string outputFileName(const std::string &name = "restart",
  ...
  */
  ...
  virtual void advanceOneFrame() {
    ...
    do {
      ...
      ZIRAN_EVENT(Color::MAGENTA, "Substep ", std::setw(8), std::right, substep,
                  Color::RESET);
      if (overwrite_mode)
