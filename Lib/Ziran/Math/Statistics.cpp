...
namespace ZIRAN {
Quantiler::Quantiler(size_t num_bins) : num_bins(num_bins), num_samples(0) {}

void Quantiler::insert(double sample) {
  num_samples++;
  auto upper =
      std::upper_bound(marker_height.begin(), marker_height.end(), sample);
  ...
  for (size_t k = upper - marker_height.begin(); k < marker_position.size();
       k++) {
    marker_position[k]++;
  }
  ...
  for (size_t i = 1; i < num_bins; i++) {
    ...
    if (desired_position > ni && diffp1 > 1) {
      double q_quadratic = qi + ((diffm1 + 1) * (qip1 - qi) / diffp1 +
                                 (diffp1 - 1) * (qi - qim1) / diffm1) /
                                    (nip1 - nim1);
...
void Quantiler::print(std::ostream &out) {
  ...
  for (size_t i = 0; i < marker_position.size(); i++) {
    ...
    out << std::setprecision(1) << std::fixed << std::setw(4) << std::left
        << quantile << "%: ";
    out << std::setprecision(3) << std::right << std::scientific
        << marker_height[i] << "\n";
  }
}

void Quantiler::constructCumulativeHistogram(StdVector<double> &histogram,
                                             double start_quantile,
                                             double end_quantile) {
  if (marker_height.size() <= num_bins) {
    return;
  }
  ...
  for (size_t i = 0; i < width; i++) {
    ...
    histogram[i] =
        (alpha * marker_position[k - 1] + (1.0 - alpha) * marker_position[k]) /
        num_samples;
  }
}
...
/**
  ...
  cumulative one
  */
double Quantiler::constructHistogram(StdVector<double> &histogram,
                                     double start_quantile,
                                     double end_quantile) {
  size_t width = histogram.size();
...
void Quantiler::printGraph(std::ostream &out, const StdVector<double> &y,
                           double data_height, size_t plot_width,
                           size_t plot_height) {
...
void Quantiler::printScale(std::ostream &out, size_t width,
                           double start_quantile, double end_quantile) {
  size_t field_width = 16;
  out << std::left << std::setprecision(1) << std::scientific;
  size_t d = width / field_width;
  for (size_t i = 0; i < d - 1; i++) {
    size_t k = closetsBinToQuantile(start_quantile +
                                    (end_quantile - start_quantile) * i / d);
    out << std::setw(field_width) << marker_height[k];
  }
  out << std::right << std::setw(field_width)
      << marker_height[closetsBinToQuantile(end_quantile)];
  out << std::endl;
}

void Quantiler::printCumulativeHistogram(std::ostream &out, size_t plot_width,
                                         size_t plot_height,
                                         double start_quantile,
                                         double end_quantile) {
  if (marker_height.size() <= num_bins) {
...
void Quantiler::printHistogram(std::ostream &out, size_t plot_width,
                               size_t plot_height, double start_quantile,
                               double end_quantile) {
  if (marker_height.size() <= num_bins) {
    out << "Not enough data\n";
    return;
