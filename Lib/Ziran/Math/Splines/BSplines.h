...
template <int interpolation_degree, class T,
          TICK_REQUIRES(interpolation_degree == 1)>
...
*/
template <class T>
inline void computeBSplineWeights(const T x, int &base_node, Vector<T, 2> &w,
                                  Vector<T, 2> *dw = 0, Vector<T, 2> *ddw = 0) {
...
*/
template <class T>
inline void computeBSplineWeights(const T x, int &base_node, Vector<T, 3> &w,
                                  Vector<T, 3> *dw = 0, Vector<T, 3> *ddw = 0) {
...
*/
template <class T>
inline void computeBSplineWeights(const T x, int &base_node, Vector<T, 4> &w,
                                  Vector<T, 4> *dw = 0, Vector<T, 4> *ddw = 0) {
...
template <class T>
inline void computeBSplineWeightsBoundary(const T x, const int size,
                                          int &base_node, Vector<T, 2> &w,
                                          Vector<T, 2> *dw = 0,
                                          Vector<T, 2> *ddw = 0) {
  base_node = baseNode<1>(x);
  // std::cout<<" X is "<<x<<" size is "<<size<<" base node is
  // "<<base_node<<std::endl;
  if (base_node >= size - 3) {
    // std::cout << " X is " << x << " size is " << size << " base node is " <<
    // base_node << std::endl;
...
inline void computeBSplineWeightsBoundary(const T x, const int size,
  ...
  T x2 = x * x;
  T x3 = x * x2;
  //  std::cout<<"base node is "<<base_node<<" size is
  //  "<<size<<std::endl<<std::flush;
  if (base_node >= 1 && base_node < size - 6) {
    ...
    //  std::cout<<"size-3 is "<< x<< "base node is "<<
    //  base_node<<std::endl<<std::flush;
    ...
    //   std::cout<<"size-2 is "<< x<< "base node is "<<
    //   base_node<<std::endl<<std::flush;
    T z = x - base_node_x - (T)1;
    T z2 = z * z;
    T z3 = z * z * z;
    ...
    std::cout << "Base node is" << base_node << " size is " << size << " X is "
              << x << std::endl
              << std::flush;
  }
}
