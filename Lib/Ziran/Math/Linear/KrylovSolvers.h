...
template <class TV> struct DirichletDofProjection {
...
template <class TV> struct JacobiPreconditioner {
  ...
  JacobiPreconditioner(
      const TM &mat,
      typename TV::Scalar tol =
          16 * std::numeric_limits<typename TV::Scalar>::epsilon()) {
    ...
    static_assert(rows != Eigen::Dynamic || cols != Eigen::Dynamic,
                  "rows and columns can't both be dynamic");
    ...
    EIGEN_EXT::vec(inverse_diagonal) = mat.diagonal().unaryExpr(
        [tol](T x) -> T { return (T)1 / std::max(tol, std::abs(x)); });
  }
...
template <class T, class TV> class EigenSparseKrylovMatrix {
public:
  ...
  EigenSparseKrylovMatrix(TM &A, Func1 project, Func2 precondition)
      : A(A), project(project), precondition(precondition) {}
