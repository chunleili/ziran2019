...
class Minres : public LinearSolver<T, TM, TV> {
  ...
  T gamma, delta, epsilon; // These are the newest entries in R from the QR of
                           // Hk
  T beta_kp1, alpha_k, beta_k, sk; // This is the last column in Hk
  TV mk, mkm1,
      mkm2; // mk is the newest search direction in the memory friendly basis
            // for the Krylov space, you need the other two to generate mk
  TV z, qkp1, qk, qkm1; // These are the Lanczos vectors needed at each
                        // iteration, qk is denoted as vk in the notes
  T tk;                 // This is the step length in the direction of mk
  TV2 last_two_components_of_givens_transformed_least_squares_rhs; // This will
                                                                   // track the
                                                                   // residual
                                                                   // with just
                                                                   // a constant
...
public:
  Minres(const int max_it_input)
      : Base(max_it_input), Gk(0, 1), Gkm1(0, 1), Gkm2(0, 1) {}
  ...
  int solve(const TM &A, TV &x, const TV &b, const bool verbose = false) {
    ...
    T local_tolerance =
        std::min(Base::relative_tolerance * residual_preconditioned_norm,
                 Base::tolerance);

    // check for convergence on initial guess
    if (residual_preconditioned_norm < local_tolerance) {
      ZIRAN_VERB_IF(
          verbose, "\tMinres terminates at ", 0,
          "; (preconditioned norm) residual = ", residual_preconditioned_norm);
    ...
    last_two_components_of_givens_transformed_least_squares_rhs =
        TV2(residual_preconditioned_norm, 0);

    ZIRAN_VERB_IF(
        verbose, "\tMinres iter ", 0,
        "; (preconditioned norm) residual = ", residual_preconditioned_norm);
    for (int k = 0; k < Base::max_iterations; k++) {
      ZIRAN_CONTEXT(residual_preconditioned_norm);
      ZIRAN_CONTEXT("Minres step", k);
      if (residual_preconditioned_norm < local_tolerance) {
        ZIRAN_VERB_IF(verbose, "\tMinres terminates at ", k,
                      "; (preconditioned norm) residual = ",
      ...
      // Equivalent to qkm1 = qk; qk = qkp1; And qkp1 will be overwritten right
      // after anyway.
      qkm1.swap(qkp1);
      qkm1.swap(qk);

      A.multiply(mk, qkp1); // qpk1 = A * zk; Get the important part of the next
                            // Lanczos vector: q_k+1
      A.project(qkp1);
      ...
      residual_preconditioned_norm =
          applyAllPreviousGivensRotationsAndDetermineNewGivens(); // This
                                                                  // determines
                                                                  // the newest
                                                                  // Givens
                                                                  // rotation
      ...
      // Three term recurence for the m's, mk stores the old zk which is set at
      // beginning of the iteration
      mk = (mk - delta * mkm1 - epsilon * mkm2) / gamma;
      x += tk * mk;

      if ((k + 1) % 50 == 0) {
        ZIRAN_VERB_IF(verbose, "\tMinres iter ", k + 1,
                      "; (preconditioned norm) residual = ",
                      residual_preconditioned_norm);
      }
    }

    ZIRAN_VERB_IF(verbose, "\tMinres terminates at ", Base::max_iterations,
                  " (max reached); (preconditioned norm) residual = ",
                  residual_preconditioned_norm);
  ...
  T applyAllPreviousGivensRotationsAndDetermineNewGivens() {
    ...
    last_two_components_of_givens_transformed_least_squares_rhs =
        TV2(residual, 0); // Set up for the next iteration
    if (residual < 0)
      return -residual;
    else
      return residual;
