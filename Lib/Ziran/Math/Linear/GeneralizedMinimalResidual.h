...
namespace ZIRAN {
template <class T, class TM, class TV> class GeneralizedMinimalResidual {
...
public:
  ...
  GeneralizedMinimalResidual(const int max_it_input)
      : max_iterations(max_it_input), relative_tolerance(1) {
    setTolerance();
  }
  ...
  void setMaxIteration(const int max_it_input) {
    ...
    for (int k = 0, system_size;; k++, system_size++) {
      ...
      if (residual <= local_tolerance || k == max_iterations) {
        ...
        if (residual <= local_tolerance) {
          ZIRAN_VERB_IF(true, "GMRES terminates at ", k + 1,
                        "; residual = ", residual);
          return k; // Output the number of iterations.
        } else {
          ZIRAN_VERB_IF(true, "GMRES terminates at ", max_iterations + 1,
                        " (max reached); residual = ", residual);
