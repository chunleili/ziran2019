...
public:
  ...
  ZIRAN_FORCE_INLINE void transposeInPlace() { s = -s; }
  ...
  /**
    Multiply givens must be for same row and column
    **/
  ZIRAN_FORCE_INLINE GivensRotation<T>
...
/**
    ...
    */
template <class T>
inline ZIRAN_FORCE_INLINE void zeroChase(Matrix<T, 3, 3> &H, Matrix<T, 3, 3> &U,
                                         Matrix<T, 3, 3> &V) {
...
/**
  ...
  */
template <class T>
inline ZIRAN_FORCE_INLINE void
makeUpperBidiag(Matrix<T, 3, 3> &H, Matrix<T, 3, 3> &U, Matrix<T, 3, 3> &V) {
...
template <class T>
inline ZIRAN_FORCE_INLINE void
makeLambdaShape(Matrix<T, 3, 3> &H, Matrix<T, 3, 3> &U, Matrix<T, 3, 3> &V) {
  U = Matrix<T, 3, 3>::Identity();
  V = Matrix<T, 3, 3>::Identity();
