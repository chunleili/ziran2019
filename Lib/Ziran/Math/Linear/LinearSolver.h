...
namespace ZIRAN {
template <class T, class TM, class TV> class LinearSolver {
...
public:
  ...
  virtual void setRelativeTolerance(const T tolerance_input = 1) {
    relative_tolerance = tolerance_input;
  }

  virtual void setMaxIteration(const int max_it_input) {
  ...
  virtual int solve(const TM &A, TV &x, const TV &b,
