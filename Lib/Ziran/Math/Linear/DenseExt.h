...
static void dPdFContractTwoVectors(TM &result, const Hessian &A, const TV1 &u,
                                   const TV2 &v) {
  result = TM::Zero();
  ...
  for (int delta = 0; delta < cols; delta++) {
    for (int gamma = 0; gamma < rows; gamma++) {
      result += A.template block<rows, cols>(rows * gamma, cols * delta) *
                u(gamma) * v(delta);
    }
  }
}
...
*/
template <class Derived> auto norm(const Eigen::MatrixBase<Derived> &x) {
  return x.norm();
}
...
*/
template <class Derived> auto squaredNorm(const Eigen::MatrixBase<Derived> &x) {
...
*/
template <class T, int m, int n, int flags,
          TICK_REQUIRES((flags & Eigen::ColMajor) == Eigen::ColMajor)>
...
/**
   ...
   Computes  \f$\delta f_{ij} = \frac{\partial f_{ij}}{\partial x_{kl}} \delta
   x_{kl}\f$ Assumes the storage order of dfdx is such that this contraction is
   dfdx*vec(dx) where where vec(dx) is dx as a vector in its storage order.
*/
template <class T, int k, int l, int m, int n, int kl, int mn, int flags,
          TICK_REQUIRES(kl == k * l && mn == m * n)>
void contract(Eigen::Matrix<T, k, l, flags, k, l> &df,
              const Eigen::Matrix<T, kl, mn, flags, kl, mn> &dfdx,
              const Eigen::Matrix<T, m, n, flags, m, n> &dx) {
  vec(df).noalias() = dfdx * vec(dx);
}

template <class T, int k, int l, int m, int n, int options, int flags>
void contract(Eigen::Matrix<T, k, l, flags, k, l> &df,
              const Eigen::SparseMatrix<T, options> &dfdx,
              const Eigen::Matrix<T, m, n, flags, m, n> &dx) {
  vec(df).noalias() = dfdx * vec(dx);
}
...
*/
template <class OutT, class Derived,
          TICK_REQUIRES(std::is_arithmetic<OutT>::value)>
void contract(OutT &df, const Eigen::MatrixBase<Derived> &dfdx,
...
*/
template <class InT, class OutT, class DerT,
          TICK_REQUIRES(std::is_arithmetic<InT>::value)>
...
template <class InT, class OutT, class DerT,
          TICK_REQUIRES(
              std::is_arithmetic<OutT>::value &&InT::ColsAtCompileTime == 1)>
void contract(OutT &df, const DerT &dfdx, const InT &dx) {
  df = dfdx.dot(dx);
}
...
*/
template <class Mat> ScalarType<Mat> firstInvariant(const Mat &F) {
...
template <class TF, class TA,
          TICK_REQUIRES(isSize<TF>(1, 1) && isSize<TA>(1, 1))>
...
*/
template <class TF, class TA,
          TICK_REQUIRES(isSize<TF>(2, 2) && isSize<TA>(2, 2))>
inline void cofactorMatrix(const TF &F, TA &A) {
  A(0, 0) = F(1, 1);
  A(1, 0) = -F(0, 1);
  A(0, 1) = -F(1, 0);
...
*/
template <class TF, class TA,
          TICK_REQUIRES(isSize<TF>(3, 3) && isSize<TA>(3, 3))>
inline void cofactorMatrix(const TF &F, TA &A) {
...
template <class T, int dim>
inline void
addScaledTensorProduct_ik_jl(const Matrix<T, dim, dim> &A,
                             const Matrix<T, dim, dim> &B, const T &scale,
                             Eigen::Matrix<T, dim * dim, dim * dim> &M) {
...
template <class T, int dim>
inline void tensorProduct_ik_jl(const Matrix<T, dim, dim> &A,
                                const Matrix<T, dim, dim> &B,
                                Eigen::Matrix<T, dim * dim, dim * dim> &M) {
  for (int j2 = 0; j2 < dim; j2++)
    for (int i2 = 0; i2 < dim; i2++)
      for (int j1 = 0; j1 < dim; j1++)
...
template <class T, int dim>
inline void
addScaledTensorProduct_il_kj(const Matrix<T, dim, dim> &A,
                             const Matrix<T, dim, dim> &B, const T &scale,
                             Eigen::Matrix<T, dim * dim, dim * dim> &M) {
  for (int j2 = 0; j2 < dim; j2++)
    for (int i2 = 0; i2 < dim; i2++)
...
template <class T, int dim>
inline void tensorProduct_il_kj(const Matrix<T, dim, dim> &A,
                                const Matrix<T, dim, dim> &B,
                                Eigen::Matrix<T, dim * dim, dim * dim> &M) {
  for (int j2 = 0; j2 < dim; j2++)
    for (int i2 = 0; i2 < dim; i2++)
...
template <class T>
void rotationalDifferential(const Vector<T, 1> &R, const Vector<T, 1> &S,
                            const Vector<T, 1> &dF, Vector<T, 1> &dR) {
  dR(0, 0) = (T)0;
}
template <class T>
void addScaledRotationalDifferential(const Vector<T, 1> &R,
                                     const Vector<T, 1> &S,
                                     const Vector<T, 1> &dF, T scale,
                                     Vector<T, 1> &M) {}
template <class T>
void rotationalDifferential(const Matrix<T, 2, 2> &R, const Matrix<T, 2, 2> &S,
                            const Matrix<T, 2, 2> &dF, Matrix<T, 2, 2> &dR) {
  // We dont need the result of R^T dF after the omega stage, so we use dR to
...
template <class T>
void addScaledRotationalDifferential(const Matrix<T, 2, 2> &R,
                                     const Matrix<T, 2, 2> &S,
                                     const Matrix<T, 2, 2> &dF, T scale,
                                     Matrix<T, 2, 2> &M) {
  T trace_s = S.trace();
  ZIRAN_ASSERT(trace_s != 0, "dR computation encountered division by zero");
  // Majorly exploited but basically derived from "rotationalDifferential'
  T omega = (R(0, 0) * dF(0, 1) + R(1, 0) * dF(1, 1) - R(0, 1) * dF(0, 0) -
             R(1, 1) * dF(1, 0)) /
            trace_s * scale;
  M(0, 0) -= omega * R(0, 1);
  M(1, 0) -= omega * R(1, 1);
...
*/
template <class T>
void addScaledRotationalDerivative(const Vector<T, 1> &R, const Vector<T, 1> &S,
                                   T scale, Vector<T, 1> &M) {}
/**
...
*/
template <class T>
void rotationalDerivative(const Matrix<T, 2, 2> &R, const Matrix<T, 2, 2> &S,
                          Matrix<T, 4, 4> &dRdF) {
  T trace_s = S.trace();
...
*/
template <class T>
void addScaledRotationalDerivative(const Matrix<T, 2, 2> &R,
                                   const Matrix<T, 2, 2> &S, T scale,
                                   Matrix<T, 4, 4> &M) {
  T trace_s = S.trace();
  ZIRAN_ASSERT(trace_s != 0, "dRdF computation encountered division by zero");
...
*/
template <class T>
void rotationalDerivative(const Matrix<T, 3, 3> &R, const Matrix<T, 3, 3> &S,
                          Matrix<T, 9, 9> &dRdF) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      for (int k = 0; k < 3; k++) {
        for (int l = 0; l < 3; l++) {
          ...
          dRdF(row_index, column_index) =
              aRS_hatRT(i, k) * S_hat(j, l) - aRS_hat(i, l) * RS_hat(k, j);
        }
      }
    }
  }
...
template <class T>
void rotationalDifferential(const Matrix<T, 3, 3> &R, const Matrix<T, 3, 3> &S,
                            const Matrix<T, 3, 3> &dF, Matrix<T, 3, 3> &dR) {
  using TM = Matrix<T, 3, 3>;
...
template <class T>
void addScaledRotationalDifferential(const Matrix<T, 3, 3> &R,
                                     const Matrix<T, 3, 3> &S,
                                     const Matrix<T, 3, 3> &dF, T scale,
                                     Matrix<T, 3, 3> &M) {
  using TM = Matrix<T, 3, 3>;
...
*/
template <class T>
void addScaledRotationalDerivative(const Matrix<T, 3, 3> &R,
                                   const Matrix<T, 3, 3> &S, T scale,
                                   Matrix<T, 9, 9> &M) {
  using TM = Matrix<T, 3, 3>;
  TM S_hat = -S;
  S_hat.diagonal().array() += S.trace();
...
template <class T>
void QRDifferential(const Vector<T, 1> &Q, const Vector<T, 1> &R,
                    const Vector<T, 1> &dF, Vector<T, 1> &dQ,
                    Vector<T, 1> &dR) {
  dQ(0) = (T)0;
  dR(0) = dF(0);
}
...
template <class T>
void QRDifferential(const Matrix<T, 2, 2> &Q, const Matrix<T, 2, 2> &R,
                    const Matrix<T, 2, 2> &dF, Matrix<T, 2, 2> &dQ,
                    Matrix<T, 2, 2> &dR) {
...
template <class T>
void QRDifferential(const Matrix<T, 3, 3> &Q, const Matrix<T, 3, 3> &R,
                    const Matrix<T, 3, 3> &dF, Matrix<T, 3, 3> &dQ,
                    Matrix<T, 3, 3> &dR) {
  using TM = Matrix<T, 3, 3>;
  ZIRAN_ASSERT(R(0, 0) != 0 && R(1, 1) != 0,
               "dQ computation encountered division by zero");
  TM QtdF = Q.transpose() * dF;
  T w3 = QtdF(1, 0) / R(0, 0);
  T w2 = -QtdF(2, 0) / R(0, 0);
...
template <class T> bool nearKink(const Matrix<T, 1, 1> &S, T tol) {
  return false;
}

template <class T> bool nearKink(const Matrix<T, 2, 2> &S, T tol) {
  T trace_s = S.trace();
  return std::abs(trace_s) < tol;
}

template <class T> bool nearKink(const Matrix<T, 3, 3> &S, T tol) {
  using TM = Matrix<T, 3, 3>;
  TM S_hat = -S;
  S_hat.diagonal().array() += S.trace();
  T b = S_hat.determinant();
...
*/
template <class T>
void cofactorMatrixDifferential(const Vector<T, 1> &F, const Vector<T, 1> &dF,
                                Vector<T, 1> &result) {
  result = dF;
}
template <class T>
void addScaledCofactorMatrixDifferential(const Vector<T, 1> &F,
                                         const Vector<T, 1> &dF, T scale,
                                         Vector<T, 1> &M) {
  M += scale * dF;
}
...
*/
template <class T>
void cofactorMatrixDifferential(const Matrix<T, 2, 2> &F,
                                const Matrix<T, 2, 2> &dF,
                                Matrix<T, 2, 2> &result) {
  cofactorMatrix(dF, result);
}

template <class T>
void addScaledCofactorMatrixDifferential(const Matrix<T, 2, 2> &F,
                                         const Matrix<T, 2, 2> &dF, T scale,
                                         Matrix<T, 2, 2> &M) {
  M(0, 0) += scale * dF(1, 1);
  M(1, 0) -= scale * dF(0, 1);
...
*/
template <class T>
void addScaledCofactorMatrixDerivative(const Vector<T, 1> &F, T scale,
                                       Vector<T, 1> &result) {
  result(0, 0) += scale;
}
...
/**
   \brief 1X1 d( JF^(-T) )/dF
   \param[in] F input matrix
   \param[out] result d( JF^(-T) )/dF << dP11/dF11, dP11/dF21, dP11/dF12,
   dP11/dF22, dP21/dF11, ...
*/
template <class T>
...
*/
template <class T>
void addScaledCofactorMatrixDerivative(const Matrix<T, 2, 2> &F, T scale,
                                       Matrix<T, 4, 4> &result) {
  result(3, 0) += scale;
  result(2, 1) -= scale;
  result(1, 2) -= scale;
...
*/
template <class T>
void cofactorMatrixDerivative(const Matrix<T, 2, 2> &F,
                              Matrix<T, 4, 4> &result) {
  result(0, 0) = 0;
  result(1, 0) = 0;
  result(2, 0) = 0;
...
*/
template <class T>
void cofactorMatrixDifferential(const Matrix<T, 3, 3> &F,
                                const Matrix<T, 3, 3> &dF,
                                Matrix<T, 3, 3> &result) {
  result(0, 0) = dF(1, 1) * F(2, 2) + F(1, 1) * dF(2, 2) - dF(2, 1) * F(1, 2) -
                 F(2, 1) * dF(1, 2);
  result(1, 0) = dF(2, 1) * F(0, 2) + F(2, 1) * dF(0, 2) - dF(0, 1) * F(2, 2) -
                 F(0, 1) * dF(2, 2);
  result(2, 0) = dF(0, 1) * F(1, 2) + F(0, 1) * dF(1, 2) - dF(1, 1) * F(0, 2) -
                 F(1, 1) * dF(0, 2);
  result(0, 1) = dF(2, 0) * F(1, 2) + F(2, 0) * dF(1, 2) - dF(1, 0) * F(2, 2) -
                 F(1, 0) * dF(2, 2);
  result(1, 1) = dF(0, 0) * F(2, 2) + F(0, 0) * dF(2, 2) - dF(2, 0) * F(0, 2) -
                 F(2, 0) * dF(0, 2);
  result(2, 1) = dF(1, 0) * F(0, 2) + F(1, 0) * dF(0, 2) - dF(0, 0) * F(1, 2) -
                 F(0, 0) * dF(1, 2);
  result(0, 2) = dF(1, 0) * F(2, 1) + F(1, 0) * dF(2, 1) - dF(2, 0) * F(1, 1) -
                 F(2, 0) * dF(1, 1);
  result(1, 2) = dF(2, 0) * F(0, 1) + F(2, 0) * dF(0, 1) - dF(0, 0) * F(2, 1) -
                 F(0, 0) * dF(2, 1);
  result(2, 2) = dF(0, 0) * F(1, 1) + F(0, 0) * dF(1, 1) - dF(1, 0) * F(0, 1) -
                 F(1, 0) * dF(0, 1);
}
template <class T>
void addScaledCofactorMatrixDifferential(const Matrix<T, 3, 3> &F,
                                         const Matrix<T, 3, 3> &dF, T scale,
                                         Matrix<T, 3, 3> &M) {
...
*/
template <class T>
void addScaledCofactorMatrixDerivative(const Matrix<T, 3, 3> &F, T scale,
                                       Matrix<T, 9, 9> &result) {
...
/**
   \brief 3X3 d( JF^(-T) )/dF
   \param[in] F input matrix
   \param[out] result d( JF^(-T) )/dF << dP11/dF11, dP11/dF21, ..., dP11/dF33,
   dP21/dF11, ...
*/
template <class T>
void cofactorMatrixDerivative(const Matrix<T, 3, 3> &F,
                              Matrix<T, 9, 9> &result) {
  result.setZero();
