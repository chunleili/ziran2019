...
public:
  ConjugateGradient(const int max_it_input) : Base(max_it_input) {}
  ...
  int solve(const TM &A, TV &x, const TV &b, const bool verbose = false) {
    ...
    A.project(r);
    A.precondition(
        r, q); // NOTE: requires that preconditioning matrix is projected
    p = q;
    ...
    for (cnt = 0; cnt < Base::max_iterations; ++cnt) {
      if (residual_preconditioned_norm < local_tolerance) {
        ZIRAN_VERB_IF(verbose, "\tCG terminates at ", cnt,
      ...
      if (cnt % 50 == 0) {
        ZIRAN_VERB_IF(verbose, "\tCG iter ", cnt,
                      "; (preconditioned norm) residual = ",
                      residual_preconditioned_norm);
      }
