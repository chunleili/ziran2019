...
template <class T> T wendlandKernelQ(const T q) {
  using MATH_TOOLS::to_the_fourth;
  T M = (q < 2) ? ((1 + 2 * q) * to_the_fourth(1 - q / 2)) : 0;
  return M;
}

template <class T> T wendlandKernelGradientQ(const T q) {
  using MATH_TOOLS::cube;
  using MATH_TOOLS::to_the_fourth;
  return (q < 2) ? (to_the_fourth(q - (T)2) / (T)8 +
                    cube(q - 2) * (1 + 2 * q) / (T)4)
                 : 0;
}
...
template <class TV>
typename TV::Scalar wendlandKernel(const TV &xa, const TV &xb,
                                   const typename TV::Scalar h) {
  using MATH_TOOLS::cube;
  using T = typename TV::Scalar;
  static const int dim = TV::RowsAtCompileTime;
  static const T Cd =
      (dim == 2) ? ((T)7 / ((T)4 * M_PI)) : ((T)21 / ((T)16 * M_PI));
  T r = (xa - xb).norm();
  T q = r / h;
...
template <class TV>
TV wendlandKernelGradient(const TV &xa, const TV &xb,
                          const typename TV::Scalar h) {
  static const int dim = TV::RowsAtCompileTime;
  using T = typename TV::Scalar;
  static const T Cd =
...
template <class T> T spikyKernelGradientQ(const T q) {
...
// This computes grad W_b(x_a)
template <class TV>
TV spikyKernelGradient(const TV &xa, const TV &xb,
                       const typename TV::Scalar h) {
  using T = typename TV::Scalar;
  static const int dim = TV::RowsAtCompileTime;
  static const T Cd =
      (dim == 2) ? ((T)5 / ((T)16 * M_PI)) : ((T)15 / ((T)64 * M_PI));
  T r = (xa - xb).norm();
  T q = r / h;
...
template <class T> T cubicBSplineKernelQ(const T q) {
  using MATH_TOOLS::cube;
  T M = 0;
  if (q < 1)
...
template <class T> T cubicBSplineKernelGradientQ(const T q) {
  using MATH_TOOLS::sqr;
...
template <class T> T cubicBSplineKernelLaplacianQ(const T q) {
  using MATH_TOOLS::sqr;
...
// This computes W_b(x_a)
template <class TV>
typename TV::Scalar cubicBSplineKernel(const TV &xa, const TV &xb,
                                       const typename TV::Scalar h) {
  using MATH_TOOLS::cube;
  using T = typename TV::Scalar;
  static const int dim = TV::RowsAtCompileTime;
...
// This computes grad W_b(x_a)
template <class TV>
TV cubicBSplineKernelGradient(const TV &xa, const TV &xb,
                              const typename TV::Scalar h) {
  using T = typename TV::Scalar;
  static const int dim = TV::RowsAtCompileTime;
  static const T Cd = (dim == 2) ? (15.0 / (7.0 * M_PI)) : (3.0 / (2.0 * M_PI));
...
template <class TV>
typename TV::Scalar cubicBSplineKernelLaplacian(const TV &xj, const TV &xi,
                                                const typename TV::Scalar h) {
  static const int dim = TV::RowsAtCompileTime;
  using T = typename TV::Scalar;
  static const T Cd = (dim == 2) ? (15.0 / (7.0 * M_PI)) : (3.0 / (2.0 * M_PI));
