...
inline static Vector<int, 2> int_floor(const Vector<T, 2> &v) {
  return Vector<int, 2>(int_floor(v(0)), int_floor(v(1)));
}

template <class T>
inline static Vector<int, 3> int_floor(const Vector<T, 3> &v) {
  return Vector<int, 3>(int_floor(v(0)), int_floor(v(1)), int_floor(v(2)));
}
...
inline static Vector<float, 2>
pseudorand_2f(const Eigen::MatrixBase<Derived> &seed) {
  // multiply by invertible matrix mod 2^32
  // to ensure each pseudo random seed depends on
  // all 3 coordinates
  return Vector<float, 2>(pseudorand_f(12 * seed(0) + 89 * seed(1)),
                          pseudorand_f(32 * seed(0) + 35 * seed(1)));
}

template <class Derived>
inline static Vector<float, 3>
pseudorand_3f(const Eigen::MatrixBase<Derived> &seed) {
  // multiply by invertible matrix mod 2^32
  // to ensure each pseudo random seed depends on
  // all 3 coordinates
...
constexpr int power(int base, unsigned int exponent, int result = 1) {
  return (exponent < 1) ? result
                        : power(base * base, exponent / 2,
                                (exponent % 2) ? result * base : result);
}
...
template <class T> inline T to_the_fourth(T a) { return a * a * a * a; }

template <class T> inline T clamp_small_magnitude(const T x, const T eps) {
  assert(eps >= 0);
  if (x < -eps)
    return x;
  else if (x < 0)
...
template <class T> inline void clamp(T &x, const T min, const T max) {
  assert(max >= min);
  if (x < min)
    x = min;
  else if (x > max)
 ...
 */
template <class T> inline T log_1px_over_x(const T x, const T eps) {
  assert(eps > 0);
  if (std::fabs(x) < eps)
    return (T)1;
  else
 ...
 */
template <class T> inline T exp_m1x_over_x(const T x, const T eps) {
  assert(eps > 0);
  if (std::fabs(x) < eps)
    return (T)1;
  else
 ...
 */
template <class T> inline T sinx_over_x(const T x, const T eps) {
  assert(eps > 0);
  if (std::fabs(x) < eps)
    return 1;
  else
 ...
 */
template <class T>
inline T diff_interlock_log_over_diff(const T x, const T y, const T logy,
                                      const T eps) {
  assert(eps > 0);
