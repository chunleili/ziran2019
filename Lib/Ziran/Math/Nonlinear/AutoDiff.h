...
template <class T, class DiffT> class AutoDiff {
public:
  T s;
  DiffT ds;
  ...
  AutoDiff(const T &s, const DiffT &ds) : s(s), ds(ds) {}

  template <class OtherDiffT>
  AutoDiff(const AutoDiff<T, OtherDiffT> &x) : s(x.s), ds(x.ds) {}
  ...
  static constexpr AutoDiff Zero() { return AutoDiff((T)0); }
  ...
  auto operator/(const AutoDiff<T, OtherDiffT> &x) const {
    ...
    return make_auto_diff(s * one_over_x,
                          (ds * x.s - x.ds * s) * one_over_x * one_over_x);
  }

  auto operator+(const T &x) const { return make_auto_diff(s + x, ds); }
  ...
  auto operator*(const T &x) const { return make_auto_diff(s * x, ds * x); }
  ...
  auto operator-() const { return make_auto_diff(-s, -ds); }
  ...
  bool operator<(const T &x) const { return s < x; }
...
template <int num_vars, class T> inline auto var(const T &s, int var_id) {
  return make_auto_diff(s, Vector<T, num_vars>::Unit(var_id));
}

template <class T, int m, int n> inline auto vars(const Matrix<T, m, n> &x) {
...
inline auto vars(const Matrix<T, m, n> &x,
                 const Matrix<T, m * n, num_vars> &dx_dv) {
...
inline auto fillJacobianTranspose(
    const Matrix<AutoDiff<T, DiffT>, m, n> &x, Matrix<T, m, n> &f,
    Matrix<T, DiffT::SizeAtCompileTime, Matrix<T, m, n>::SizeAtCompileTime>
        &JT) {
...
template <class T, class DiffT, int m, int n>
inline auto fillJacobian(const Matrix<AutoDiff<T, DiffT>, m, n> &x,
                         Matrix<T, m, n> &f,
                         Matrix<T, Matrix<T, m, n>::SizeAtCompileTime,
                                DiffT::SizeAtCompileTime> &J) {
...
template <class T, class DiffT> inline auto conj(const AutoDiff<T, DiffT> &x) {
  return x;
}
template <class T, class DiffT> inline auto real(const AutoDiff<T, DiffT> &x) {
  return x;
}

template <class T, class DiffT> inline auto imag(const AutoDiff<T, DiffT> &) {
  return AutoDiff<T, DiffT>::Zero();
}
template <class T, class DiffT> inline auto abs(const AutoDiff<T, DiffT> &x) {
  T sign = x.s < 0 ? -1 : 1;
  return sign * x;
}
template <class T, class DiffT> inline auto abs2(const AutoDiff<T, DiffT> &x) {
  return x * x;
}

template <class T, class DiffT> inline auto sqrt(const AutoDiff<T, DiffT> &x) {
  using std::sqrt;
  T sqrt_x = sqrt(x.s);
...
template <class T, class DiffT> inline auto sin(const AutoDiff<T, DiffT> &x) {
  using std::cos;
  using std::sin;
  return make_auto_diff(sin(x.s), cos(x.s) * x.ds);
}

template <class T, class DiffT> inline auto cos(const AutoDiff<T, DiffT> &x) {
  using std::cos;
  using std::sin;
  return make_auto_diff(cos(x.s), -sin(x.s) * x.ds);
}

template <class T, class DiffT> inline auto tan(const AutoDiff<T, DiffT> &x) {
  using std::cos;
  using std::tan;
  T sec = (T)1 / cos(x.s);
...
inline auto atan2(const AutoDiff<T, DiffT1> &x, const AutoDiff<T, DiffT2> &y) {
  using std::atan2;
  return make_auto_diff(atan2(x.s, y.s),
                        (y.s * x.ds - x.s * y.ds) / (x.s * x.s + y.s * y.s));
}

template <class T, class DiffT> inline auto exp(const AutoDiff<T, DiffT> &x) {
  using std::exp;
  T exps = exp(x.s);
  return make_auto_diff(exps, exps * x.ds);
}

template <class T, class DiffT> inline auto log(const AutoDiff<T, DiffT> &x) {
  using std::log;
...
template <class T, class DiffT>
struct NumTraits<ZIRAN::AutoDiff<T, DiffT>> : NumTraits<T> {
...
struct significant_decimals_default_impl<ZIRAN::AutoDiff<T, DiffT>, false> {
  static inline int run() {
    ...
    using std::log;
    return cast<T, int>(
        ceil(-log(std::numeric_limits<T>::epsilon()) / log(10.0)));
  }
};
} // namespace internal
} // namespace Eigen
