...
/**
  ...
  TV& computeStep()  // computes the next step based on the previous x passed
  into computeResidual and typedefs TV and T
*/
template <class Objective> class NewtonsMethod {
...
public:
  ...
  NewtonsMethod(Objective &objective, const T tolerance = (T)1e-6,
                const int max_iterations = 5)
      : objective(objective), tolerance(tolerance),
        max_iterations(max_iterations), linear_solve_tolerance_scale((T)1) {}
  ...
  bool solve(Vec &x, const bool verbose = false) {
    ...
    for (int it = 0; it < max_iterations; it++) {
      ...
      ZIRAN_VERB_IF(verbose, "Newton iter ", it,
                    "; (designated norm) residual = ", residual_norm);
      ZIRAN_CONTEXT("Newton step", it);
      ZIRAN_CONTEXT(residual_norm);
      if (residual_norm < tolerance) {
        ZIRAN_VERB_IF(true, "Newton terminates at ", it,
                      "; (designated norm) residual = ", residual_norm);
        return true;
      } else {
        ZIRAN_VERB_IF(true, "Newton residual = ", residual_norm);
      }
      T linear_solve_relative_tolerance =
          std::min((T)0.5, linear_solve_tolerance_scale *
                               std::sqrt(std::max(residual_norm, tolerance)));
      objective.computeStep(step_direction, residual,
