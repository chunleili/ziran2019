...
void readPartio(const std::string &particleFile, Particles<T, dim> &particles) {
  ...
  ZIRAN_ASSERT(data->attributeInfo("position", posAttr) &&
                   posAttr.type == Partio::VECTOR && posAttr.count == 3,
               "could not get position as vector of size 3");
...
void writePositionVectorToPartio(const std::string &particleFile,
                                 const StdVector<Vector<T, dim>> &particles,
                                 const StdVector<int> &inside_out) {
  Partio::ParticlesDataMutable *parts = Partio::create();
  Partio::ParticleAttribute posH;
...
template <class T, int dim>
void writeMatrixVectorToPartio(const std::string &particleFile,
...
//#########################################################################
template <class T, int dim>
void writePartioAttribute(const std::string &particleFile,
                          const Particles<T, dim> &particles,
                          AttributeName<T> scalar_attribute,
...
//
// Dump out a bgeo file that contains a certain attribute from a plasticity
// model. For particles without the attribute, put a background value there. The
// plasticity model needs to provide function fillAttributesToVec3
//#########################################################################
template <class T, class T_PLASTICITY, int dim>
void writePartioAttribute(const std::string &particleFile,
                          const Particles<T, dim> &particles,
                          AttributeName<T_PLASTICITY> plasticity,
                          const T background_value) {
...
template <class T, int dim>
void writePartioWaterDebugReversible(const std::string &particleFile,
                                     const Particles<T, dim> &particles) {
  AttributeName<T> mass = Particles<T, dim>::mass_name();
  AttributeName<Vector<T, dim>> x_name = Particles<T, dim>::X_name();
  AttributeName<Vector<T, dim>> v_name = Particles<T, dim>::V_name();
  ...
  water_grad_porosityH =
      parts->addAttribute("grad_porosity", Partio::VECTOR, 3);
  auto iter = particles.iter(mass, x_name, v_name, water_pressure_name<T>(),
                             water_grad_porosity_name<T, dim>());
  for (; iter; ++iter) {
    int idx = parts->addParticle();
    float *m = parts->dataWrite<float>(mH, idx);
    float *p = parts->dataWrite<float>(posH, idx);
    ...
    float *water_grad_porosity =
        parts->dataWrite<float>(water_grad_porosityH, idx);
