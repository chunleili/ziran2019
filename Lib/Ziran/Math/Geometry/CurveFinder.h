...
class CurveFinder {
  ...
  int addCurve(StdVector<int> &pids, VertexID &vertex, EdgeID starting_edge,
               bool is_cycle);

public:
  CurveFinder(const StdVector<Edge> &segments);

  void constructPaths(StdVector<int> &pids, std::vector<int> &vertex_counts,
                      bool consistent_ordering);

  void constructCycles(StdVector<int> &pids, std::vector<int> &vertex_counts);
