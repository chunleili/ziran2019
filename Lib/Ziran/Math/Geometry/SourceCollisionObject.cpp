...
void SourceCollisionObject<T, dim>::uniformSample(T grid_dx,
  ...
  RandomSampling<T, dim> rs(random_seed, min_corner, max_corner,
                            total_particles_number);
  rs.sample(samples, [&](const TV &x) {
    TV v;
    materialVelocity(x, v);
    return (ls->inside(x) && !ls->inside(x));
  });
  ZIRAN_ASSERT(samples.size() == (size_t)total_particles_number);
}

template <class T, int dim>
void SourceCollisionObject<T, dim>::poissonSample(T grid_dx,
  ...
  PoissonDisk<T, dim> pd(random_seed, 0, min_corner, max_corner, 30,
                         /*periodic=*/true);
  pd.setDistanceByParticlesPerCell(grid_dx, particles_per_cell);
  if (dim == 3)
    pd.sampleFromPeriodicData(samples,
                              [&](const TV &x) { return bb.inside(x); });
  else
...
template <class T, int dim>
T SourceCollisionObject<T, dim>::evalMaxSpeed(const TV &p_min_corner,
...
void SourceCollisionObject<T, dim>::sampleAndPrune(T dt, T grid_dx) {
  // move the particles in the sample modulo bounding box of the Analytic Level
  // Set
  ZIRAN_ASSERT(ls != nullptr);
  TV min_corner, max_corner;
  ls->getBounds(min_corner, max_corner);
  ...
  for (auto &s : samples) {
    ...
    // condition B: check if after the particles being advected (modulo the
    // bounding box) that the particle is outside of the sample
    TV v;
    materialVelocity(s, v);
    s += dt * v;
  ...
  std::cout << "inside.size() inside sampleAndPrune = " << inside.size()
            << std::endl;
}
