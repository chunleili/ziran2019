...
class SimplexElements<T, _manifold_dim, _dim,
                      std::enable_if_t<_manifold_dim <= _dim>>
    : public ElementManager<T, _dim> {
public:
  using Scalar = T;
  ...
  template <class DataType> using PerQuadrature = DataType;
  ...
  StdVector<int> partition_offsets; // with size n+1, where n is # partitions.
                                    // This is a state and will be read/write.

  // Helpers for parallelization:

  // for splatting to pads
  using Base::pads; // StdVector<TVStack> pads. they are just element partition
                    // scratch pads of particles for parallization. no write.
  using Base::pads_dx; // StdVector<TVStack> pads. they are just element
                       // partition scratch pads of particles for parallization.
                       // no write.
  using Base::particle_l2g; // StdVector<StdVector<int>> particle_l2g; particle
                            // index local to global. no write.

  StdVector<IV> local_indices; // mapping from element id to pad wise local
                               // particle indices. no write.
  ...
  }; // this is a map from me to my neighbors who have the same colored
     // particles with me. for gathering. no write.
  StdVector<NeighborHelper>
      neighbors_of_my_color; // This is a state and will be read/write.
  ...
  inline static AttributeName<TM> F_name() { return AttributeName<TM>("F"); }
  ...
  inline static AttributeName<TM2> Q_name() { return AttributeName<TM2>("Q"); }

  //  template <class DataType>
  //  inline static AttributeName<DataType> attributeName(const std::string&
  //  name)
  //  {
  ...
  void
  registerParticleReorderingCallback(Particles<T, dim> &particles) override;

  bool renumberParticles(const int particle_count, StdVector<int> &old_to_new,
                         StdVector<int> &new_to_old) override;

  Range addUndeformedMesh(const SimplexMesh<manifold_dim> &mesh,
  ...
  void updateF(const XArray &X,
               const AttributeName<TM2>
                   &F_name) // when called with dF name, writes to dF
  {
  ...
  template <class XArray,
            TICK_REQUIRES(
                dim !=
                manifold_dim)> // XArray is StdVector<Vector<T, dim>> or TVStack
  void
  updateF(
      const XArray &X,
      const AttributeName<TM> &F_name) // when called with dF name, writes to dF
  {
  ...
  TM2 dS(const IV &these_indices,
         const Matrix<T, dim, Eigen::Dynamic> &X) const;

  auto getVertices(const IV &these_indicies, const StdVector<Vector<T, dim>> &X)
  ...
  void partitionElements(const int n_partitions,
