...
SimplexElements<T, _manifold_dim, _dim,
                std::enable_if_t<_manifold_dim <= _dim>>::SimplexElements()
    : Base(indices_name(), Dm_inv_name(), F_name(), element_measure_name()),
      indices(get(indices_name())), Dm_inv(get(Dm_inv_name())),
      F(get(F_name())), element_measure(get(element_measure_name())) {
  if (dim != manifold_dim)
    add(Q_name());
}
...
template <class T, int _manifold_dim, int _dim>
void SimplexElements<T, _manifold_dim, _dim,
                     std::enable_if_t<_manifold_dim <= _dim>>::
    registerParticleReorderingCallback(Particles<T, dim> &particles) {
...
template <class T, int _manifold_dim, int _dim>
bool SimplexElements<T, _manifold_dim, _dim,
                     std::enable_if_t<_manifold_dim <= _dim>>::
    renumberParticles(const int particle_count, StdVector<int> &old_to_new,
...
template <class T, int _manifold_dim, int _dim>
Range SimplexElements<T, _manifold_dim, _dim,
                      std::enable_if_t<_manifold_dim <= _dim>>::
    addUndeformedMesh(const SimplexMesh<manifold_dim> &mesh,
                      const StdVector<Vector<T, dim>> &X0,
  ...
  // the unit simplex has measure 1 / d_factorial and the element measure is the
  // product of the determinant of the mapping from the unit simplex to the
  // current simplex and the volumn of the unit simplex
  auto ap =
      appender(indices_name(), Dm_inv_name(), element_measure_name(), F_name());
  ...
  for (size_t e = 0; e < mesh.numberElements(); e++) {
    ...
    T element_measure =
        std::abs(Dm.template triangularView<Eigen::Upper>().determinant() *
                 unit_simplex_measure);

    // Dm_inverse is manifold_dim x manifold_dim
    TM Dm_inverse =
        Dm.template triangularView<Eigen::Upper>().solve(TM::Identity());

    // For full dimensional mesh, Dm_inverse stores the real Dm_inverse
    if (dim == manifold_dim)
      Dm_inverse *=
          Q.template topLeftCorner<manifold_dim, manifold_dim>().transpose();

    // For co-dimensional mesh, Dm_inverse stores (R(Ds))^{-1} (e.g. isotropic
    // cloth in 3d) The rotational part is discarded.
...
T SimplexElements<T, manifold_dim, dim, std::enable_if_t<manifold_dim <= dim>>::
    totalMeasure(Range element_range) const {
  T result = 0;
...
template <class T, int _manifold_dim, int _dim>
typename SimplexElements<T, _manifold_dim, _dim>::ShapeGradientType
computeShapeGradients() {
  typename SimplexElements<T, _manifold_dim, _dim>::ShapeGradientType
      grad_N_hat;
  grad_N_hat << Vector<T, _manifold_dim>::Constant(-1),
      Matrix<T, _manifold_dim, _manifold_dim>::Identity();
  return grad_N_hat;
}

template <class T, int _manifold_dim, int _dim>
const typename SimplexElements<
    T, _manifold_dim, _dim,
    std::enable_if_t<_manifold_dim <= _dim>>::ShapeGradientType
    SimplexElements<T, _manifold_dim, _dim,
                    std::enable_if_t<_manifold_dim <= _dim>>::grad_N_hat =
        computeShapeGradients<T, _manifold_dim, _dim>();

template <class T, int _manifold_dim, int _dim>
typename SimplexElements<T, _manifold_dim, _dim,
                         std::enable_if_t<_manifold_dim <= _dim>>::TM2
SimplexElements<T, _manifold_dim, _dim,
                std::enable_if_t<_manifold_dim <= _dim>>::
    dS(const IV &these_indices, const StdVector<Vector<T, dim>> &X) const {
  TM2 result;
  for (int i = 1; i < manifold_dim + 1; i++)
...
template <class T, int _manifold_dim, int _dim>
auto SimplexElements<T, _manifold_dim, _dim,
                     std::enable_if_t<_manifold_dim <= _dim>>::
    getVertices(const IV &these_indicies, const StdVector<Vector<T, dim>> &X)
        -> Matrix<T, dim, manifold_dim + 1> const {
...
SimplexElements<T, _manifold_dim, _dim,
                std::enable_if_t<_manifold_dim <= _dim>>::
    dS(const IV &these_indices, const Matrix<T, dim, Eigen::Dynamic> &X) const {
  TM2 result;
  for (int i = 1; i < manifold_dim + 1; i++)
...
template <class T, int _manifold_dim, int _dim>
bool SimplexElements<
    T, _manifold_dim, _dim,
    std::enable_if_t<_manifold_dim <= _dim>>::needRepartitioning() {
...
void SimplexElements<T, _manifold_dim, _dim,
                     std::enable_if_t<_manifold_dim <= _dim>>::
    gatherFromPadsToTVStack(const T scale, TVStack &force) {
...
template <class T, int _manifold_dim, int _dim>
void SimplexElements<
    T, _manifold_dim, _dim,
    std::enable_if_t<_manifold_dim <= _dim>>::resizePadsAndBuildHelpers() {
  ZIRAN_QUIET_TIMER();
  ...
  for (size_t i = 0; i < partition_offsets.size() - 1;
       ++i) // loop over all partitions
  {
    particle_l2g[i].clear();
    int start_element = partition_offsets[i];   // [
    int end_element = partition_offsets[i + 1]; // )
    HashTable<int, int> particles; // key is global particle ids in this
                                   // parititon, value is local index
    int count = 0;
...
void SimplexElements<
    T, _manifold_dim, _dim,
    std::enable_if_t<_manifold_dim <= _dim>>::buildNeighbors() {
  ZIRAN_QUIET_TIMER();
  ...
  for (size_t i = 0; i < particle_l2g.size(); ++i) // loop over all partitions
  {
    ...
    for (size_t local_pid = 0; local_pid < my_l2g.size(); local_pid++) {
      ...
      auto spot = std::find(neighbor.neighbor_ids.begin(),
                            neighbor.neighbor_ids.end(), my_color);
...
void SimplexElements<T, _manifold_dim, _dim,
                     std::enable_if_t<_manifold_dim <= _dim>>::
    partitionAndReindexElements(const int n_partitions) {
  if (count == 0)
    return;
  ZIRAN_QUIET_TIMER();
...
void SimplexElements<T, _manifold_dim, _dim,
                     std::enable_if_t<_manifold_dim <= _dim>>::
    partitionElements(int n_partitions, StdVector<StdVector<int>> &buckets) {
  ...
  METIS_PartMeshDual(&ne, &nn, eptr.data(), eind, NULL, NULL, &ncommon, &nparts,
                     NULL, NULL, &objval, epart.data(), npart.data());
  ...
  for (size_t i = 0; i < buckets.size(); i++)
    ZIRAN_ASSERT(buckets[i].size() != 0,
                 "partition failed. some partition is empty.");
...
void SimplexElements<T, _manifold_dim, _dim,
                     std::enable_if_t<_manifold_dim <= _dim>>::
    reindexElements(const StdVector<StdVector<int>> &buckets) {
  ZIRAN_QUIET_TIMER();
...
void SimplexElements<
    T, _manifold_dim, _dim,
    std::enable_if_t<_manifold_dim <= _dim>>::readData(std::istream &in) {
  Base::readData(in);
