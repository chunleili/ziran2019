...
/**
   This is a ZIRAN rotation class that wraps Eigen's Rotation2D (in 2D) and
   Quaternion (in 3D).
 */
template <class T, int dim> class Rotation {
public:
  ...
  // The resulting rotation rotates a to b. a or b don't need to be normalized
  // and can have different length (not zero through).
  TICK_MEMBER_REQUIRES(dim == 3)
  ...
  void setIdentityRotation() { rotation = TM::Identity(); }
};

template <class T, int dim, class enable = void> class AngularVelocity;
...
public:
  ...
  void operator=(const AngularVelocity &another) {
    set(another.angular_velocity);
  }
  void operator+=(const AngularVelocity &another) {
    angular_velocity += another.angular_velocity;
  }
  void operator-=(const AngularVelocity &another) {
    angular_velocity -= another.angular_velocity;
  }
  ...
  AngularVelocity operator+(const AngularVelocity &another) const {
    return AngularVelocity(angular_velocity + another.angular_velocity);
  }
  AngularVelocity operator*(T alpha) const {
    return AngularVelocity(angular_velocity * alpha);
  }
  ...
  template <class Type> AngularVelocity<Type, dim> cast() {
...
public:
  ...
  Vector<T, dim> cross(const Vector<T, dim> &x) const {
    return angular_velocity.cross(x);
  }

  T norm() const { return angular_velocity.norm(); }
