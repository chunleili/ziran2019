...
public:
  ...
  */
  SourceCollisionObject(
      ...
      std::unique_ptr<AnalyticLevelSet<T, dim>> &&ls_in, const TV &const_vel)
      : AnalyticCollisionObject<T, dim>(updateState, std::move(ls_in),
                                        Base::COLLISION_OBJECT_TYPE::STICKY) {
    setStatic();
   ...
   */
  SourceCollisionObject(std::unique_ptr<AnalyticLevelSet<T, dim>> &&ls_in,
                        const TV &const_vel)
      : AnalyticCollisionObject<T, dim>(std::move(ls_in),
                                        Base::COLLISION_OBJECT_TYPE::STICKY) {
    setStatic();
    materialVelocity = [const_vel](const TV &x, TV &v) { v = const_vel; };
    const_material_velocity = const_vel;
  ...
  */
  SourceCollisionObject(
      std::function<void(T, AnalyticCollisionObject<T, dim> &)> updateState,
      AnalyticLevelSet<T, dim> &ls_in, const TV &const_vel)
      : AnalyticCollisionObject<T, dim>(updateState, ls_in,
                                        Base::COLLISION_OBJECT_TYPE::STICKY) {
    setStatic();
    materialVelocity = [const_vel](const TV &x, TV &v) { v = const_vel; };
    const_material_velocity = const_vel;
  ...
  SourceCollisionObject(AnalyticLevelSet<T, dim> &ls_in, const TV &const_vel)
      : AnalyticCollisionObject<T, dim>(ls_in,
                                        Base::COLLISION_OBJECT_TYPE::STICKY) {
    setStatic();
    materialVelocity = [const_vel](const TV &x, TV &v) { v = const_vel; };
    const_material_velocity = const_vel;
  ...
  void setMaterialVelocity(
      std::function<void(const TV &, TV &)> new_material_velocity) {
    materialVelocity = new_material_velocity;
  }

  void setMaterialVelocity(const TV &new_material_velocity) {
    materialVelocity = [new_material_velocity](const TV &x, TV &v) {
      v = new_material_velocity;
    };
  ...
  virtual T evalMaxSpeed(const TV &p_min_corner,
                         const TV &p_max_corner) const override;
