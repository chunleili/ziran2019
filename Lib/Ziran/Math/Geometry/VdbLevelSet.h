...
/**
   This is a level set wrapper class that internally uses an openvdb level set
   to represent a 3D sdf. phi and normals are only correct near narrow band.
...
public:
  ...
  typedef typename openvdb::Grid<
      typename openvdb::tree::Tree4<float, 5, 4, 3>::Type>
      GridT;
  typedef typename GridT::TreeType TreeT;
  typedef typename openvdb::tools::ScalarToVectorConverter<GridT>::Type
      GradientGridT;
  ...
  // A safe way to achieve this is to fill the interor when creating the
  // levelset.
  TV normal(const TV &X_input) const override;

  bool query(const TV &X, T &signed_distance, TV &n,
