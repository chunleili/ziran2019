...
// private
int CurveFinder::addCurve(StdVector<int> &pids, VertexID &vertex,
                          EdgeID starting_edge, bool is_cycle) {
  ...
  if (is_cycle) {
    ...
    assert(segments[previous_edge_id][0] == vertex ||
           segments[previous_edge_id][1] == vertex);
  }
  ...
  do {
    ...
    assert(is_cycle || (p.first == previous_edge_id) ||
           (p.second == previous_edge_id));
...
void CurveFinder::constructPaths(StdVector<int> &pids,
...
void CurveFinder::getCurveEndpoints(
    HashTable<VertexID, EdgeID> &curve_endpoints_in) const {
  for (auto iter = curve_endpoints.begin(); iter != curve_endpoints.end();
       ++iter)
