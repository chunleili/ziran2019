...
template <class T, int dim> VdbLevelSet<T, dim>::VdbLevelSet() {}
...
void VdbLevelSet<T, dim>::constructFromVdbFile(const std::string &file_in) {
  ...
  ZIRAN_ASSERT(count == 1,
               "Vdb file to load should only contain one levelset.");

  recomputeGradient();
}

template <class T, int dim> void VdbLevelSet<T, dim>::recomputeGradient() {
...
auto VdbLevelSet<T, dim>::normal(const TV &X_input) const -> TV {
  ...
  for (int d = 0; d < dim; d++)
    X(d) = X_input(d);
  openvdb::tools::GridSampler<GradientTreeT, openvdb::tools::BoxSampler>
      interpolator(grad_phi->constTree(), grad_phi->transform());
  openvdb::math::Vec3<T> P(X(0), X(1), X(2));
...
template <class T, int dim>
bool VdbLevelSet<T, dim>::query(const TV &X, T &signed_distance, TV &n,
                                TM *hessian) const {
...
VdbLevelSet<T, dim>::createPtr() const {
