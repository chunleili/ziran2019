...
template <typename T, int dim> class VertexMesh {
public:
  ...
  VertexMesh(const StdVector<TV> &vertices,
             StdVector<typename Mesh::IV> indices)
      : vertices(vertices), mesh(std::move(indices)) {}

  VertexMesh(StdVector<TV> &&vertices, StdVector<typename Mesh::IV> &&indices)
      : vertices(std::move(vertices)), mesh(std::move(indices)) {}
};

template <class T, int dim> struct VertexMeshTag {};

template <class T, int dim> struct RW<VertexMesh<T, dim>> {
  using Tag = VertexMeshTag<T, dim>;
};

template <class T, int dim>
void writeHelper(std::ostream &out, const VertexMesh<T, dim> &mesh,
                 VertexMeshTag<T, dim>) {
