...
template <int dim> void SimplexMesh<dim>::initializeBoundaryElements() {
...
template <>
void SimplexMesh<1>::getFaces(const size_t element,
...
template <>
void SimplexMesh<2>::getFaces(const size_t element,
...
template <>
void SimplexMesh<3>::getFaces(const size_t element,
...
template <>
void SimplexMesh<1>::constructListOfEdges(StdVector<EIV> &edge_list,
                                          HashTable<EIV, int> *edge_hash_ptr) {
...
template <>
void SimplexMesh<2>::constructListOfEdges(StdVector<EIV> &edge_list,
                                          HashTable<EIV, int> *edge_hash_ptr) {
  SimplexMesh<2>::constructListOfFaces(edge_list, edge_hash_ptr);
}

template <>
void SimplexMesh<3>::constructListOfEdges(StdVector<EIV> &edge_list,
                                          HashTable<EIV, int> *edge_hash_ptr) {
  HashTable<EIV, int> inner_edge_hash;
  if (!edge_hash_ptr)
    edge_hash_ptr = &inner_edge_hash;
...
// template <>
// void SimplexMesh<1>::constructListOfEdges(StdVector<EIV>& edge_list, EEIV*
// associated_edge_list_begin, EEIV* associated_edge_list_end)
// {
...
// template <>
// void SimplexMesh<2>::constructListOfEdges(StdVector<EIV>& edge_list, EEIV*
// associated_edge_list_begin, EEIV* associated_edge_list_end)
// {
...
//             if (result.inserted)
//                 edge_list.emplace_back(sorted);
//             assert(associated_edge_list_begin + i <
//             associated_edge_list_end); associated_edge_list_begin[i](j) =
//             result.value;
//         }
//     }
// }
...
// template <>
// void SimplexMesh<3>::constructListOfEdges(StdVector<EIV>& edge_list, EEIV*
// associated_edge_list_begin, EEIV* associated_edge_list_end)
// {
//     //TODO
//     constructListOfEdges(edge_list);
...
template <int dim>
void SimplexMesh<dim>::constructListOfFaces(
    StdVector<BIV> &face_list, HashTable<BIV, int> *face_hash_ptr) {
...
template <int dim>
void SimplexMesh<dim>::findSharedFace(const IV &element1, const IV &element2,
                                      BIV &face) {
  int e = 0;
  bool job_done = false;
...
template <int dim>
void SimplexMesh<dim>::findSharedFaceAndIndex(const IV &element1,
                                              const IV &element2, BIV &sorted,
                                              BIV *local_index1_ptr,
                                              BIV *local_index2_ptr) {
  ...
  std::sort(local_index1_ptr->data(),
            local_index1_ptr->data() + local_index1_ptr->size());
  std::sort(local_index2_ptr->data(),
            local_index2_ptr->data() + local_index2_ptr->size());
  std::sort(sorted.data(), sorted.data() + sorted.size());
}

template <int dim>
void SimplexMesh<dim>::findUnsortedFace(const IV &element, const BIV &sorted,
                                        BIV &face) {
  int find = 0;
  for (int k = 0; k < num_vertices && find < boundary_vertices; k++) {
...
template <int dim>
void SimplexMesh<dim>::constructNeighborVertexList(
    const StdVector<NIV> &neighbor_list,
    StdVector<BIV> &neighbor_vertex_list) const {
...
template <int dim> const char *SimplexMesh<dim>::name() {
  switch (dim) {
  case 1:
    return "Seg";
    break;
...
/**
    ...
    */
template <int dim>
void SimplexMesh<dim>::constructListOfFaceNeighboringElements(
    StdVector<Eigen::Vector2i> &result, const bool sort,
    HashTable<BIV, Eigen::Vector2i> *face_hash_ptr) {
...
template <int dim>
void SimplexMesh<dim>::constructListOfFaceNeighboringElementsInternal(
    const bool sort, HashTable<BIV, Eigen::Vector2i> *face_hash_ptr) {
  ...
  if (sort) {
    for (size_t i = 0; i < neighbor_list.size(); i++) {
      std::sort(neighbor_list[i].data(),
                neighbor_list[i].data() + neighbor_list[i].size());
    }
  }
...
template <int dim> void SimplexMesh<dim>::initializeNeighboringElements() {
      ...
      for (int p = 0; p < dim + 1; p++) {
        ...
        for (int q = 0; q < dim + 1; q++) {
          ...
          if (p != q) {
            ...
            auto found =
                std::find(std::begin(list), std::end(list), your_index);
            if (found == list.end())
...
/**
    ...
    */
template <int dim>
void SimplexMesh<dim>::closestElement(const int index, int &result,
                                      const HashTable<int, bool> &target) {
