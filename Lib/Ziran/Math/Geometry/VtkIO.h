...
template <class T>
void readTetmeshVtk(std::istream &in, StdVector<Vector<T, 3>> &X,
  ...
  ZIRAN_ASSERT(n_points == X.size() - initial_X_size,
               "vtk read X count doesn't match.");
  ZIRAN_ASSERT((size_t)n_tets == indices.size() - initial_indices_size,
               "vtk read element count doesn't match.");
}

template <class T>
void readTetmeshVtk(const std::string vtk_file, StdVector<Vector<T, 3>> &X,
                    StdVector<Vector<int, 4>> &indices) {
  std::ifstream fs;
  fs.open(vtk_file);
...
template <class T>
void writeTetmeshVtk(const std::string &filename,
                     const StdVector<Vector<T, 3>> &X,
                     StdVector<Vector<int, 4>> &indices) {
  std::ofstream fs;
  fs.open(filename);
