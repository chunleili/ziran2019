...
void readBinaryCurveCollectionFile(const std::string &filename,
  ...
  ZIRAN_ASSERT(header.sign[0] == 'B' && header.sign[1] == 'C' &&
               header.sign[2] == 'C' && header.byteCount == 0x44);
  ...
  for (uint64_t i = 0; i < header.curveCount; i++) {
    ...
    for (int k = 0; k < curve_point_count; ++k) {
      ...
      this_point << controlPoints[firstControlPoint[i] + k].data[0],
          controlPoints[firstControlPoint[i] + k].data[1],
          controlPoints[firstControlPoint[i] + k].data[2];
      Vector<T, dim> P;
    ...
    if (loop) {
      Vector<int, 2> seg;
      seg << (int)(firstControlPoint[i] + curve_point_count - 1),
          firstControlPoint[i];
      segments.emplace_back(seg);
    }
  }
