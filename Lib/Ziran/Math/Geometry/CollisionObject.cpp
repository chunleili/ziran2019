...
AnalyticCollisionObject<T, dim>::AnalyticCollisionObject(
    std::function<void(T, AnalyticCollisionObject<T, dim> &)> updateState,
    std::unique_ptr<AnalyticLevelSet<T, dim>> &&ls_in,
    COLLISION_OBJECT_TYPE type_in)
    : ls(std::move(ls_in)), type(type_in), friction(0),
...
AnalyticCollisionObject<T, dim>::AnalyticCollisionObject(
    std::unique_ptr<AnalyticLevelSet<T, dim>> &&ls_in,
    COLLISION_OBJECT_TYPE type_in)
    : ls(std::move(ls_in)), type(type_in), friction(0), updateState(nullptr) {
  setStatic();
}
...
AnalyticCollisionObject<T, dim>::AnalyticCollisionObject(
    std::function<void(T, AnalyticCollisionObject<T, dim> &)> updateState,
    AnalyticLevelSet<T, dim> &ls_in, COLLISION_OBJECT_TYPE type_in)
    : ls(ls_in.createPtr()), type(type_in), friction(0),
      updateState(updateState) {
  setStatic();
}
...
AnalyticCollisionObject<T, dim>::AnalyticCollisionObject(
    AnalyticLevelSet<T, dim> &ls_in, COLLISION_OBJECT_TYPE type_in)
    : ls(ls_in.createPtr()), type(type_in), friction(0), updateState(nullptr) {
  setStatic();
}

template <class T, int dim>
void AnalyticCollisionObject<T, dim>::trialCollision(
    const StdVector<std::unique_ptr<AnalyticCollisionObject<T, dim>>> &objects,
...
template <class T, int dim>
bool AnalyticCollisionObject<T, dim>::multiObjectCollision(
    const StdVector<std::unique_ptr<AnalyticCollisionObject<T, dim>>> &objects,
...
template <class T, int dim>
bool AnalyticCollisionObject<T, dim>::multiObjectCollisionWithFakeSeparate(
    const StdVector<std::unique_ptr<AnalyticCollisionObject<T, dim>>> &objects,
    const TV &xi, TV &vi, TM &normal_basis) {
  bool any_collision = false;
  int slip_collision_count = 0;
  ...
  for (size_t k = 0; k < objects.size(); ++k) {
    ...
    if (collide && objects[k]->type == SEPARATE) {
      ...
      TV boundary_normal;
      objects[k]->getBoundaryVelocityAndNormal(xi, boundary_velocity,
                                               boundary_normal);
...
template <class T, int dim>
T AnalyticCollisionObject<T, dim>::evalMaxSpeed(const TV &p_min_corner,
                                                const TV &p_max_corner) const {
  if (dsdt != 0 || omega.norm() != 0) {
    ...
    for (int i = 0; i < 1 << dim; i++) {
      ...
      if ((min_corner.array() < X.array()).any() &&
          (X.array() < max_corner.array()).any())
    ...
    for (int i = 0; i < 1 << dim; i++) {
      ...
      if ((p_min_corner.array() < x.array()).any() &&
          (x.array() < p_max_corner.array()).any())
        intersection_corners.emplace_back(x);
    }
    T max_speed = 0;
...
template <class T, int dim>
void AnalyticCollisionObject<T, dim>::transformedPoint(const TV &X, TV &x,
                                                       TV &v) const {
  TV x_minus_b = R.rotation * s * X;
  x = x_minus_b + b;
  v = omega.cross(x_minus_b) + (dsdt / s) * x_minus_b +
      R.rotation * s * getMaterialVelocity(X) + dbdt;
}

template <class T, int dim>
void AnalyticCollisionObject<T, dim>::transformedVector(const TV &N,
                                                        TV &n) const {
  n = R.rotation * s * N;
}

template <class T, int dim>
void AnalyticCollisionObject<T, dim>::untransformedPoint(const TV &x,
                                                         TV &X) const {
  TV x_minus_b = x - b;
  assert(s);
  T one_over_s = 1 / s;
...
template <class T, int dim>
bool AnalyticCollisionObject<T, dim>::queryInside(const TV &x, T &phi, TV &n,
...
template <class T, int dim>
void AnalyticCollisionObject<T, dim>::particleCollision(const T phi_critical,
                                                        const TV &x,
                                                        TV &v) const {
  if (type == GHOST)
    return;
  ...
  if (colliding) {
    ...
    if (type == STICKY) {
      ...
      n = R.rotation *
          N; // we care about world space normal, otherwise it is nan
      T dot = v.dot(n);
      TV v_normal = n * dot;
      v -= v_normal;
...
template <class T, int dim>
bool AnalyticCollisionObject<T, dim>::getBoundaryVelocityAndNormal(
    const TV &x, TV &v, TV &n) const {
  if (type == GHOST)
    return false;
  ...
  if (colliding) {
    v = omega.cross(x_minus_b) + (dsdt * one_over_s) * x_minus_b +
        R.rotation * s * getMaterialVelocity(X) + dbdt;
    n = R.rotation * N;
    return true;
  }
...
/**
   ...
   project the velocity, and returns a normal if the collision happened as a
   SLIP collsion.
*/
template <class T, int dim>
bool AnalyticCollisionObject<T, dim>::detectAndResolveCollision(const TV &x,
                                                                TV &v,
                                                                TV &n) const {
  if (type == GHOST)
    return false;
