...
public:
  ...
  virtual bool query(const TV &X, T &signed_distance, TV &normal,
                     TM *hessian = 0) const;

  virtual bool queryInside(const TV &X, T &signed_distance, TV &normal,
                           const T phi_critical = 0) const;
...
public:
  ...
  DifferenceLevelSet(const StdVector<std::unique_ptr<AnalyticLevelSet<T, dim>>>
                         &level_sets_input);

  virtual ~DifferenceLevelSet() {}

  void add(const AnalyticLevelSet<T, dim> &ls1,
...
template <class T, int dim> class HalfSpace : public AnalyticLevelSet<T, dim> {
public:
  using TV = Vector<T, dim>;
  using TM = Matrix<T, dim, dim>;
...
//
template <class T, int dim> class Sphere : public AnalyticLevelSet<T, dim> {
public:
  using TV = Vector<T, dim>;
  using TM = Matrix<T, dim, dim>;
  ...
  bool queryInside(const TV &X, T &signed_distance, TV &norm,
                   const T phi_critical = 0) const override;

  bool query(const TV &X, T &signed_distance, TV &norm,
             TM *hess = 0) const override;
...
public:
   ...
   */
  CappedCylinder(const T radius_in, const T height_in, const Vector<T, 4> &q,
                 const TV &b_in)
      : radius(radius_in), height(height_in) {
    ZIRAN_ASSERT(dim == 3);
    // the following is basically:
    // R = Eigen::Quaternion<T>(q(0), q(1), q(2),
    // q(3)).normalized().toRotationMatrix().template topLeftCorner<dim, dim>();
    Vector<T, 4> vv = q;
    vv.normalize();
  ...
  template <class S> S signedDistancePrimitive(const Vector<S, dim> &X) const {
    using std::max;
    using std::min;
  ...
  T volume() const override { return M_PI * MATH_TOOLS::sqr(radius) * height; }
...
template <class T, int dim> class Torus : public AnalyticLevelSet<T, dim> {
public:
  using TV = Vector<T, dim>;
  using TV2 = Vector<T, 2>;
  using TV3 = Vector<T, 3>;
