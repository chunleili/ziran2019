...
template <class T, int dim> class AnalyticLevelSet;

template <class T, int dim> struct CollisionNode {
  int node_id;
  Matrix<T, dim, dim> P;
  ...
  /**
     ...
     v = P*v, where P = I - KK'
   */
  template <class TV> inline void project(const TV &cv) {
    TV &v = const_cast<TV &>(cv);
    v = P * v;
  }
};

template <class T, int dim> class AnalyticCollisionObject {
public:
  using TV = Vector<T, dim>;
  using TM = Matrix<T, dim, dim>;
  enum COLLISION_OBJECT_TYPE { STICKY = 1, SLIP = 2, SEPARATE = 3, GHOST = 4 };
  ...
  /**
   function for updating transformations and their derivatives (veloties) wrt
   time
  */
  std::function<void(T, AnalyticCollisionObject<T, dim> &)> updateState;

  static void trialCollision(
      const StdVector<std::unique_ptr<AnalyticCollisionObject<T, dim>>>
          &objects,
      const TV &xi, const T dt, TV &vi);

  static bool multiObjectCollision(
      const StdVector<std::unique_ptr<AnalyticCollisionObject<T, dim>>>
          &objects,
      const TV &xi, TV &vi, TM &normal_basis);

  static bool multiObjectCollisionWithFakeSeparate(
      const StdVector<std::unique_ptr<AnalyticCollisionObject<T, dim>>>
          &objects,
      const TV &xi, TV &vi, TM &normal_basis);

  /**
     A moving collision object supporting x = R S x0 + b, where R is rotation, S
     is uniform scaling, b is translation
  */
  AnalyticCollisionObject(
      std::function<void(T, AnalyticCollisionObject<T, dim> &)> updateState,
   ...
   */
  AnalyticCollisionObject(std::unique_ptr<AnalyticLevelSet<T, dim>> &&ls_in,
                          COLLISION_OBJECT_TYPE type_in);
  ...
  */
  ...
  /**
     if no updateState is passed in, the collision object is static
   */
  AnalyticCollisionObject(AnalyticLevelSet<T, dim> &ls_in,
                          COLLISION_OBJECT_TYPE type_in);
  ...
  virtual TV getMaterialVelocity(const TV &X) const { return TV::Zero(); }
  ...
  */
  virtual bool rigid() const { return true; }
  ...
  void setFriction(const T &friction_in) { friction = friction_in; }
  ...
  void setRotation(const Vector<T, 4> &q) { R = Rotation<T, dim>(q); }

  void setAngularVelocity(const Vector<T, 3> &q) { omega.set(q); }
  ...
  // find the world space position and velocity for any material space point
