...
public:
  ...
  PoissonDisk(const RandomNumber<T> &rnd, T min_distance,
              const EigenTV &min_corner, const EigenTV &max_corner,
              int max_attempts = 30, bool periodic = false)
      : rnd(rnd), min_distance(min_distance), max_attempts(max_attempts),
        h(min_distance / std::sqrt((T)dim)), min_corner(min_corner),
        max_corner(max_corner), periodic(periodic) {
    ZIRAN_ASSERT(min_corner != max_corner,
                 "min_corner == max_corner in PoissonDisk");
  }

  PoissonDisk(const int seed, T min_distance, const EigenTV &min_corner,
              const EigenTV &max_corner, int max_attempts = 30,
              bool periodic = false)
      : min_distance(min_distance), max_attempts(max_attempts),
        h(min_distance / std::sqrt((T)dim)), min_corner(min_corner),
        max_corner(max_corner), periodic(periodic) {
    rnd.resetSeed(seed);
    ZIRAN_ASSERT(min_corner != max_corner,
...
private:
  ...
  /**
  Generate one random point around center with a distance between .5 and 1 from
  center.
   */
  ...
  in samples are sufficiently far away (at least min_distance away), and false
  otherwise.
   */
  bool checkDistance(const EigenTV &point,
                     const MultiArray<int, dim> &background_grid,
                     const StdVector<EigenTV> &samples) const {
    ...
    // If not periodic, clamp local_min_index and local_max_index to the size of
    // the background grid
    if (!periodic) {
    ...
    if (!periodic)
      ...
      for (MaxExclusiveBoxIterator<dim> it(local_box); it.valid(); ++it) {
        ...
        // Need to shift point in MultiArray if one of the indices is negative
        // or greater than background_grid.size
        ...
        for (int d = 0; d < dim; ++d) {
          ...
          if (it.index(d) < 0) {
            local_index(d) = local_index(d) % background_grid.size(d) +
                             background_grid.size(d);
            shift(d) = min_corner(d) - max_corner(d);
          }
          // If it.index(d) >= background_grid(d) update local index to all the
          // way down to the left/bottom/front If there is a point in that
          // MultiArray index, we need to shift that point to the left
          // right/top/back
...
public:
  /**
  Set min_distance between particles based on the number of particles per cell
  and grid dx. ppc = particles per cell.
   */
  ...
  template <class Func>
  void sampleFromPeriodicData(StdVector<Vector<T, 2>> &samples,
                              Func &&feasible) {
  ...
  this file is saved in Data/MpmParticles/particles-1000k.dat. min_distance =
   ...
   */
  template <class Func>
  void sampleFromPeriodicData(StdVector<Vector<T, 3>> &samples,
                              Func &&feasible) {
    ...
    for (int d = 0; d < dim; ++d)
      offset_number(d) =
          std::ceil(side_length(d) / scaled_ref_box_length(d)) + 1;
    ...
    for (size_t i = 0; i < size; i++) {
      ...
      for (MaxExclusiveBoxIterator<dim> it(index_box); it.valid(); ++it) {
        ...
        bool inside = true;
        for (int d = 0; d < dim; ++d)
          if (offset_new_point(d) < min_corner(d) ||
              offset_new_point(d) > max_corner(d))
            inside = false;
  ...
  background_grid is a MultiArray which keeps tracks the indices of points in
  samples. the value of background_grid is initialized to be -1, meaning that
  there is no particle in that cell
  */
  ...
  */
    while (active_list.size()) {
      ...
      // Generate up to max_attempts points in the annulus of radius h and 2h
      // around current_point
      bool found_at_least_one = false;
      for (int i = 0; i < max_attempts; ++i) {
        EigenTV new_point = generateRandomPointAroundAnnulus(current_point);

        // If periodic and new_point is outside of the min_corner, max_corner
