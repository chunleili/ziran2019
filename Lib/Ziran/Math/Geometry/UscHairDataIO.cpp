  ...
  for (int i = 0; i < num_strands; i++) {
    ...
    if (live) {
      ...
      for (int j = 0; j < num_vertices; j++) {
        X.emplace_back(readEntry<Vector<float, 3>>(in)
                           .template cast<T>()
                           .template head<dim>());
        if (j > 0)
...
template void readUscHairData<double, 2>(
    std::string const &,
    std::vector<Eigen::Matrix<double, 2, 1, 0, 2, 1>,
                Eigen::aligned_allocator<Eigen::Matrix<double, 2, 1, 0, 2, 1>>>
        &,
    std::vector<Eigen::Matrix<int, 2, 1, 0, 2, 1>,
                Eigen::aligned_allocator<Eigen::Matrix<int, 2, 1, 0, 2, 1>>> &,
    double);
template void readUscHairData<double, 3>(
    std::string const &,
    ...
    std::vector<Eigen::Matrix<int, 2, 1, 0, 2, 1>,
                Eigen::aligned_allocator<Eigen::Matrix<int, 2, 1, 0, 2, 1>>> &,
    double);
template void readUscHairData<float, 2>(
    std::string const &,
    ...
    std::vector<Eigen::Matrix<int, 2, 1, 0, 2, 1>,
                Eigen::aligned_allocator<Eigen::Matrix<int, 2, 1, 0, 2, 1>>> &,
    float);
template void readUscHairData<float, 3>(
    std::string const &,
