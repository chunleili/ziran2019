...
template <class T>
void barycentricWeights(const Vector<T, 3> &p, const Vector<T, 3> &a,
                        const Vector<T, 3> &b, const Vector<T, 3> &c,
                        const Vector<T, 3> &d, Vector<T, 4> &weights) {
  Matrix<T, 3, 3> Dm;
  Dm.col(0) = a - d;
  Dm.col(1) = b - d;
  Dm.col(2) = c - d;
  ...
  ZIRAN_ASSERT(std::abs(weights.array().sum() - (T)1) < 1e-7,
               weights.transpose());
...
template void
barycentricWeights<double>(Eigen::Matrix<double, 3, 1, 0, 3, 1> const &,
...
template void
barycentricWeights<float>(Eigen::Matrix<float, 3, 1, 0, 3, 1> const &,
