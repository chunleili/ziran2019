...
namespace ZIRAN {
/**
    This is a class for keeping track of simplex mesh topology: e, i, j, k
   (element e has nodes i,j,k), etc. The class also has basic helpers like
   routines for computing the boundary simplex mesh
*/
template <int dim> class SimplexMesh {
public:
  ...
  SimplexMesh(StdVector<IV> &&indices) : indices(indices) {}
  ...
  /**
      ...
      Any (dim -1) dimension element (which is stored in vector of size dim)
     traversed only once is a boundary element. Any (dim -1) dimension element
     traversed twice is an interior element.
  */

  void initializeBoundaryElements();

  void constructListOfEdges(StdVector<EIV> &edge_list,
                            HashTable<EIV, int> *edge_hash_ptr = nullptr);

  // void constructListOfEdges(StdVector<EIV>& edge_list, EEIV*
  // associated_edge_list_begin, EEIV* associated_edge_list_end); // TODO: this
  // is no longer needed I believe (I == Stephanie)

  void constructListOfFaces(StdVector<BIV> &face_list,
                            HashTable<BIV, int> *face_hash_ptr = nullptr);

  void getFaces(const size_t element,
                std::array<Vector<int, dim>, dim + 1> &result);
  ...
  static void findSharedFaceAndIndex(const IV &element1, const IV &element2,
                                     BIV &sorted, BIV *local_index1 = nullptr,
                                     BIV *local_index2 = nullptr);
  ...
  void constructNeighborVertexList(const StdVector<NIV> &neighbor_list,
  ...
  */
  void constructListOfFaceNeighboringElements(
      StdVector<Vector<int, 2>> &result, const bool sort = false,
      HashTable<BIV, Eigen::Vector2i> *face_hash_ptr = nullptr);

  void constructListOfFaceNeighboringElementsInternal(
  ...
  */
  void closestElement(const int index, int &result,
