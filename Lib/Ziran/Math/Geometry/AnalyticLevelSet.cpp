...
std::function<OutputPolyMesh()>
AnalyticLevelSet<T, dim>::createMesh(T dx) const {
#ifdef ZIRAN_WITH_VDB
  std::vector<openvdb::Vec3s> points;
  std::vector<openvdb::Vec4I> quads;
  using GridT = typename openvdb::Grid<
      typename openvdb::tree::Tree4<float, 5, 4, 3>::Type>;
  ...
  return [points(std::move(points)), quads(std::move(quads)),
          face_vertex_count(std::move(face_vertex_count))]() {
    return OutputPolyMesh{points.data()->asPointer(),
                          points.size(),
...
template <class T, int dim>
typename AnalyticLevelSet<T, dim>::TV
AnalyticLevelSet<T, dim>::closestPointOnSurface(const TV &X) const {
  TV normal;
  T signed_distance;
...
*/
template <class T, int dim>
typename AnalyticLevelSet<T, dim>::TM
AnalyticLevelSet<T, dim>::hessian(const TV &X) const {
  ZIRAN_ASSERT(false, "Analytic level set hessian not implemented.");
}

template <class T, int dim>
bool AnalyticLevelSet<T, dim>::query(const TV &X, T &signed_distance,
                                     TV &normal, TM *hessian) const {
...
template <class T, int dim>
bool AnalyticLevelSet<T, dim>::queryInside(const TV &X, T &signed_distance,
                                           TV &normal,
                                           const T phi_critical) const {
  signed_distance = this->signedDistance(X);
...
template <class T, int dim> T AnalyticLevelSet<T, dim>::volume() const {
  ZIRAN_ASSERT(false, "No volume available.");
}

// DisjointUnioniLevelSet
template <class T, int dim>
DisjointUnionLevelSet<T, dim>::DisjointUnionLevelSet(
    const StdVector<std::unique_ptr<AnalyticLevelSet<T, dim>>>
        &level_sets_input) {
...
template <class T, int dim>
void DisjointUnionLevelSet<T, dim>::getBounds(TV &min_corner,
                                              TV &max_corner) const {
...
template <class T, int dim> T DisjointUnionLevelSet<T, dim>::volume() const {
  T result = 0;
  for (const auto &tls : level_sets)
    result += tls->volume();
  return result;
...
template <class T, int dim>
typename DisjointUnionLevelSet<T, dim>::TV
DisjointUnionLevelSet<T, dim>::normal(const TV &X) const {
  T min = std::numeric_limits<T>::max();
  TV n = TV::Zero();
  for (const auto &ls : level_sets) {
...
template <class T, int dim>
std::unique_ptr<AnalyticLevelSet<T, dim>>
DisjointUnionLevelSet<T, dim>::createPtr() const {
  return std::make_unique<DisjointUnionLevelSet<T, dim>>(level_sets);
}

// DifferenceLevelSet
template <class T, int dim>
DifferenceLevelSet<T, dim>::DifferenceLevelSet(
...
template <class T, int dim>
void DifferenceLevelSet<T, dim>::add(const AnalyticLevelSet<T, dim> &ls1,
                                     const AnalyticLevelSet<T, dim> &ls2) {
...
template <class T, int dim>
typename DifferenceLevelSet<T, dim>::TV
DifferenceLevelSet<T, dim>::normal(const TV &X) const {
  auto &setA = level_sets[0];
  auto &setB = level_sets[1];
...
template <class T, int dim>
void DifferenceLevelSet<T, dim>::getBounds(TV &min_corner,
                                           TV &max_corner) const {
  level_sets[0]->getBounds(min_corner, max_corner);
}

template <class T, int dim> T DifferenceLevelSet<T, dim>::volume() const {
  return level_sets[0]->volume() - level_sets[1]->volume();
}

template <class T, int dim>
std::unique_ptr<AnalyticLevelSet<T, dim>>
DifferenceLevelSet<T, dim>::createPtr() const {
...
HalfSpace<T, dim>::HalfSpace(const TV &origin_in, const TV &outward_normal_in)
    : origin(origin_in), outward_normal(outward_normal_in.normalized()) {}

template <class T, int dim> bool HalfSpace<T, dim>::inside(const TV &X) const {
  return (signedDistance(X) <= (T)0);
}
...
template <class T, int dim>
typename HalfSpace<T, dim>::TV
HalfSpace<T, dim>::closestPointOnSurface(const TV &X) const {
...
template <class T, int dim>
bool HalfSpace<T, dim>::query(const TV &X, T &signed_distance, TV &normal,
                              TM *hessian) const {
...
std::function<OutputPolyMesh()> HalfSpace<T, dim>::createMesh(T dx) const {
  ...
  return [points(std::move(points)), quads(std::move(quads)),
          face_vertex_count(std::move(face_vertex_count))]() {
    return OutputPolyMesh{(float *)points.data(),   points.size(),
                          (int32_t *)quads.data(),  quads.size() * 4,
                          face_vertex_count.data(), face_vertex_count.size()};
  };
}
// template <class T, int dim>
// class AnalyticBox;
...
template <class T, int dim>
AxisAlignedAnalyticBox<T, dim>::AxisAlignedAnalyticBox(const TV &min_corner,
                                                       const TV &max_corner)
    : box((max_corner - min_corner) / (T)2, Vector<T, 4>(dim == 3, 0, 0, 0),
          (min_corner + max_corner) / (T)2) {}
...
template <class T, int dim>
typename AxisAlignedAnalyticBox<T, dim>::TV
AxisAlignedAnalyticBox<T, dim>::normal(const TV &X) const {
  return box.normal(X);
}

template <class T, int dim>
void AxisAlignedAnalyticBox<T, dim>::getBounds(TV &min_corner,
                                               TV &max_corner) const {
  box.getBounds(min_corner, max_corner);
}

template <class T, int dim> T AxisAlignedAnalyticBox<T, dim>::volume() const {
  return box.volume();
}

template <class T, int dim>
std::unique_ptr<AnalyticLevelSet<T, dim>>
AxisAlignedAnalyticBox<T, dim>::createPtr() const {
...
template <class T, int dim> bool Sphere<T, dim>::inside(const TV &X) const {
...
template <class T, int dim>
typename Sphere<T, dim>::TV
Sphere<T, dim>::closestPointOnSurface(const TV &X) const {
...
typename Sphere<T, dim>::TM Sphere<T, dim>::hessian(const TV &X) const {
  ...
  return inv_norm * TM::Identity() -
         std::pow(inv_norm, 3) * outward_normal * outward_normal.transpose();
}

template <class T, int dim>
bool Sphere<T, dim>::queryInside(const TV &X, T &signed_distance, TV &norm,
                                 const T phi_critical) const {
...
template <class T, int dim>
bool Sphere<T, dim>::query(const TV &X, T &signed_distance, TV &norm,
                           TM *hess) const {
  signed_distance = signedDistance(X);
  norm = normal(X);
  if (hess) {
...
template <class T, int dim> T Sphere<T, dim>::volume() const {
  using std::pow;
...
template <class T, int dim>
AnalyticBox<T, dim>::AnalyticBox(const TV &half_edges_in, const Vector<T, 4> &q,
                                 const TV &b_in) {
  using std::cos;
  using std::sin;
  half_edges = half_edges_in;
  if (dim == 3) {
    R = Eigen::Quaternion<T>(q(0), q(1), q(2), q(3))
            .normalized()
            .toRotationMatrix()
            .template topLeftCorner<dim, dim>();
  } else if (dim == 2) {
    T theta = q(0);
...
template <class T, int dim>
typename AnalyticBox<T, dim>::TV
AnalyticBox<T, dim>::normal(const TV &X) const {
  TV X_primitive = R_inverse * (X - b);
...
template <class T, int dim> T AnalyticBox<T, dim>::volume() const {
  T result = (T)1;
  for (int i = 0; i < dim; i++)
...
template <class T, int dim>
std::unique_ptr<AnalyticLevelSet<T, dim>>
AnalyticBox<T, dim>::createPtr() const {
...
template <class T, int dim>
Torus<T, dim>::Torus(const T r0_in, const T r1_in, const Vector<T, 4> &q,
                     const TV &b_in)
    : r0(r0_in), r1(r1_in), b(b_in) {
  ZIRAN_ASSERT(dim == 3);
  R = Eigen::Quaternion<T>(q(0), q(1), q(2), q(3))
          .normalized()
          .toRotationMatrix()
...
template <class T, int dim> T Torus<T, dim>::signedDistance(const TV &X) const {
  TV X_primitive = R_inverse * (X - b);
  return signedDistancePrimitive(X_primitive);
}
...
template <class T, int dim> T Torus<T, dim>::volume() const {
  return std::abs(M_PI * MATH_TOOLS::sqr(r1) * 2 * M_PI *
                  r0); // the abs is for football (negative r0). TODO: fix.
}
