...
Grid<T, dim>::Grid(const TVI &number_cells, const T dx_in,
                   const TV &lowerCorner) {
  set(number_cells, dx_in, lowerCorner);
}

template <class T, int dim>
void Grid<T, dim>::set(const TVI &number_cells, const T dx_in,
                       const TV &lowerCorner) {
  dx = dx_in;
  for (int d = 0; d < dim; d++) {
...
template <class T, int dim>
Grid<T, dim>::Grid(const int m_input, const T dx_input, const T xmin_input,
                   const T ymin_input)
    : dx(dx_input) {
  assert(dim == 2);
...
template <class T, int dim>
Grid<T, dim>::Grid(const int m_input, const int n_input, const T dx_input,
                   const T xmin_input, const T ymin_input)
    : dx(dx_input) {
  assert(dim == 2);
  number_cells_per_dimension[0] = m_input;
...
template <class T, int dim>
Grid<T, dim>::Grid(const int m_input, const int n_input, const int p_input,
                   const T dx_input, const T xmin_input, const T ymin_input,
                   const T zmin_input)
    : dx(dx_input) {
  ZIRAN_ASSERT(dim == 3);
  number_cells_per_dimension[0] = m_input;
...
template <class T, int dim>
void Grid<T, dim>::resize(const int m_input, const int n_input,
                          const T dx_input, const T xmin_input,
                          const T ymin_input) {
  assert(dim == 2);
  number_cells_per_dimension[0] = m_input;
  number_cells_per_dimension[1] = n_input;
...
template <class T, int dim> T Grid<T, dim>::Dx() const { return dx; }
...
template <class T, int dim> int Grid<T, dim>::numberNodes() const {
  int result = 1;
  for (int d = 0; d < dim; d++)
...
template <class T, int dim> int Grid<T, dim>::numberCells() const {
  int result = 1;
  for (int d = 0; d < dim; d++)
...
template <class T, int dim> T Grid<T, dim>::gridVolume() const {
...
/**
     In the case of 2D, this returns the cell in a grid that covers all of the
   plane. I.e. the lower left cell in the grid is (0,0), but you could get (i,j)
   with i and j being any integers, negative, larger than
   number_cells_per_dimension etc. The user can decide how to handle cells not
   on the grid The case is similar for arbitrary dimension.
     */

template <class T, int dim>
void Grid<T, dim>::cellContainingPoint(const TV &position, TV &lambda,
                                       TVI &index) const {
