...
template <class T, int _dim>
void StvkWithHencky<T, _dim>::updateScratch(const TM &new_F,
...
template <class T, int _dim>
Matrix<T, 2, 2> StvkWithHencky<T, _dim>::Bij(const TV &sigma, int i, int j,
                                             T clamp_value) const {
...
template <class T, int _dim>
Matrix<T, 2, 2> StvkWithHencky<T, _dim>::BijFull(const TV &zi, const TV &sigma,
                                                 const T &second_term, int i,
                                                 int j, T clamp_value) const {
  if constexpr (dim == 2) {
    auto mingClampMagnitude = [&](const T input) {
      T magnitude = input > 0 ? input : -input;
      T sign = input > 0 ? 1.f : -1.f;
...
T StvkWithHencky<T, _dim>::psi(const Scratch &s) const {
  ...
  return mu * log_sigma_squared.array().sum() +
         (T).5 * lambda * trace_log_sigma * trace_log_sigma;
}
...
template <class T, int _dim>
void StvkWithHencky<T, _dim>::firstPiolaDerivative(const Scratch &s,
                                                   Hessian &dPdF) const {
...
template <class T, int _dim>
typename StvkWithHencky<T, _dim>::TV
StvkWithHencky<T, _dim>::firstPiolaDiagonal(const TV &sigma) const {
  TV dE;
  if constexpr (dim == 1) {
    // TODO: implement, and modify hessianImplemented() function.
    ZIRAN_ASSERT(false, "not implemented");
...
template <class T, int _dim>
typename StvkWithHencky<T, _dim>::TM
StvkWithHencky<T, _dim>::firstPiolaDerivativeDiagonal(const TV &sigma) const {
  TM ddE;
  if constexpr (dim == 1) {
    // TODO: implement, and modify hessianImplemented() function.
    ZIRAN_ASSERT(false, "not implemented");
...
template <class T, int _dim>
void StvkWithHencky<T, _dim>::firstPiolaDifferential(const Scratch &s,
                                                     const TM &dF,
                                                     TM &dP) const {
  // TODO: implement, and modify hessianImplemented() function.
  ZIRAN_ASSERT(false, "not implemented");
