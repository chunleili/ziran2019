...
template <typename Derived> struct HyperelasticTraits;

// scratch (non-state) variables for the consitutive model
template <class T, int dim> struct StvkWithHenckyScratch {
  ...
  StvkWithHenckyScratch() : F(TM::Identity()) {}

  static const char *name() { return "StvkWithHenckyScratch"; }
};

template <class T, int _dim>
class StvkWithHencky
    : public HyperelasticConstitutiveModel<StvkWithHencky<T, _dim>> {
public:
  ...
  using Scratch =
      typename HyperelasticTraits<StvkWithHencky<T, dim>>::ScratchType;
  ...
  Matrix<T, 2, 2> BijFull(const TV &zi, const TV &sigma, const T &second_term,
                          int i, int j, T clamp_value) const;
...
template <class T, int dim> struct HyperelasticTraits<StvkWithHencky<T, dim>> {
  using ScratchType = StvkWithHenckyScratch<T, dim>;
};

template <class T, int dim> struct RW<StvkWithHenckyScratch<T, dim>> {
