...
template <class Derived> class HyperelasticConstitutiveModel;

template <typename Derived> struct ScratchTrait;

template <class T, int _dim> class StvkWithHenckyIsotropic;

// scratch (non-state) variables for the consitutive model
template <class T, int dim> struct StvkWithHenckyIsotropicScratch {
  ...
  StvkWithHenckyIsotropicScratch() : isotropic(0) {}

  static const char *name() { return "StvkWithHenckyIsotropicScratch"; }
};

template <class T, int _dim>
class StvkWithHenckyIsotropic
    : public HyperelasticConstitutiveModel<StvkWithHenckyIsotropic<T, _dim>> {
public:
  ...
  using Scratch =
      typename HyperelasticTraits<StvkWithHenckyIsotropic<T, dim>>::ScratchType;
  ...
  void updateScratch(const TM &new_F, Scratch &s) const {
    ...
    s.isotropic.psi00 =
        (g * (1 - s.logS(0)) - lambda * s.logS(1)) / sqr(s.sigma(0));
    s.isotropic.psi11 =
        (g * (1 - s.logS(1)) - lambda * s.logS(0)) / sqr(s.sigma(1));
    ...
    // (psi0+psi1)/(sigma0+sigma1)
    s.isotropic.p01 = (s.isotropic.psi0 + s.isotropic.psi1) /
                      clamp_small_magnitude(s.sigma(0) + s.sigma(1), eps);
  }
  ...
  void updateScratch(const TM &new_F, Scratch &s) const {
    ...
    s.isotropic.psi00 =
        (g * (1 - s.logS(0)) - lambda * (s.logS(1) + s.logS(2))) /
        sqr(s.sigma(0));
    s.isotropic.psi11 =
        (g * (1 - s.logS(1)) - lambda * (s.logS(0) + s.logS(2))) /
        sqr(s.sigma(1));
    s.isotropic.psi22 =
        (g * (1 - s.logS(2)) - lambda * (s.logS(0) + s.logS(1))) /
        sqr(s.sigma(2));
    ...
    s.isotropic.m01 =
        -(lambda * sum_log + 2 * mu *
                                 diff_interlock_log_over_diff(
                                     s.sigma(0), s.sigma(1), s.logS(1), eps)) /
        prod01;
    s.isotropic.m02 =
        -(lambda * sum_log + 2 * mu *
                                 diff_interlock_log_over_diff(
                                     s.sigma(0), s.sigma(2), s.logS(2), eps)) /
        prod02;
    s.isotropic.m12 =
        -(lambda * sum_log + 2 * mu *
                                 diff_interlock_log_over_diff(
                                     s.sigma(1), s.sigma(2), s.logS(2), eps)) /
        prod12;

    // (psiA+psiB)/(sigmaA+sigmaB)
    s.isotropic.p01 = (s.isotropic.psi0 + s.isotropic.psi1) /
                      clamp_small_magnitude(s.sigma(0) + s.sigma(1), eps);
    s.isotropic.p02 = (s.isotropic.psi0 + s.isotropic.psi2) /
                      clamp_small_magnitude(s.sigma(0) + s.sigma(2), eps);
    s.isotropic.p12 = (s.isotropic.psi1 + s.isotropic.psi2) /
                      clamp_small_magnitude(s.sigma(1) + s.sigma(2), eps);
  }
  ...
  T psi(const Scratch &s) const {
    ...
    return mu * logS_squared.array().sum() +
           (T).5 * lambda * trace_logS * trace_logS;
  }
  ...
  Matrix<T, 2, 2> Bij(const TV &sigma, int i, int j, T clamp_value) const {
    ...
    if constexpr (dim == 2) {
      ...
      T B_Mij =
          -0.5 *
          (lambda * sum_log + 2 * mu *
                                  MATH_TOOLS::diff_interlock_log_over_diff(
                                      sigma(i), sigma(j), logS(j), eps)) /
          prodij;
  ...
  TM firstPiolaDerivativeDiagonal(const TV &sigma) const {
    ...
    if constexpr (dim == 1) {
      ...
      T prod = sigma(0) * sigma(1);
      ddE(0, 0) =
          (g * (1 - logS(0)) - lambda * logS(1)) / (sigma(0) * sigma(0));
      ddE(1, 1) =
          (g * (1 - logS(1)) - lambda * logS(0)) / (sigma(1) * sigma(1));
      ddE(0, 1) = ddE(1, 0) = lambda / prod;
    } else {
      TV logS = sigma.array().abs().log();
      T g = 2 * mu + lambda;
      ddE(0, 0) = (g * (1 - logS(0)) - lambda * (logS(1) + logS(2))) /
                  (sigma(0) * sigma(0));
      ddE(1, 1) = (g * (1 - logS(1)) - lambda * (logS(0) + logS(2))) /
                  (sigma(1) * sigma(1));
      ddE(2, 2) = (g * (1 - logS(2)) - lambda * (logS(0) + logS(1))) /
                  (sigma(2) * sigma(2));
  ...
  */
  bool hessianImplemented() const { return true; }
  ...
  static const char *name() { return "StvkWithHenckyIsotropic"; }

  inline static AttributeName<StvkWithHenckyIsotropic<T, dim>> attributeName() {
    return AttributeName<StvkWithHenckyIsotropic<T, dim>>(
        "StvkWithHenckyIsotropic");
  }
...
template <class T, int dim> struct RW<StvkWithHenckyIsotropicScratch<T, dim>> {
