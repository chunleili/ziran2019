...
template <class T, int _dim>
void NeoHookeanBorden<T, _dim>::updateScratch(const TM &new_F,
                                              Scratch &scratch) {
  using std::log;
...
template <class T, int _dim>
void NeoHookeanBorden<T, _dim>::firstPiolaDifferential(const Scratch &s,
                                                       const TM &dF,
                                                       TM &dP) const {
  using namespace EIGEN_EXT;
  ...
  TM dP_dev = 2 * a * muJ2a * contractMatrices(s.FinvT, dF) *
              (a * FcontractF * s.FinvT + s.F);
  dP_dev += muJ2a * (2 * a * contractMatrices(s.F, dF) * s.FinvT +
                     a * FcontractF * deltaFinvT + dF);

  TM dP_vol = deltaJ * prime * s.FinvT + s.J * deltaJ * prime2 * s.FinvT +
              s.J * prime * deltaFinvT;
