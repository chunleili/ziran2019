...
template <class T, int dim> class CotangentOrthotropic;

class PlasticityApplierBase {
public:
  virtual ~PlasticityApplierBase() {}

  virtual void applyPlasticity(const DisjointRanges &subrange,
                               DataManager &data_manager) = 0; // parallel
};
...
public:
  ...
  // This is assumed to be called in a parallel loop that split all
  // particles/elements to subranges.
  void applyPlasticity(const DisjointRanges &subrange,
                       DataManager &data_manager) override {
    auto constitutive_model_name = AttributeName<TConst>(TConst::name());
    auto plastic_name = AttributeName<TPlastic>(TPlastic::name());
    if (!data_manager.exist(constitutive_model_name) ||
        !data_manager.exist(plastic_name))
      return;

    DisjointRanges subset(subrange,
                          data_manager.commonRanges(constitutive_model_name,
                                                    strain_name, plastic_name));
    for (auto it = data_manager.subsetIter(subset, constitutive_model_name,
                                           strain_name, plastic_name);
         it; ++it) {
...
template <class T> class DummyPlasticity {
public:
  ...
  template <class TConst>
  Vector<T, TConst::dim> projectSigma(TConst &c,
  ...
  projectSigmaDerivative(TConst &c, const Vector<T, TConst::dim> &sigma) {
    return Matrix<T, TConst::dim, TConst::dim>::Identity();
  }

  template <class TConst>
  void projectSigmaAndDerivative(
  ...
  template <int dim>
  T secondTerm(const StvkWithHencky<T, dim> &c, const Vector<T, dim> &sigma,
               int i, int j) {
    return (T)1;
  }

  static const char *name() { return "DummyPlasticity"; }
};

template <class T> class NonAssociativeCamClay {
public:
  T logJp, M, beta, xi;
  bool hardeningOn;

  NonAssociativeCamClay(T logJp = 0, T friction_angle = 0, T beta = 0, T xi = 0,
                        int dim = 2, bool hardeningOn = true);
 ...
 */
template <class T> class DruckerPragerStvkHencky {
public:
  ...
  DruckerPragerStvkHencky(const T friction_angle = 30, const T beta = 1,
                          const T cohesion = 0,
                          const bool volume_correction = true);

  void setParameters(const T friction_angle_in, const T beta_in,
                     const T cohesion_in);
  ...
  T secondTerm(const StvkWithHencky<T, dim> &c, const Vector<T, dim> &sigma,
    ...
    T delta_gamma = epsilon_hat_norm + (dim * c.lambda + 2 * c.mu) /
                                           (2 * c.mu) * trace_epsilon * alpha;
    TV H;
    ...
    return ((T)1 - delta_gamma / epsilon_hat_norm) *
           MATH_TOOLS::diff_exp_over_diff(H(i), H(j), eps) *
           MATH_TOOLS::diff_log_over_diff(sigma(i), sigma(j), eps);
  }

  template <class TConst>
  void computeSigmaPInverse(TConst &c, const Vector<T, TConst::dim> &sigma_e,
...
template <class T, int dim> class VonMisesStvkHencky {
public:
  ...
  VonMisesStvkHencky() : yield_stress(0), xi(0), fail_stress(0) {}

  VonMisesStvkHencky(const T yield_stress, const T fail_stress, const T xi);

  void setParameters(const T yield_stress_in, const T xi_in,
                     const T fail_stress_in = -1);
...
template <class T, int dim> struct RW<VonMisesStvkHencky<T, dim>> {
