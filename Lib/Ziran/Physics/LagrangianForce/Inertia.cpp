...
MassLumpedInertia<T, dim>::MassLumpedInertia(const Vec &mass_matrix,
                                             const TVStack &dv, const T &dt)
    : mass_matrix(mass_matrix), dv(dv), dt(dt) {}

// E
template <class T, int dim> T MassLumpedInertia<T, dim>::totalEnergy() const {
  T ke = tbb::parallel_reduce(
      tbb::blocked_range<int>(0, dv.cols(), 256), (T)0,
      [&](const tbb::blocked_range<int> &range, T ns) -> T {
        ...
        return ns +
               ((dv_block.colwise().squaredNorm()).array() * mass_block.array())
                   .sum();
      },
...
// f= -dE/dx
template <class T, int dim>
void MassLumpedInertia<T, dim>::addScaledForces(T scale,
...
// df
template <class T, int dim>
void MassLumpedInertia<T, dim>::addScaledForceDifferential(T scale,
                                                           const TVStack &dx,
                                                           TVStack &df) const {
  scale /= dt * dt;
...
// -df/dx
template <class T, int dim>
void MassLumpedInertia<T, dim>::addScaledStiffnessEntries(
...
template <class T, int dim>
void MassLumpedInertia<T, dim>::initializeStiffnessSparsityPattern(
