...
template <class T, int dim> class SimplexMeshPoisson {
public:
  ...
  SimplexMeshPoisson(SimplexMesh<dim> &mesh, StdVector<TV> &X, VEC &x, VEC &b,
                     StdVector<int> &dirichlet_in,
                     StdVector<T> &dirichlet_value_in)
      : tet_mesh(mesh), X(X), element_measure(mesh.numberElements()),
        b_scale(mesh.numberElements()), grad_N(mesh.numberElements()),
        n(X.size()), x(x), b(b), dirichlet(dirichlet_in),
    ...
    // set up Dm_inverse and interpolating function derivatives (wrt rest
    // configuration)
    TM dm;
    ...
    for (size_t e = 0; e < tet_mesh.numberElements(); e++) {
      for (int i = 0; i <= dim; i++) {
        for (int j = 0; j <= dim; j++) {
          mat.coeffRef(tet_mesh.index(e, i), tet_mesh.index(e, j)) +=
              grad_N[e].col(i).dot(grad_N[e].col(j)) * element_measure[e];
        }
      }
    }
    mat.makeCompressed();
  ...
  void dS(const size_t element, TM &result) {
    for (int i = 0; i < dim; i++)
      result.col(i) =
          X[tet_mesh.index(element, i + 1)] - X[tet_mesh.index(element, 0)];
  }
  ...
  // after solving the poission for \u(x), get normalized \grad \u(x) for each
  // tetrahedron
