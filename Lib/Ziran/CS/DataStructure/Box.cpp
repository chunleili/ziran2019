...
template <class T, int dim> Box<T, dim>::Box() {
...
Box<T, dim>::Box(const Box<T, dim> &box)
    : min_corner(box.min_corner), max_corner(box.max_corner),
      side_length(box.side_length), volume(box.volume) {}

template <class T, int dim>
void Box<T, dim>::updateCorners(const EigenTV &new_min_corner,
                                const EigenTV &new_max_corner) {
...
template <int dim>
MaxExclusiveBoxIterator<dim>::MaxExclusiveBoxIterator(
    const MaxExclusiveBoxIterator<dim> &rhs)
    : box(rhs.box), index(rhs.index) {}
...
template <int dim> bool MaxExclusiveBoxIterator<dim>::valid() {
  bool ret = true;
  for (int d = 0; d < dim; ++d) {
...
template <int dim> int &MaxExclusiveBoxIterator<dim>::operator()(int d) {
  return index(d);
}

template Box<int, 2>::Box(Eigen::Matrix<int, 2, 1, 0, 2, 1> const &,
                          Eigen::Matrix<int, 2, 1, 0, 2, 1> const &);
template Box<int, 3>::Box(Eigen::Matrix<int, 3, 1, 0, 3, 1> const &,
                          Eigen::Matrix<int, 3, 1, 0, 3, 1> const &);
template MaxExclusiveBoxIterator<2>::MaxExclusiveBoxIterator(
    Box<int, 2> const &);
template MaxExclusiveBoxIterator<2> &MaxExclusiveBoxIterator<2>::operator++();
template bool MaxExclusiveBoxIterator<2>::valid();
template MaxExclusiveBoxIterator<3>::MaxExclusiveBoxIterator(
    Box<int, 3> const &);
