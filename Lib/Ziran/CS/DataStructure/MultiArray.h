...
template <class T, int dim, class enable = void> class MultiArray;
...
public:
  ...
  template <class IV> T &operator()(const IV &index) {
    return data(index(0), index(1));
  }

  template <class IV> const T &operator()(const IV &index) const {
    return data(index(0), index(1));
  }
  ...
  template <class Func> void loop(Func &&func) {
    for (int i = 0; i < size(0); ++i) {
...
public:
  ...
  MultiArray() : data(NULL) {}

  MultiArray(const IV &size_in) {
    size = size_in;
    // ZIRAN_ASSERT(size.prod() > 0, "ERROR in MultiArray<T,
    // dim>::MultiArray(const IV& size_in), size.prod() = 0!");
  ...
  MultiArray(const IV &size_in, T val) {
    size = size_in;
    // ZIRAN_ASSERT(size.prod() > 0, "ERROR in MultiArray<T,
    // dim>::MultiArray(const IV& size_in, T val), size.prod() = 0!");
    data = new T[size.prod()];
  ...
  MultiArray(const MultiArray<T, dim> &multi) {
    size = multi.size;
    // ZIRAN_ASSERT(size.prod() > 0, "ERROR in MultiArray<T,
    // dim>::MultiArray(const MultiArray<T, dim>& multi), size.prod() = 0!");
    data = new T[size.prod()];
    for (int i = 0; i < size.prod(); ++i)
      data[i] = multi.data[i];
  }
  ...
  void resize(const IV &size_in, T val) {
    size = size_in;
    // ZIRAN_ASSERT(size.prod() > 0, "ERROR in MultiArray<T, dim>::resize(const
    // IV& size_in, T val), size.prod() = 0!");
    data = new T[size.prod()];
    for (int i = 0; i < size.prod(); ++i)
      data[i] = val;
  }

  template <class IV> inline int id(const IV &index) const {
    return index(0) * size(1) * size(2) + index(1) * size(2) + index(2);
  }

  template <class IV> T &operator()(const IV &index) { return data[id(index)]; }
...
template <class T, int dim, int res, class enable = void> class CubeMultiArray;
...
public:
  ...
  template <class IV> T &operator()(const IV &index) {
    assert((index[0] >= 0) && (index[1] >= 0) && (index[0] < res) &&
           (index[1] < res));
    return data[id(index)];
  }
...
public:
  ...
  template <class IV> T &operator()(const IV &index) {
    assert((index[0] >= 0) && (index[1] >= 0) && (index[2] >= 0) &&
           (index[0] < res) && (index[1] < res) && (index[2] < res));
    return data[id(index)];
  }
