...
/**
  ...
  */
template <class Type> struct AttributeName {
public:
  const std::string name;
  const size_t hash;

  AttributeName(const char *a)
      : name(a), hash(std::hash<std::string>()(name)) {}
...
template <bool is_const, class... Types> class DataManagerIterator;

template <class... Types> class DataAppender;
...
/**
    ...
    Once created, this class is the only interface of using or modifying entry
   data.
*/
class DataManager {
public:
  ...
  /**
    Construct empty arrays with names and types given by the input
    `AttributeName<T>` Note that since `AttributeName<T>` can be freely
    constructed from a char * You can use the form `DataManager<int> data("a");`
    or
    `DataManager data(AttributeName<int>("a"));`
    the later is useful when the `AttributeName<int>("a")` is stored in a
    variable for frequent usage
    */
  ...
  /**
    ...
    */
  template <class Type> DataArray<Type> &add(const AttributeName<Type> &a) {
  ...
  DataArray<Type> &add(const AttributeName<Type> &name, Range range,
                       const std::vector<Type, Allocator> &array) {
    return add<Type>(AttributeName<Type>(name), range,
                     StdVector<Type>(array.begin(), array.end()));
  }

  template <class Type>
  DataArray<Type> &add(const AttributeName<Type> &a, Range range,
    ...
    if (search == batches.end()) {
      ...
      auto r = batches.emplace(
          a.hash, std::make_unique<DataArray<Type>>(
  ...
  template <class Type> bool exist(const AttributeName<Type> &a) const {
    auto search = batches.find(a.hash);
    if (search != batches.end()) {
      ZIRAN_ASSERT(a.name == search->second->name);
    }
  ...
  Range getNextRange(int size) const { return Range{count, count + size}; }
  ...
  template <typename... Types>
  DataManagerIterator<false, Types...>
  iter(const AttributeName<Types> &... names) {
    return DataManagerIterator<false, Types...>(get(names)...);
  }

  template <typename... Types>
  DataManagerIterator<true, Types...>
  iter(const AttributeName<Types> &... names) const {
  ...
  template <typename... Types>
  DataManagerIterator<false, Types...>
  subsetIter(const DisjointRanges &subset,
             const AttributeName<Types> &... names) {
  ...
  DataManagerIterator<true, Types...>
  subsetIter(const DisjointRanges &subset,
             const AttributeName<Types> &... names) const {
  ...
  */
  virtual void readData(std::istream &in) {
    ...
    for (size_t i = 0; i < size; i++) {
      ...
      ZIRAN_ASSERT(search != batches.end(), "Array ", name,
                   " was not initialized before reading.");
  ...
  void parallel_for(const Func &f, const AttributeName<Types> &... names) {
    ...
    return tbb::parallel_for(
        range, [&](const tbb::blocked_range<int> &subrange) {
  ...
  template <class Func, typename... Types>
  void parallel_for_subset(const DisjointRanges &subset, const Func &f,
                           const AttributeName<Types> &... names) {
  ...
  Value map_reduce(const Value &identity, const Func1 &map,
                   const Func2 &reduction,
                   const AttributeName<Types> &... names) const {
    auto master = iter(names...);
    tbb::blocked_range<int> range(0, master.common_ranges->length(), 1024);
    return tbb::parallel_reduce(range, identity,
                                [&](const tbb::blocked_range<int> &subrange,
                                  ...
                                  auto end = master + subrange.end();
                                  Value x = init;
                                  for (auto i = master + subrange.begin();
                                       i != end; ++i) {
...
/**
  ...
  */
template <class Type, bool is_const> class DataManagerIteratorHelper {
  typedef std::conditional_t<is_const, typename StdVector<Type>::const_iterator,
...
public:
  typedef typename std::conditional<is_const, const DataArray<Type> &,
                                    DataArray<Type> &>::type BatchType;

  typedef typename std::conditional<is_const, const Type &, Type &>::type
      ReferenceType;

  DataManagerIteratorHelper(BatchType batch, int start)
      : r(batch.ranges.cbegin()), a(batch.array.begin())
#ifndef NDEBUG
        ,
...
public:
  ...
  DataManagerIterator(
      typename DataManagerIteratorHelper<Types, is_const>::BatchType... batches)
      : common_ranges(new DisjointRanges(batches.ranges...)), owns_ranges(true),
        s(common_ranges->cbegin()),
        p(s != common_ranges->cend() ? s->lower : -1),
        iterators(DataManagerIteratorHelper<Types, is_const>(batches, p)...) {}

  DataManagerIterator(
      const DisjointRanges &common_ranges_input,
      typename DataManagerIteratorHelper<Types, is_const>::BatchType... batches)
      : common_ranges(new DisjointRanges(common_ranges_input)),
        owns_ranges(true), s(common_ranges->cbegin()),
        p(s != common_ranges->cend() ? s->lower : -1),
        iterators(DataManagerIteratorHelper<Types, is_const>(batches, p)...) {}

  DataManagerIterator(const DataManagerIterator &other)
      : common_ranges(other.common_ranges), owns_ranges(false), s(other.s),
        p(other.p), iterators(other.iterators) {}
  ...
  explicit operator bool() const { return s != common_ranges->cend(); }

  bool operator==(const DataManagerIterator &o) const { return p == o.p; }

  bool operator!=(const DataManagerIterator &o) const { return p != o.p; }

  int entryId() const { return p; }

  DataManagerIterator<is_const, Types...> &operator++() { return (*this += 1); }
  ...
  std::tuple<typename DataArrayIterator<Types, is_const>::ReferenceType...>
  operator*() {
  ...
  template <class Func> auto apply(const Func &f) {
    return applyHelper(f, std::index_sequence_for<Types...>());
  }

  template <size_t I> auto get() -> decltype(*std::get<I>(iterators)) {
    return *std::get<I>(iterators);
  }

  template <size_t I> auto get() const -> decltype(*std::get<I>(iterators)) {
    return *std::get<I>(iterators);
  }
...
private:
  ...
  std::tuple<typename DataArrayIterator<Types, is_const>::ReferenceType...>
  dereferenceHelper(std::index_sequence<I...>) {
...
public:
  ...
  DataAppender(int &count, DataArray<Types> &... batches)
      : batches(batches...), entry_id(count) {}

  int entryId() const { return entry_id; }

  template <class... Ts> void append(Ts &&... values) {
...
private:
  template <int index> void appendHelper() {}
  ...
  template <int index, class T,
            TICK_REQUIRES(!std::is_same<std::decay<T>, Skip>::value),
            class... Ts>
...
template <class Type> using AttributeName = DATA::AttributeName<Type>;
using DATA::Skip;
