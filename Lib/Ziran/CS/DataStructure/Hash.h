...
inline void murmurHash3(const void *key, const int len, const uint32_t seed,
                        void *out);

namespace INTERNAL {
template <class T, bool has = HasType<T>::value> struct GetHashType {
  using Type = size_t;
};

template <class T> struct GetHashType<T, true> {
  using Type = typename T::Type;
};
} // namespace INTERNAL

template <class T> using GetHashType = typename INTERNAL::GetHashType<T>::Type;

template <class T, int m, int n> struct Hash<Matrix<T, m, n>> {
  const uint32_t seed;
  ...
  size_t operator()(const Matrix<T, m, n> &A) const {
    uint64_t hash[2];
    murmurHash3((const void *)A.data(), sizeof(T) * A.size(), seed,
                (void *)hash);
...
template <class T>
struct Hash<T, TICK_CLASS_REQUIRES(std::is_arithmetic<T>::value &&
                                   sizeof(T) <= 4)> {
  using Type = uint32_t;
...
template <class T> struct Hash<std::basic_string<T>> {
  const uint32_t seed;
...
template <class T> struct Hash<T *> {
  ...
  size_t operator()(const T *ptr) const { return internal((uintptr_t)ptr); }
};
...
inline ZIRAN_FORCE_INLINE uint64_t getBlock64(const uint64_t *p, int i) {
  return p[i];
}
