...
public:
  ...
  DisjointRanges(int lg2_grain_size = 7) : lg2_grain_size(lg2_grain_size) {}
  ...
  */
  ...
  DisjointRanges(const DisjointRanges &dr, const Rest &... rest)
      : lg2_grain_size(dr.lg2_grain_size), ranges(dr.ranges) {
    intersect(rest...);
  }

  // tbb splitting constructors are only defined if tbb/tbb_stddef.h was
  // included
  ...
  void append(const Range &range) {
    ZIRAN_ASSERT(ranges.empty() || range.lower >= ranges.back().upper,
  ...
  ConstIterator begin() const { return ranges.cbegin(); }

  ConstIterator end() const { return ranges.cend(); }

  ConstIterator cbegin() const { return ranges.cbegin(); }

  ConstIterator cend() const { return ranges.cend(); }
  ...
  // for tbb
  bool is_divisible() const { return length() > lg2_grain_size; }
  ...
  template <class... Rest>
  void intersect(const DisjointRanges &ranges1, const Rest &... rest) {
    auto r0 = cbegin();
    if (r0 == cend())
      return;
    {
  ...
  template <class... Rest>
  void merge(const DisjointRanges &ranges1, const Rest &... rest) {
    auto r0 = cbegin();
    auto r1 = ranges1.cbegin();
    if (r0 == cend()) {
      *this = ranges1;
