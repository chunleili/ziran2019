...
template <class Type, bool is_const> class DataArrayIterator;

template <class Type> class DataArray : public DataArrayBase {
public:
  using DataArrayBase::name;
  using DataArrayBase::ranges;
  /**
    The value array is public for conveniance, it's size should not be changed
    directly unless you can uphold the necessary invariants.
    1. The ranges and entries of the array must be kept consistent
    2. The total number of all possible entries should match the count in the
    data manager
    */
  StdVector<Type> array;

  DataArray(const std::string &name, const DisjointRanges &ranges,
            StdVector<Type> &&array)
  ...
  virtual std::unique_ptr<DataArrayBase> makeEmptyCopy() override {
    return std::make_unique<DataArray<Type>>(name, DisjointRanges(),
                                             StdVector<Type>());
  }
  ...
  virtual void reorder(const StdVector<int> &new_to_old) override {
    ...
    for (const auto &callback : reordering_callbacks)
      callback(
          [&](int old_value_id) { return old_to_new_value_id[old_value_id]; });
  }
  ...
  */
  void reserveMore(const size_t n) { array.reserve(array.size() + n); }
...
*/
template <class Type>
Eigen::Map<Vector<ScalarType<Type>, Eigen::Dynamic>>
toVectorMap(DataArray<Type> &data) {
...
public:
  ...
  DataArrayIterator(BatchType batch, const DisjointRanges &subset)
      : r(batch.ranges.cbegin()), s(subset.cbegin()), a(batch.array.begin())
#ifndef NDEBUG
        ,
        aend(batch.array.end())
#endif
        ,
        end(subset.cend()), p((r != batch.ranges.cend()) ? (r->lower) : -1) {
    if (s != end) {
      advanceToEntryId(s->lower);
    }
  }

  DataArrayIterator(BatchType batch) : DataArrayIterator(batch, batch.ranges) {}
...
template <class Type> using DataArray = DATA::DataArray<Type>;
} // namespace ZIRAN
#endif
