...
template <class T> inline static T makePowerOfTwo(T x) {
...
*/
   ...
   */
  template <class K, class V> InsertResult insert(K &&key, V &&value) {
    if (m_size + 1 >= max_load_factor * m_capacity) {
      rehash(2 * m_capacity);
    }
    HashType hash = computeHash(key);
    return insertHelper(hash,
                        Entry{std::forward<K>(key), std::forward<V>(value)});
  }

  /**
    Removes the entry from the hashtable for key, returning whether it was
    removed. The removed entry, if any will be moved into the passed in entry
    */
  ...
  template <class K> Value &operator[](K &&key) {
    if (m_size + 1 >= max_load_factor * m_capacity) {
      rehash(2 * m_capacity);
    }
    HashType hash = computeHash(key);
    InsertResult new_entry =
        insertHelper(hash, Entry{std::forward<K>(key), Value()});
...
*/
  ...
  double loadFactor() const { return (double)m_size / m_capacity; }

  HashType size() const { return m_size; }

  HashType capacity() const { return m_capacity; }
  ...
  Iterator<false> end() { return Iterator<false>(*this, m_capacity); }

  Iterator<true> begin() const { return cbegin(); }

  Iterator<true> end() const { return cend(); }
  ...
  Iterator<true> cend() const { return Iterator<true>(*this, m_capacity); }

  template <bool is_const> class Iterator {
    using HT = std::conditional_t<is_const, const HashTable &, HashTable &>;
    HT &hash_table;
    HashType index;
    friend class HashTable;

  public:
    using Ptr = std::conditional_t<is_const, const KeyValuePair<Key, Value> *,
                                   KeyValuePair<Key, Value> *>;
    using Ref =
        std::conditional_t<is_const,
                           KeyValuePair<std::reference_wrapper<const Key>,
                                        std::reference_wrapper<const Value>>,
                           KeyValuePair<std::reference_wrapper<const Key>,
                                        std::reference_wrapper<Value>>>;

    Iterator(HT &hash_table, HashType index)
        : hash_table(hash_table), index(index) {}

    Iterator(const Iterator &other)
        : hash_table(other.hash_table), index(other.index) {}
    ...
    Ptr operator->() { return &hash_table.entries[index]; }
...
private:
  ...
  HashType desiredPosition(HashType hash) const { return hash & m_mask; }
  ...
  HashType probe(HashType position) const { return (position + 1u) & m_mask; }
