...
*/
template <int dim> class KdTreeHelper {
  typedef Vector<double, dim> TV;

public:
  int id;
  TV pos;
  KdTreeHelper(int id, const TV &pos) : id(id), pos(pos) {}
...
public:
  struct tac {
    ...
    double operator()(KdTreeHelper<dim> const &t, size_t k) const {
      return t.pos(k);
    }
  };
  ...
  */
  template <class TV_IN> void addPoint(const int i, const TV_IN &new_p);
  ...
  */
  template <class TV_IN> void erasePoint(const int i, const TV_IN &erase_p);
};
} // namespace ZIRAN
