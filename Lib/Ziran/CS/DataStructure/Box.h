...
Box
*/
template <class T, int dim> class Box {
public:
  ...
  void updateCorners(const EigenTV &new_min_corner,
...
void maxExclusiveBoxOp(Func &func, const Box<T, dim> &box) {
...
*/
template <class T, int dim, class Func>
void maxExclusiveBoxOpWithEarlyExit(Func &func, const Box<T, dim> &box,
                                    Func &exit_func) {
  Vector<T, dim> indices(box.min_corner);
  int d = 0;
...
*/
template <int dim> class MaxExclusiveBoxIterator {
public:
