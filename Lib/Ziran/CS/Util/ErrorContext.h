...
class ErrorContext {
public:
  ErrorContext(const char *function_name, const char *file, int line_number,
               const char *var_name);
  ...
  template <typename T> void capture(const T &var) {
    stream << var << '\n';
    auto &call_stack = callStack();
    call_stack.emplace_back(stream.str());
  }
...
#define ZIRAN_CONTEXT2(string, var)                                            \
  ErrorContext ZIRAN_COMBINE(context, __LINE__){__func__, __FILE__, __LINE__,  \
                                                string};                       \
  ZIRAN_COMBINE(context, __LINE__).capture(var)
