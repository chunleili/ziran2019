...
template <class Type, class enable = void> struct RW;

template <class Type> struct TriviallyCopyableTag {};
template <class T, int m, int n> struct EigenMatrixTag {};
template <class T> struct EigenSparseMatrixTag {};
template <class T> struct UniquePtrTag {};
template <class T> struct BasicStringTag {};
template <class T> struct CustomTypeTag {};
template <class T> struct NoWriteTag {};

template <class T, size_t size> struct StdArrayTag {};
...
template <class T> struct RW<Eigen::SparseMatrix<T>> {
  using Tag = EigenSparseMatrixTag<T>;
};

template <class T> struct RW<std::unique_ptr<T>> {
  using Tag = UniquePtrTag<T>;
};

template <class T> struct RW<std::basic_string<T>> {
  using Tag = BasicStringTag<T>;
};

// template <class T, size_t size>
// struct RW<std::array<T, size>,
// std::enable_if_t<std::is_trivially_copyable<T>::value>> {
//     using Tag = StdArrayTag<T, size>;
// };

template <size_t size, typename Type>
struct RW<std::array<Type, size>,
          std::enable_if_t<!std::is_trivially_copyable<Type>::value>> {
  using Tag = StdArrayTag<Type, size>;
};

template <class Type> void writeEntry(std::ostream &out, const Type &x);

template <class Type> inline Type readEntry(std::istream &in);
...
template <class T> void readSTDVector(std::istream &in, StdVector<T> &a) {
  uint64_t size = readEntry<uint64_t>(in);
  uint64_t bytes = readEntry<uint64_t>(in);
  ZIRAN_ASSERT(bytes == sizeof(T),
               "Read error: The size of the types don't match");
  a.clear();
  a.reserve(size);
...
template <class T>
inline void writeHelper(std::ostream &out, const T a, TriviallyCopyableTag<T>) {
  out.write(reinterpret_cast<const char *>(&a), sizeof a);
}

template <class T>
inline T readHelper(std::istream &in, TriviallyCopyableTag<T>) {
  T a;
...
template <class T, int m, int n, int flags, int mm, int nn>
void writeHelper(std::ostream &out,
                 const Eigen::Matrix<T, m, n, flags, mm, nn> &a,
                 EigenMatrixTag<T, m, n>) {
  if (m == Eigen::Dynamic)
...
void writeHelper(std::ostream &out, const std::unique_ptr<CustomType> &model,
                 UniquePtrTag<CustomType>) {
  if (model == nullptr)
    writeEntry(out, '\0');
  else {
    writeEntry(out, 'u');
...
template <class CustomType>
inline std::unique_ptr<CustomType> readHelper(std::istream &in,
                                              UniquePtrTag<CustomType>) {
...
template <class T>
void writeHelper(std::ostream &out, const std::basic_string<T> &a,
                 BasicStringTag<T>) {
  writeEntry(out, (uint64_t)a.size());
...
template <class T>
void writeHelper(std::ostream &out, const Eigen::SparseMatrix<T> &a,
                 EigenSparseMatrixTag<T>) {
  writeEntry(out, a.rows());
  writeEntry(out, a.cols());
  writeEntry(out, (uint64_t)a.nonZeros());
  writeEntry(out,
             sizeof(Eigen::Triplet<T, typename Eigen::SparseMatrix<T>::Index>));
...
template <class T>
inline Eigen::SparseMatrix<T> readHelper(std::istream &in,
                                         EigenSparseMatrixTag<T>) {
...
/**
  ...
  */
template <class CustomType>
void writeHelper(std::ostream &out, const CustomType &model,
...
template <class T, size_t size>
void writeHelper(std::ostream &out, const std::array<T, size> &a,
                 StdArrayTag<T, size>) {
  for (const auto &s : a)
    writeEntry(out, s);
}
