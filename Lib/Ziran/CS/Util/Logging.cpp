...
std::ostream &TimingResult::print(std::ostream &os, bool print_self,
                                  int time_width) const {
  os << std::fixed << std::setw(time_width) << total_seconds.count() << "s";
  if (print_self)
...
std::string LogMessage::toString() {
  ...
  std::string line;
  std::getline(is, line);
  Color c = (level < Warning) ? Color::GREY
                              : (level < Error) ? Color::YELLOW : Color::RED;
  std::string indent;
  if (level >= Fatal) {
    os << "\n"
       << c << level.text << Color::RESET
       << "  Received fatal signal: " << exitReasonName(signal) << "("
       << signal.number << ")\n";
  } else {
...
void LogWorker::save(TimingResult &&msg, std::thread::id id) {
  bg->send([this, m(std::move(msg)), id]() mutable {
    bgRecordTiming(std::move(m), id);
  });
}

void LogWorker::openLogFile(const std::string &filename, bool append) {
  bg->send(
      [this, filename, append]() mutable { bgOpenLogFile(filename, append); });
}
...
/** Write to the global background logger if it exists otherwise write
 * synchronously
 */
...
void LogWorker::recordTiming(const TimingResult &tr,
...
LogCapture::LogCapture(const Level &level, bool in_signal_handler)
    : stream(getStream()), level(level), signal{SIGABRT},
      in_signal_handler(in_signal_handler), send(true) {
