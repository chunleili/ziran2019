...
ErrorContext::ErrorContext(const char *function_name, const char *file,
                           int line_number, const char *var_name)
    : stream(getStream()) {
  std::string ss(file);
  size_t found = ss.find_last_of("/\\");
  stream << ss.substr(found + 1) << ':' << line_number << ' ' << function_name
         << ' ' << var_name << ":\t";
}
