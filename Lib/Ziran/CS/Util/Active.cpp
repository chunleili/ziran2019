...
void Active::send(std::function<void()> &&msg) { mq.push(std::move(msg)); }

unsigned Active::size() const { return mq.size(); }

std::unique_ptr<Active> Active::create(std::function<bool()> &&stop) {
