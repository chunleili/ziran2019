...
/** The Ziran logging design is based on that of g3log.  It has a background
 * thread which handles all the io.  */
...
struct Level {
  ...
  bool operator<(const Level &other) const { return value < other.value; }
  bool operator<=(const Level &other) const { return value <= other.value; }
  bool operator>(const Level &other) const { return value > other.value; }
  bool operator>=(const Level &other) const { return value >= other.value; }
  bool operator==(const Level &other) const { return value == other.value; }
...
const Level Debug{0, "  DEBUG"}, Verbose{50, "VERBOSE"}, Info{100, "   INFO"},
    Timing{200, " TIMING"}, Event{400, "  EVENT"}, Warning{500, "WARNING"},
    Error{1000, "  ERROR"}, Fatal{9000, "  FATAL"};
...
struct TimingResult {
  ...
  std::ostream &print(std::ostream &os, bool print_self = false,
                      int time_width = 8) const;
};
...
public:
  ...
  template <class... Ts> void capture(Signal s, Ts &&... rest) {
    signal = s;
    capture(std::forward<Ts>(rest)...);
  }

  template <class T, class... Ts> void capture(T &&message, Ts &&... rest) {
