...
template <typename T> struct IsEigenMatrix : std::false_type {};

template <class T, int m, int n>
struct IsEigenMatrix<Matrix<T, m, n>> : std::true_type {};

template <typename T> struct IsEigenSparseMatrix : std::false_type {};

template <class T>
struct IsEigenSparseMatrix<Eigen::SparseMatrix<T>> : std::true_type {};

template <typename T> struct IsEigenTriplet : std::false_type {};
...
template <class T, class Enable = void> struct ValueTypeHelper {
...
/**
  ...
  */
template <class T> struct ToVectorMap {
  T *data;
  size_t size;

public:
  ToVectorMap(StdVector<T> &array) : data(array.data()), size(array.size()) {}

  template <int d>
  ToVectorMap(StdVector<Eigen::Matrix<T, d, 1, 0, d, 1>> &array)
      : data(array.data()->data()), size(d * array.size()) {}
...
/**
  Helper struct for converting a std::vector to a eigen matrix with the entries
  as columns with the same backing memory.

  converts to Eigen::Map<Matrix<T, Eigen::Dynamic, d>>
  */
template <class T, int d> struct ToMatrixColMap {
  T *data;
  size_t cols;

public:
  ToMatrixColMap(StdVector<Vector<T, d>> &array)
      : data(array.data()->data()), cols(array.size()) {}
...
/**
  Helper struct for converting a std::vector to a eigen matrix with the entries
  as rows with the same backing memory.

  converts to Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, d, RowMajor>>
  */
template <class T, int d> struct ToMatrixRowMap {
  T *data;
  size_t rows;

public:
  ToMatrixRowMap(StdVector<Vector<T, d>> &array)
      : data(array.data()->data()), rows(array.size()) {}

  operator Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, d, Eigen::RowMajor>>() {
    return Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, d, Eigen::RowMajor>>(
        data, rows, d);
  }
};
...
template <typename T, typename U> constexpr inline bool NonSelf() {
