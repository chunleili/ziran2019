...
public:
  ...
  */
  ...
  Vector<T, d> randInBox(const Eigen::Matrix<T, d, 1, flags, d, 1> &min_corner,
                         const Eigen::Matrix<T, d, 1, flags, d, 1> &max_corner);

  /**
     Random barycentric weights
  */
  template <int d> Vector<T, d> randomBarycentricWeights();

  /**
    Random vector in ball
    */
  template <int d, int flags>
  Vector<T, d> randInBall(const Eigen::Matrix<T, d, 1, flags, d, 1> &center,
                          T radius);
  ...
  */
  template <class T2> void fill(T2 &x, T a = 0, T b = 1);

  /**
    Fill with random integers
  */
  template <class T2> void fillInt(T2 &x, int a = 0, int b = 1);
};
} // namespace ZIRAN
#endif
