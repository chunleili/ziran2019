...
template <typename T> inline int parse(int argc, char **argv, T &value) {
  if (argc < 2)
...
public:
  ...
  */
  ...
  Flag(const char *name, const char *help, Callback &&callback)
      : name(name), help(help), callback(std::move(callback)) {}
  ...
  template <class T>
  Flag(const char *name, const char *help, T &value) : name(name), help(help) {
    callback = [&value](int argc, char **argv) -> int {
      return parse(argc, argv, value);
    };
  }
...
public:
  ...
  void add(const Flag &flag) {
    bool inserted =
        flag_map.emplace(flag.name, std::make_shared<Flag>(flag)).second;
    assert(inserted);
    (void)inserted;
  }
  ...
  void add(Flag &&flag) {
    bool inserted =
        flag_map.emplace(flag.name, std::make_shared<Flag>(std::move(flag)))
            .second;
    assert(inserted);
    (void)inserted;
  }
  ...
  template <class Func> void addNonFlagCallback(Func &&c) {
...
inline void PrintUsage(std::ostream &os) { Registry::global().printUsage(os); }

inline void ParseFlags(int argc, char **argv, bool skip_unknown = false) {
  Registry::global().parseFlags(argc, argv);
}

template <class Func> inline void addNonFlagCallback(Func &&c) {
  Registry::global().addNonFlagCallback(std::forward<Func>(c));
}

template <class T> inline void addMandatoryArgument(T &value) {
  bool set = false;
  addNonFlagCallback([&value, set](int argc, char **argv) mutable -> int {
    if (set)
      return 0;
    set = true;
...
public:
  /// Perfect forwarding
  template <typename... Args> Register(Args &&... args) noexcept {
