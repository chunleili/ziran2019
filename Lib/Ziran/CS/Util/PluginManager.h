...
class ZIRAN_EXPORT PluginManager {
  using TypeMap =
      std::unordered_multimap<std::type_index, std::unique_ptr<FactoryBase>>;
...
public:
  ...
  int numPlugins() const { return plugins.size(); }
  ...
  template <class Interface> class FactoryIter {
    TypeMapIter current;

  public:
    FactoryIter(const TypeMapIter &t) : current(t) {}
    ...
    template <class... Args> operator AFactory<Interface, Args...> *() {
      return dynamic_cast<AFactory<Interface, Args...> *>(
          current->second.get());
    }
  ...
  template <class Interface> class FactoryRange {
    std::pair<TypeMapIter, TypeMapIter> p;

  public:
    FactoryRange(const std::pair<TypeMapIter, TypeMapIter> &p) : p(p) {}

    FactoryIter<Interface> begin() const { return p.first; }

    FactoryIter<Interface> end() const { return p.second; }
  };
  ...
  template <class Interface> FactoryRange<Interface> getAll() const {
