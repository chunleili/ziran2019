...
std::string stripColors(const std::string &s) {
  static const std::regex r(
      "[\033\007][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]");
...
std::ostream &operator<<(std::ostream &os, const SetColor &p) { return p(os); }
...
void showCursor(bool visible) { setCursorVisiblity(true); }

void hideCursor(bool visible) { setCursorVisiblity(false); }
...
std::string moveCursorRelative(int lines_down, int columns_right) {
  return "\033[" + std::to_string(abs(lines_down)) +
         (lines_down < 0 ? 'A' : 'B') + "\033[" +
         std::to_string(abs(columns_right)) + (columns_right < 0 ? 'D' : 'C');
}

std::string clearScreen() { return std::string("\033[2J\033[3J\033[H"); }
...
std::ostream &ProgressBar::operator()(std::ostream &os) const {
  ...
  os << Color::RESET;
  os << "] ";
  os << std::setprecision(3) << std::setw(6) << std::fixed << progress * 100.0
     << " " << tail;
  return os;
}
...
PrettyPrintDuration::PrettyPrintDuration(
