...
MpmParticleHandleBase<T, dim>::MpmParticleHandleBase(
    Particles<T, dim> &particles, Scene<T, dim> &scene,
    MpmForceBase<T, dim> *mpmforce,
    StdVector<std::unique_ptr<PlasticityApplierBase>> &plasticity_appliers,
    StdVector<TV> &scratch_xp, T &dt, Range particle_range, T total_volume,
    int cotangent_manifold_dim)
    : particles(particles), scene(scene), mpmforce(mpmforce),
      plasticity_appliers(plasticity_appliers), scratch_xp(scratch_xp), dt(dt),
      particle_range(particle_range), total_volume(total_volume),
      cotangent_manifold_dim(cotangent_manifold_dim) {}

// Creates a copy with new particles
template <class T, int dim>
MpmParticleHandleBase<T, dim> MpmParticleHandleBase<T, dim>::copy() {
  ...
  return MpmParticleHandleBase(particles, scene, mpmforce, plasticity_appliers,
                               scratch_xp, dt, new_particle_range,
                               total_volume);
}

template <class T, int dim>
void MpmParticleHandleBase<T, dim>::transform(
    const std::function<void(int, Ref<T>, Vector<T, dim> &, Vector<T, dim> &)>
        &mapping) {
  for (int i = particle_range.lower; i < particle_range.upper; ++i) {
    // lua does not support passing scalars by reference. This is a work around
    // to actually change mass.
    mapping(i - particle_range.lower, particles.mass[i], particles.X[i],
            particles.V[i]);
  }
}

template <class T, int dim>
void MpmParticleHandleBase<T, dim>::addVolumeFraction(const T b) {
  particles.add(volume_fraction_name<T>(), particle_range, b);
}

template <class T, int dim>
template <class TCONST>
void MpmParticleHandleBase<T, dim>::addFBasedMpmForce(const TCONST &model) {
...
template <class T, int dim>
template <class TCONST>
void MpmParticleHandleBase<T, dim>::addFBasedMpmForceWithPhaseField(
    const T &percentage, const T &l0, const TCONST &model, bool allow_damage,
    const T random_fluctuation_percentage) {
  ZIRAN_ASSERT(cotangent_manifold_dim == 0);
  FBasedMpmForceHelper<TCONST> &helper =
      mpmforce->getHelper(); // this will let mpmforce create a consitutive
                             // model helper
  particles.add(helper.constitutive_model_name(), particle_range, model);
  if (total_volume != 0) {
    particles.add(element_measure_name<T>(), particle_range,
                  total_volume / particle_range.length());
    particles.add(helper.F_name(), particle_range, TM::Identity());
  }
  particles.add(helper.constitutive_model_scratch_name(), particle_range,
                typename FBasedMpmForceHelper<TCONST>::Scratch());
  PhaseField<T, dim> pf;
  ...
  pf.l0 = l0;
  pf.one_over_sigma_c =
      PhaseField<T, dim>::Get_Sigma_C_Based_On_Max_Deformation(percentage,
                                                               model);
  pf.pf_Fp = (T)1;
  ...
  if (random_fluctuation_percentage)
    randomizePhaseFieldSigmaC(random_fluctuation_percentage);
}
...
template <class T, int dim>
void MpmParticleHandleBase<T, dim>::randomizePhaseFieldSigmaC(
    const T random_fluctuation_percentage) {
  RandomNumber<T> rn(123);
  for (auto iter = particles.subsetIter(DisjointRanges{particle_range},
                                        phase_field_name<PhaseField<T, dim>>());
       iter; ++iter) {
    auto &pf = iter.template get<0>();
    T scale = 1 + rn.randReal(-random_fluctuation_percentage,
                              random_fluctuation_percentage);
    pf.one_over_sigma_c *= scale;
  }
}

// Read in a file of voronoi points and use them to initialize sigmaC to be
// scaled based on point distance from voronoi surfaces
template <class T, int dim>
void MpmParticleHandleBase<T, dim>::voronoiSigmaC(std::string voronoiPointsFile,
                                                  T radius)

// void MpmParticleHandleBase<T, dim>::voronoiSigmaC(std::string
// voronoiPointsFile, T radius, T minPercent, T maxPercent)
{
  ...
  // Now we can query this voronoi surface point KDTree for distances from our
  // actual points!
  int i = 0;
  ...
  for (auto iter = particles.subsetIter(DisjointRanges{particle_range},
    ...
    TV p;
    T dist;
    voronoiPoints.findNearest(
        particles.X[i], id, p,
        dist); // fill in "dist" with distance from nearest point

    // 4th Way: use base to reverse engineer what value of zeta we need to fit
    // the values to the curve y = e^(zeta * x) - 1
    ...
    // 3rd way: use 1 - log(dist/radius) and clamp all scales past
    // (dist/radius)=1 to be 1 NOTE: this relies on scaling SPECIFICALLY one
    // over sigma C!!!!!
    /*T x = dist / radius;
    T scale = 1;
    if (x < 1) {
        scale = (1 - std::log(x)) * magnitude; //scale by magnitude to allow for
    greater disparity between large and small sigmaC
    }
    ...
    */
    ...
    /*T proportion = (std::exp((dist / radius)) - 1) * (maxPercent -
    minPercent); //proportion through the interval min% to max% we are based on
    dist --> but using tanh instead of linear T scale = 1 + (minPercent +
    proportion);
    ...
    // pf.one_over_sigma_c = (T)1 / newSigmaC;
    // pf.one_over_sigma_c *= scale; //for if we want to directly scale one over
    // sigmaC
...
template <class T, int dim>
void MpmParticleHandleBase<T, dim>::scaleF(const T scale) {
  for (auto iter = particles.subsetIter(DisjointRanges{particle_range},
                                        F_name<T, dim>());
       iter; ++iter) {
...
template <class T, int dim>
void MpmParticleHandleBase<T, dim>::scaleF(const TV scale) {
  for (auto iter = particles.subsetIter(DisjointRanges{particle_range},
                                        F_name<T, dim>());
       iter; ++iter) {
    auto &F = iter.template get<0>();
...
template <class T, int dim>
void MpmParticleHandleBase<T, dim>::scaleJ(const T scale) {
  for (auto iter =
           particles.subsetIter(DisjointRanges{particle_range}, J_name<T>());
       iter; ++iter) {
...
template <class T, int dim>
void MpmParticleHandleBase<T, dim>::scaleFCurve(
    int frame, const std::function<T(int)> &growCurve) {
  DisjointRanges subset{particle_range};
  for (auto iter = particles.subsetIter(subset, F_name<T, dim>()); iter;
       ++iter) {
    auto &F = iter.template get<0>();
    if (frame == 0)
      F /= growCurve(0);
...
template <class T, int dim>
void MpmParticleHandleBase<T, dim>::resetDeformation() {
  for (auto iter = particles.subsetIter(DisjointRanges{particle_range},
                                        F_name<T, dim>());
       iter; ++iter) {
    auto &F = iter.template get<0>();
...
template <class T, int dim>
void MpmParticleHandleBase<T, dim>::addElementMeasure() {
  if (total_volume != 0) {
    particles.add(element_measure_name<T>(), particle_range,
                  total_volume / particle_range.length());
  }
...
void MpmParticleHandleBase<T, dim>::addPlasticity(const TConst &cons,
                                                  const TPlastic &plasticity,
  ...
  for (auto &p : plasticity_appliers) {
    plasticity_model =
        dynamic_cast<PlasticityApplier<TConst, TPlastic, TStrain> *>(p.get());
  ...
  if (plasticity_model == nullptr)
    plasticity_appliers.push_back(
        std::make_unique<PlasticityApplier<TConst, TPlastic, TStrain>>(
            strain_name));
  particles.add(TPlastic::name(), particle_range, plasticity);
}

template <class T, int dim>
void MpmParticleHandleBase<T, dim>::addDummyPlasticity() {
  auto p = DummyPlasticity<T>();
  particles.add(DummyPlasticity<T>::name(), particle_range, p);
}

template <class T, int dim>
void MpmParticleHandleBase<T, dim>::prescorePhaseFieldSigmaC(
    AnalyticLevelSet<T, dim> &levelset, T grain_size, T scale_lower_clamp) {
  ZIRAN_INFO("Prescoring phase field sigma_c");
  TV min_corner, max_corner;
  levelset.getBounds(min_corner, max_corner);
  PoissonDisk<T, dim> pd(/*random seed*/ 123, grain_size, min_corner,
                         max_corner);
  StdVector<TV> grain_centroids;
  if (dim == 3)
    pd.sampleFromPeriodicData(grain_centroids,
  ...
  RandomNumber<T> rand;
  for (auto iter =
           particles.subsetIter({particle_range}, Particles<T, dim>::X_name(),
                                phase_field_name<PhaseField<T, dim>>());
       iter; ++iter) {
    ...
    T scale = 1 - std::min(std::sqrt(min_distance2) *
                               rand.randReal(0.75, 1.25) / (2 * grain_size),
                           (T)1);
...
template <class T, int dim>
void MpmParticleHandleBase<T, dim>::prescoreWetSand2D(
    AnalyticLevelSet<T, dim> &levelset, T grain_size, T density_min, T Jp_min,
    T scale_min) {
  ...
  auto dpshn = AttributeName<DruckerPragerStvkHencky<T>>(
      DruckerPragerStvkHencky<T>::name());
  auto stvkhi = AttributeName<StvkWithHenckyIsotropic<T, dim>>(
      StvkWithHenckyIsotropic<T, dim>::name());
  T phi = T(0.5) * (1 - std::sqrt((T)5));
  RandomNumber<T> rand;

  for (auto iter =
           particles.subsetIter({particle_range}, Particles<T, dim>::X_name(),
                                Particles<T, dim>::mass_name(), dpshn,
                                element_measure_name<T>(), stvkhi);
       iter; ++iter) {
    TV &x = iter.template get<0>();
    for (const TV &c : grain_centroids) {
      TV v = c - x;
    ...
    if (x(0) > 0.43 && x(1) > 1.05 && (x(2) > -0.2 && x(2) < -0.08) &&
        rand.randReal(0, 1) < 0.5) {
      scale_scale = (T)0.1;
      scale_scale_min = 0.1;
    } else if (x(0) > 0.43 && (x(1) > 0.8 && x(1) < 1.05) &&
               (x(2) > 0.1 && x(2) < 0.26) && rand.randReal(0, 1) < 0.5) {
      scale_scale = (T)0.14;
      scale_scale_min = 0.1;
    } else if (x(0) > 0.43 && (x(1) > 0.75 && x(1) < 0.905) &&
               (x(2) > -0.26 && x(2) < -0.13) && rand.randReal(0, 1) < 0.7) {
      scale_scale = (T)0.24;
      scale_scale_min = 0.1;
    }
    ...
    cm_elastic.lambda *=
        std::max(scale * scale_scale, scale_min * scale_scale_min);
...
template <class T, int dim>
template <class TCONST>
void MpmParticleHandleBase<T, dim>::addFBasedMpmForceWithMeasure(
    const TCONST &model, const Range &range, T total_volume) {
  FBasedMpmForceHelper<TCONST> &helper =
      mpmforce->getHelper(); // this will let mpmforce create a consitutive
                             // model helper
  particles.add(helper.constitutive_model_name(), range, model);
  if (total_volume != 0) {
    particles.add(element_measure_name<T>(), range,
                  total_volume / range.length());
    particles.add(helper.F_name(), range, TM::Identity());
  }
  particles.add(helper.constitutive_model_scratch_name(), range,
                typename FBasedMpmForceHelper<TCONST>::Scratch());
}

template <class T, int dim>
template <class TCONST>
void MpmParticleHandleBase<T, dim>::addFElasticNonequilibratedBasedMpmForce(
    const TCONST &model, T viscosity_d_input, T viscosity_v_input) {
  FElasticNonequilibratedBasedMpmForceHelper<TCONST> &helper =
      mpmforce->getHelper(); // this will let mpmforce create a consitutive
                             // model helper
  particles.add(helper.constitutive_model_name(), particle_range, model);
  particles.add(helper.F_name(), particle_range, TM::Identity());
  particles.add(
      helper.constitutive_model_scratch_name(), particle_range,
      typename FElasticNonequilibratedBasedMpmForceHelper<TCONST>::Scratch());
...
void MpmParticleHandleBase<T, dim>::setMassFromDensity(const T density) {
  ZIRAN_INFO("Setting mass from densiy: total_volume = ", total_volume,
             ", particle.count = ", particle_range.length());
