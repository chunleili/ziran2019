...
template <class T, int dim> class GridState {
public:
...
static_assert(is_power_of_two(sizeof(GridState<float, 2>)),
              "GridState<float, 2> size must be POT");
static_assert(is_power_of_two(sizeof(GridState<float, 3>)),
              "GridState<float, 3> size must be POT");
static_assert(is_power_of_two(sizeof(GridState<double, 2>)),
              "GridState<double, 2> size must be POT");
static_assert(is_power_of_two(sizeof(GridState<double, 3>)),
              "GridState<double, 3> size must be POT");
...
template <class T, int dim> class BSplineWeights {
  const T dx;
...
template <class T, int dim> class MpmGrid {
public:
  static constexpr int log2_page = 12;
  static constexpr int spgrid_size = 4096;
  static constexpr int interpolation_degree = ZIRAN_MPM_DEGREE;
  static constexpr int kernel_size =
      (dim == 2) ? (interpolation_degree + 1) * (interpolation_degree + 1)
  ...
  int getNumNodes() {
    ...
    for (int b = 0; b < (int)blocks.second; ++b) {
      GridState<T, dim> *g =
          reinterpret_cast<GridState<T, dim> *>(&grid_array(blocks.first[b]));
  ...
  template <typename OP> void iterateTouchedGrid(const OP &target) {
    auto blocks = page_map->Get_Blocks();
    auto grid_array = grid->Get_Array();
    if constexpr (dim == 2) {
      tbb::parallel_for(0, (int)blocks.second, [&](int b) {
        ...
        Vector<int, dim> base_coord{stdarray_base_coord[0],
                                    stdarray_base_coord[1]};
        auto x = 1 << SparseMask::block_xbits;
        auto y = 1 << SparseMask::block_ybits;
        for (int i = 0; i < x; ++i)
          for (int j = 0; j < y; ++j) {
            auto offset = SparseMask::Packed_Add(
                base_offset, SparseMask::Linear_Offset(i, j));
            GridState<T, dim> &g =
                reinterpret_cast<GridState<T, dim> &>(grid_array(offset));
      ...
      tbb::parallel_for(0, (int)blocks.second, [&](int b) {
        ...
        for (int i = 0; i < x; ++i)
          for (int j = 0; j < y; ++j)
            for (int k = 0; k < z; ++k) {
              auto offset = SparseMask::Packed_Add(
                  base_offset, SparseMask::Linear_Offset(i, j, k));
              GridState<T, dim> &g =
                  reinterpret_cast<GridState<T, dim> &>(grid_array(offset));
  ...
  template <typename OP> void iterateGrid(const OP &target) {
    auto blocks = page_map->Get_Blocks();
    auto grid_array = grid->Get_Array();
    if constexpr (dim == 2) {
      tbb::parallel_for(0, (int)blocks.second, [&](int b) {
  ...
  inline void iterateKernel(const BSplineWeights<T, dim> &spline,
                            uint64_t base_offset, const OP &target) {
    auto grid_array = grid->Get_Array();
    ...
    if constexpr (dim == 2) {
      for (int i = 0; i < interpolation_degree + 1; ++i) {
        ...
        for (int j = 0; j < interpolation_degree + 1; ++j) {
          ...
          auto offset = SparseMask::Packed_Add(base_offset,
                                               SparseMask::Linear_Offset(i, j));
          GridState<T, dim> &g =
              reinterpret_cast<GridState<T, dim> &>(grid_array(offset));
  ...
  template <typename OP>
  inline void superIterateKernel(const BSplineWeights<T, dim> &spline,
                                 const OP &target) {
    auto grid_array = grid->Get_Array();
    T one_over_dx = spline.one_over_dx;
    auto &w = spline.w;
    ...
    const Vector<int, dim> &base_coord = spline.base_node;
    __m128 __base_coord =
        _mm_set_ps((float)0, (float)base_coord[2], (float)base_coord[1],
                   (float)base_coord[0]);
    int cnt = 0;
    for (int i = 0; i < interpolation_degree + 1; ++i) {
      T wi = w[0](i);
...
template <class T, int dim> class CachedOffset {
  ...
  static constexpr int interpolation_degree =
      MpmGrid<T, dim>::interpolation_degree;
  ...
  CachedOffset(uint64_t particle_offset) {
    ...
    for (int delta = 0; delta < 1 << SparseMask::block_bits; ++delta) {
      ...
      for (int i = 0; i < interpolation_degree + 1; ++i)
        for (int j = 0; j < interpolation_degree + 1; ++j)
          for (int k = 0; k < interpolation_degree + 1; ++k)
            data[delta][cnt++] = SparseMask::Packed_Add(
                offset, SparseMask::Linear_Offset(i, j, k));
    }
  }
  inline uint64_t *getCachedLine(uint64_t particle_offset) {
    return data[(particle_offset & 0x0000000000000ffful) >>
