...
template <class T, int dim> class Scene;
template <class T, int dim> class MpmSimulation;
template <class T, int dim> class AnalyticLevelSet;
template <class T, int dim> class Particles;
template <class T, int dim> class Scene;

template <class T, int dim> class MpmParticleHandleBase {
public:
  static constexpr int interpolation_degree = ZIRAN_MPM_DEGREE;
  static const int splat_size =
      MATH_TOOLS::power(interpolation_degree + 1, dim);
  ...
  MpmParticleHandleBase(
      Particles<T, dim> &particles, Scene<T, dim> &scene,
      MpmForceBase<T, dim> *mpmforce,
      StdVector<std::unique_ptr<PlasticityApplierBase>> &plasticity_appliers,
      StdVector<TV> &scratch_xp, T &dt, Range particle_range, T total_volume,
      int cotangent_manifold_dim = 0);

  // Creates a copy with new particles
  MpmParticleHandleBase copy();

  void transform(const std::function<void(int, Ref<T>, Vector<T, dim> &,
                                          Vector<T, dim> &)> &mapping);

  void addVolumeFraction(const T b);

  template <class TCONST> void addFBasedMpmForce(const TCONST &model);

  template <class TCONST>
  void
  addFBasedMpmForceWithPhaseField(const T &percentage, const T &l0,
                                  const TCONST &model, bool allow_damage = true,
                                  const T random_fluctuation_percentage = 0);

  void addOriginalPositionAsAttribute();

  void randomizePhaseFieldSigmaC(const T random_fluctuation_percentage);

  // void voronoiSigmaC(std::string voronoiPointsFile, T radius, T minPercent, T
  // maxPercent);
  ...
  void prescoreWetSand2D(AnalyticLevelSet<T, dim> &levelset, T grain_size,
                         T density_min, T Jp_min, T scale_min = (T)0.5);

  void prescorePhaseFieldSigmaC(AnalyticLevelSet<T, dim> &levelset,
                                T grain_size, T scale_lower_clamp = 0);
  ...
  void addPlasticity(const TConst &cons, const TPlastic &plasticity,
                     std::string strain_name = "F");

  void addDummyPlasticity();

  template <class TCONST>
  void addFBasedMpmForceWithMeasure(const TCONST &model, const Range &range,
                                    T total_volume);

  template <class TCONST>
  void addFElasticNonequilibratedBasedMpmForce(const TCONST &model,
                                               T viscosity_d_input,
                                               T viscosity_v_input);
