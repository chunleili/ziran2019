...
template <class T, int dim> class MpmSimulationDataAnalysis {
...
public:
  ...
  TV evalTotalLinearMomentumParticles() // summing mass*velocity over particles
  {
    ...
    DoubleV p = mpm.particles.map_reduce(
        zero,
        [](T m, const TV &v) { return double(m) * v.template cast<double>(); },
        [](const DoubleV &a, const DoubleV &b) { return a + b; },
        mpm.particles.mass_name(), mpm.particles.V_name());
    return p.template cast<T>();
  }

  /** Evaluate total angular momentum from particles */
  AngularVelocity<T, dim>
  evalTotalAngularMomentumParticles() // summing mass*(velocity `cross_product`
                                      // position) over particles
  {
    ...
    if (mpm.transfer_scheme == MpmSimulationBase<T, dim>::APIC_blend_RPIC) {
      ...
      result = mpm.particles.map_reduce(
          init,
          [D](const T &mass, const TV &position, const TV &velocity,
              const TM &C) {
            DoubleM tempDoubleM =
                (D * C.transpose().template cast<double>()) +
                (position.template cast<double>() *
                 velocity.transpose().template cast<double>());
            return LeviCivitaContract(tempDoubleM) * double(mass);
          },
          [](const AngularVelocity<double, dim> &a,
             const AngularVelocity<double, dim> &b) { return a + b; },
          mpm.particles.mass_name(), mpm.particles.X_name(),
          mpm.particles.V_name(), C_name<TM>());
    } else {
      result = mpm.particles.map_reduce(
          init,
          [](const T &mass, const TV &position, const TV &velocity) {
            DoubleM tempDoubleM = position.template cast<double>() *
                                  velocity.transpose().template cast<double>();
            return LeviCivitaContract(tempDoubleM) * double(mass);
          },
          [](const AngularVelocity<double, dim> &a,
  ...
  T evalTotalKineticEnergyParticles() // 0.5*(summing
                                      // mass*velocity.squaredNorm() over
                                      // particles)
  {
    ...
    return (T)(0.5 *
               tbb::parallel_reduce(
                   ranges, (double)0,
                   [&](const DisjointRanges &subset, double init) -> double {
                     for (auto iter = mpm.particles.subsetIter(
                              subset, mpm.particles.mass_name(),
                              mpm.particles.V_name());
                          iter; ++iter) {
                       auto &mass = iter.template get<0>();
                       auto &velocity =
                           iter.template get<1>().template cast<double>();
  ...
  AngularVelocity<T, dim> evalTotalAngularMomentumGrid(bool new_v = false) {
    ...
    mpm.grid.iterateGrid([&](Vector<int, dim> node, GridState<T, dim> &g) {
      ...
      AngularVelocity<double, dim> momentum =
          LeviCivitaContract(tempDoubleM) * m;
      mtx.lock();
      result += momentum;
      mtx.unlock();
    });
  ...
  AngularVelocity<T, dim>
  evalIncompressibleTotalAngularMomentumGrid(bool new_v = false) {
    std::mutex mtx;
    ...
    mpm.grid.iterateGrid([&](Vector<int, dim> node, GridState<T, dim> &g) {
      ...
      if (new_v)
        ...
        v = g.v.template cast<double>();
      AngularVelocity<double, dim> momentum =
          LeviCivitaContract(x * v.transpose()) * 1; // assume constant mass
      mtx.lock();
      result += momentum;
      mtx.unlock();
    });
  ...
  T evalMaxMassParticles() {
    return mpm.particles.map_reduce(
        (T)0, [](T m) -> T { return m; },
        [](const T &a, const T &b) -> T { return std::max(a, b); },
        mpm.particles.mass_name());
  }

  T evalMinMassParticles() {
    return mpm.particles.map_reduce(
        (T)FLT_MAX, [](T x) -> T { return x; },
        [](const T &a, const T &b) -> T { return std::min(a, b); },
        mpm.particles.mass_name());
  }
};
} // namespace ZIRAN
