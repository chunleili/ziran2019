...
template <class T, int dim> class MpmForceHelperBase;

template <class TCONST>
class FBasedMpmForceHelper
    : public MpmForceHelperBase<typename TCONST::Scalar,
                                TCONST::TM::RowsAtCompileTime> {
public:
  ...
  void updateState(const DisjointRanges &subrange, StdVector<TM> &vtau,
                   TVStack &fp) override;

  // add stress to vPFnT (called only by implicit)
  void updateImplicitState(const DisjointRanges &subrange, StdVector<TM> &vPFnT,
                           TVStack &fp) override;

  void evolveStrain(const DisjointRanges &subrange, T dt,
                    const StdVector<TM> &gradV) override;

  double totalEnergy(const DisjointRanges &subrange) override;

  void computeStressDifferential(const DisjointRanges &subrange,
                                 const StdVector<TM> &gradDv,
                                 StdVector<TM> &dstress, const TVStack &dvp,
                                 TVStack &dfp) override;

  template <class TCCONST, class TPCONST>
  void computeStressDifferentialWithPlasticity(const DisjointRanges &subrange,
                                               const StdVector<TM> &gradDv,
                                               StdVector<TM> &dstress) {
    ...
    DisjointRanges subset(subrange,
                          particles.commonRanges(c_name, element_measure_name(),
                                                 F_name(), p_name));
    if (subset.size() == 0)
      return;
    for (auto iter = particles.subsetIter(subset, c_name, F_name(),
                                          element_measure_name(),
                                          valueIdOnly(F_name()), p_name);
         iter; ++iter) {
      ...
      TM test_matrix =
          plasticity_model.projectSigmaDerivative(constitutive_model, sigma);
      if (test_matrix == TM::Zero()) {
        ...
        if constexpr (dim == 2) {
          ...
          Vector<T, 4> T3_flatten =
              (M_hat * Vector<T, 4>(T2.data())).transpose();
        ...
        TM A =
            constitutive_model.firstPiolaDerivativeDiagonal(zi) *
            plasticity_model.projectSigmaDerivative(constitutive_model, sigma);
        if constexpr (dim == 2) {
          T clamp_value = 1e-8;
  ...
  void
  computeStressDifferentialWithPlasticity(const DisjointRanges &subrange,
                                          const StdVector<TM> &gradDv,
                                          StdVector<TM> &dstress) override {
    computeStressDifferentialWithPlasticity<StvkWithHencky<T, dim>,
                                            DruckerPragerStvkHencky<T>>(
        subrange, gradDv, dstress);
  }
  ...
  inline virtual AttributeName<TM> F_name() { return AttributeName<TM>("F"); }
};
} // namespace ZIRAN
