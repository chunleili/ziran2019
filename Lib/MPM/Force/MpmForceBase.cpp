...
MpmForceBase<T, dim>::MpmForceBase(
    const bool &mls_mpm, const T &dx, const T &dt, Particles<T, dim> &particles,
    Scene<T, dim> &scene, StdVector<TV> &scratch_xp, TVStack &scratch_vp,
    TVStack &scratch_fp, StdVector<TM> &scratch_gradV,
    StdVector<TM> &scratch_stress, const TVStack &dv, const TVStack &vn,
    MpmGrid<T, dim> &grid, StdVector<uint64_t> &particle_base_offset,
    StdVector<int> &particle_order,
    std::vector<std::pair<int, int>> &particle_group,
    std::vector<uint64_t> &block_offset, int &num_nodes,
    StdVector<std::unique_ptr<PlasticityApplierBase>> &plasticity_appliers,
    bool &full_implicit)

    : mls_mpm(mls_mpm), D_inverse(0), dx(dx), dt(dt), dv(dv), vn(vn),
      particles(particles), scene(scene), scratch_xp(scratch_xp),
      scratch_vp(scratch_vp), scratch_fp(scratch_fp),
      scratch_gradV(scratch_gradV), scratch_stress(scratch_stress), grid(grid),
      particle_base_offset(particle_base_offset),
      particle_order(particle_order), particle_group(particle_group),
      block_offset(block_offset), num_nodes(num_nodes),
      plasticity_appliers(plasticity_appliers), full_implicit(full_implicit) {}

template <class T, int dim> MpmForceBase<T, dim>::~MpmForceBase() {}

template <class T, int dim> void MpmForceBase<T, dim>::reinitialize() {
...
template <class T, int dim> void MpmForceBase<T, dim>::backupStrain() {
  ZIRAN_QUIET_TIMER();
  for (auto &h : helpers)
    h->backupStrain();
}

template <class T, int dim> void MpmForceBase<T, dim>::restoreStrain() {
  ZIRAN_QUIET_TIMER();
  for (auto &h : helpers)
    h->restoreStrain();
}

template <class T, int dim> void MpmForceBase<T, dim>::computeVAndGradV() {
  ZIRAN_QUIET_TIMER();
  evalInterpolantAndGradient(
      [&](int node_id) -> TV { return vn.col(node_id) + dv.col(node_id); },
      scratch_vp, scratch_gradV);
}

template <class T, int dim>
void MpmForceBase<T, dim>::computeDvAndGradDv(const TVStack &dv) const {
  ZIRAN_QUIET_TIMER();
  evalInterpolantAndGradient([&](int node_id) -> TV { return dv.col(node_id); },
                             scratch_vp, scratch_gradV);
}

template <class T, int dim>
template <bool USE_MLS_MPM>
void MpmForceBase<T, dim>::rasterizeForceToTVStack(const T scale,
                                                   TVStack &force) const {
  ZIRAN_QUIET_TIMER();
        ...
        grid.iterateKernel(
            ...
            [&](const IV &node, T w, const TV &dw, GridState<T, dim> &g) {
              ...
              if constexpr (USE_MLS_MPM) {
                ...
                g.new_v -= scale * delta.template topLeftCorner<
                                       dim, 1>(); // fi -= \sum_p (Ap (xi-xp)  -
                                                  // fp )w_ip Dp_inv
              } else {
  ...
  grid.iterateGrid(
      [&](IV node, GridState<T, dim> &g) { force.col(g.idx) += g.new_v; });
}

template <class T, int dim> void MpmForceBase<T, dim>::updateParticleState() {
  ZIRAN_QUIET_TIMER();
...
/**
      Compute for a vector function f defined on active grid nodes, compute f
   and grad f on particles
      */
template <class T, int dim>
template <class Func>
void MpmForceBase<T, dim>::evalInterpolantAndGradient(
    Func &&f, TVStack &f_eval, StdVector<TM> &grad_f) const {
  grid.iterateTouchedGrid(
      [&](IV node, GridState<T, dim> &g) { g.new_v = TV::Zero(); });
  grid.iterateGrid(
      [&](IV node, GridState<T, dim> &g) { g.new_v = f((int)g.idx); });
  for (uint64_t color = 0; color < (1 << dim); ++color) {
    tbb::parallel_for(0, (int)particle_group.size(), [&](int group_idx) {
      if ((block_offset[group_idx] & ((1 << dim) - 1)) != color)
        return;
...
template <class T, int dim>
void MpmForceBase<T, dim>::addScaledForces(const T scale,
                                           TVStack &forces) const {
  ZIRAN_QUIET_TIMER();
  if (mls_mpm) {
...
template <class T, int dim>
void MpmForceBase<T, dim>::addScaledForceDifferential(const T scale,
                                                      const TVStack &dv,
                                                      TVStack &df) const {
  ZIRAN_QUIET_TIMER();
  ...
  // scratch_gradV is now temporaraly used for storing gradDV (evaluated at
  // particles) scratch_vp is now temporaraly used for storing DV (evaluated at
  // particles)
  ...
  tbb::parallel_for(ranges, [&](DisjointRanges &subrange) {
    for (auto &h : helpers) {
      if (full_implicit) {
        h->computeStressDifferentialWithPlasticity(subrange, scratch_gradV,
                                                   scratch_stress);
      } else {
        h->computeStressDifferential(subrange, scratch_gradV, scratch_stress,
                                     scratch_vp, scratch_fp);
      }
    }
...
void MpmForceBase<T, dim>::updatePositionBasedState() {
  ...
  // xp = xn + dt * vp
  tbb::parallel_for(
      tbb::blocked_range<size_t>(0, particles.count),
      [&](const tbb::blocked_range<size_t> &range) {
...
void MpmForceBase<T, dim>::updatePositionBasedState(const StdVector<TV> &x) {
  ZIRAN_ASSERT(false, "UpdatePositionBasedState(const StdVector<TV>& x) is not "
                      "supported by mpm");
}

template <class T, int dim> void MpmForceBase<T, dim>::evolveStrain(T dt) {
...
template <class T, int dim> T MpmForceBase<T, dim>::totalEnergy() const {
  ZIRAN_QUIET_TIMER();
  auto ranges = particles.X.ranges;
