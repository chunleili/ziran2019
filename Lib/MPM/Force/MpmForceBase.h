...
public:
  ...
  MpmForceBase(
      const bool &mls_mpm, const T &dx, const T &dt,
      Particles<T, dim> &particles, Scene<T, dim> &scene,
      StdVector<TV> &scratch_xp, TVStack &scratch_vp, TVStack &scratch_fp,
      StdVector<TM> &scratch_gradV, StdVector<TM> &scratch_stress,
      const TVStack &dv, const TVStack &vn, MpmGrid<T, dim> &grid,
      StdVector<uint64_t> &particle_base_offset, StdVector<int> &particle_order,
      std::vector<std::pair<int, int>> &particle_group,
  ...
  /**
    Compute for a vector function f defined on active grid nodes, compute f and
    grad f on particles
    */

  template <class Func>
  void evalInterpolantAndGradient(Func &&f, TVStack &f_eval,
                                  StdVector<TM> &grad_f) const;

  template <class TCONST, class TPCONST>
  void computeStressDifferential(const StdVector<TM> &gradDv,
                                 StdVector<TM> &dstress) const;

  void addScaledForceDifferential(const T scale, const TVStack &dv,
                                  TVStack &df) const override;
  ...
  struct Proxy {
    ...
    Proxy(StdVector<std::unique_ptr<MpmForceHelperBase<T, dim>>> &helpers,
          Particles<T, dim> &particles)
        : helpers(helpers), particles(particles) {}
    ...
    /**
       ...
       Not threadsafe
    */
    template <class HelperT> operator HelperT &() {
      for (auto &helper : helpers)
        if (HelperT *h = dynamic_cast<HelperT *>(helper.get()))
          return *h;
      // We didn't find it
      helpers.emplace_back(
          std::make_unique<std::remove_const_t<HelperT>>(particles));
      return *dynamic_cast<HelperT *>(helpers.back().get());
    }
  };

  Proxy getHelper() { return Proxy(helpers, particles); }
};
} // namespace ZIRAN
#endif
