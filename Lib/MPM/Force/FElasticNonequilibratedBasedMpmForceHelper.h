...
class FElasticNonequilibratedBasedMpmForceHelper
    : public FBasedMpmForceHelper<TCONST> {
public:
  ...
  explicit FElasticNonequilibratedBasedMpmForceHelper(
  ...
  void updateState(const DisjointRanges &subrange, StdVector<TM> &vtau,
    ...
    DisjointRanges subset(
        ...
        particles.commonRanges(constitutive_model_name(),
                               ...
                               Base::element_measure_name(), F_name()));
    for (auto iter =
             particles.subsetIter(subset, constitutive_model_name(),
                                  constitutive_model_scratch_name(),
                                  Base::element_measure_name(), F_name());
         iter; ++iter) {
  ...
  void evolveStrain(const DisjointRanges &subrange, T dt,
    ...
    for (auto iter =
      ...
      T beta =
          (T)2.0 * ((T)2.0 * c.mu + c.lambda * dim) / ((T)9.0 * viscosity_v) -
          (T)2.0 * c.mu / (viscosity_d * dim);
      ...
      TV epsilon =
          (T)1.0 / ((T)1.0 + dt * alpha) *
          (epsilon_trial - dt * beta / ((T)1.0 + dt * (alpha + dim * beta)) *
