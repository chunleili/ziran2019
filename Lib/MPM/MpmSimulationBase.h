...
template <class T, int _dim> class MpmSimulationBase : public SimulationBase {
public:
  ...
  enum DebugEvents : int { BeforeNewtonSolve, AfterNewtonSolve };

  enum TransferScheme { APIC_blend_RPIC, FLIP_blend_PIC, OTHER };
...
public:
  ...
  virtual void beginTimeStep(int frame, int substep, double time,
                             double dt) override;
  ...
  */
  ...
  virtual void addScaledForces(
      const T scale,
      TVStack &f); // called BE Newton objective (for computing residual)

  virtual void addScaledForceDifferentials(
  ...
  */
  ...
  // Accessors. Mainly used in MpmInitializationBase to pass as arguments for
  ...
  inline StdVector<std::unique_ptr<PlasticityApplierBase>> &
  getPlasticityAppliers() {
    return plasticity_appliers;
  }
