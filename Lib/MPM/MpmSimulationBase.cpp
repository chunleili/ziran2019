...
MpmSimulationBase<T, dim>::MpmSimulationBase()
    : transfer_scheme(APIC_blend_RPIC), apic_rpic_ratio((T)1),
      flip_pic_ratio((T)0), flip_rpic_ratio((T)0), print_stats(false),
      write_partio(true), write_meshes(true), autorestart(true),
      gravity(TV::Unit(1) * (0)), cfl((T).6), dx(1), dt((T)1.0 / 24),
      element_partitions(0), particles(scene.particles),
      element_measure(particles.add(element_measure_name<T>())),
      objective(*this), newton(objective, (T)1e-5, 3),
      force((forces.emplace_back(std::make_unique<MpmForceBase<T, dim>>(
                 mls_mpm, dx, dt, particles, scene, scratch_xp, scratch_vp,
                 scratch_fp, scratch_gradV, scratch_stress, dv, vn, grid,
                 particle_base_offset, particle_order, particle_group,
                 block_offset, num_nodes, plasticity_appliers, full_implicit)),
             forces.back())) {
  symplectic = true;
  quasistatic = false;
  openvdb::initialize();
...
template <class T, int dim> MpmSimulationBase<T, dim>::~MpmSimulationBase() {}
...
template <class T, int dim>
void MpmSimulationBase<T, dim>::addCollisionObject(
    AnalyticCollisionObject<T, dim> &&object) {
  collision_objects.emplace_back(
      std::make_unique<AnalyticCollisionObject<T, dim>>(std::move(object)));
}

template <class T, int dim>
int MpmSimulationBase<T, dim>::addSourceCollisionObject(
    SourceCollisionObject<T, dim> &&object) {
  collision_objects.emplace_back(
      std::make_unique<SourceCollisionObject<T, dim>>(std::move(object)));
  return collision_objects.size() - 1;
}

template <class T, int dim> void MpmSimulationBase<T, dim>::initialize() {
  ZIRAN_TIMER();
  Base::initialize();
  ...
  if (mls_mpm) {
    ZIRAN_ASSERT(transfer_scheme == APIC_blend_RPIC &&
                     interpolation_degree != 1 && symplectic == true,
                 "mls_mpm requires apic quadratic/cubic and only supports "
                 "explicit for now");
  ...
  if (!symplectic) {
    objective.initialize([&](TVStack &dv) {
      for (auto iter = collision_nodes.begin(); iter != collision_nodes.end();
           ++iter) {
...
template <class T, int dim> void MpmSimulationBase<T, dim>::reinitialize() {
  ZIRAN_TIMER();
  Base::reinitialize();
    ...
    if (element_partitions) {
      ...
      for (auto &em : scene.element_managers) {
        if (em && em->needRepartitioning()) {
          ...
          if (count++ == 0) {
            ...
            if (em->renumberParticles(particles.count, old_to_new,
                                      new_to_old)) {
      ...
      if (repartitioned) {
        ...
        for (auto &em : scene.element_managers) {
          ...
          if (SimplexElements<T, 2, dim> *tris =
                  dynamic_cast<SimplexElements<T, 2, dim> *>(em.get()))
            scene.trimesh_to_write.indices = tris->indices.array;

          if (SimplexElements<T, 1, dim> *segs =
                  dynamic_cast<SimplexElements<T, 1, dim> *>(em.get())) {
            if (!segs->exist(no_write)) {
              ...
              for (size_t z = 0; z < segs->indices.array.size(); ++z) {
                if (no_write_array.valueId(z) ==
                    -1) // that means z is not labled no write
                  scene.segmesh_to_write.indices.push_back(
                      segs->indices.array[z]);
              }
            }
          }
        }
...
template <class T, int dim>
void MpmSimulationBase<T, dim>::beginTimeStep(int frame, int substep,
...
void MpmSimulationBase<T, dim>::advanceOneTimeStep(double dt) {
  ZIRAN_TIMER();
  ZIRAN_INFO("Advance one time step from time ", std::setw(7), step.time,
             " with                     dt = ", dt);
  ...
  if (symplectic) {
    ...
    for (size_t k = 0; k < collision_objects.size(); ++k)
      if (collision_objects[k]->updateState)
        collision_objects[k]->updateState(step.time + dt,
                                          *collision_objects[k]);
...
template <class T, int dim> void MpmSimulationBase<T, dim>::trialCollision() {
  ZIRAN_TIMER();
  ...
  grid.iterateGrid([&](IV node, GridState<T, dim> &g) {
    ...
    AnalyticCollisionObject<T, dim>::trialCollision(collision_objects, xi, dt,
                                                    vi);
    g.new_v = vi;
  });
}
...
void MpmSimulationBase<T, dim>::buildGradVn(StdVector<TM> &grad_vn) {
  // TODO: use real rasterized grad_vn instead of grad_v from previous time
  // step!
...
template <class T, int dim>
void MpmSimulationBase<
    T, dim>::performRpicVelocityFilteringAtBeginningOfTimeStep() {
      ...
      if constexpr (!USE_APIC_BLEND_RPIC) {
        ...
        grid.iterateKernel(spline, particle_base_offset[i],
                           [&](IV node, T w, TV dw, GridState<T, dim> &g) {
                             ...
                             Bp.noalias() +=
                                 w * g.new_v * xi_minus_xp.transpose();
                             // !mls_mpm
        ...
        particles.V[i] = picV;
        TM CC = Bp * D_inverse;
        (*C)[i] = ((apic_rpic_ratio + 1) * (T)0.5) * CC +
                  ((apic_rpic_ratio - 1) * (T)0.5) * CC.transpose();
...
template <class T, int dim> void MpmSimulationBase<T, dim>::particlesToGrid() {
  if (symplectic) {
    ZIRAN_TIMER();
    if (mls_mpm) {
...
void MpmSimulationBase<T, dim>::particlesToGridWithForceHelper() {
  ...
  Carray_pointer =
      ((transfer_scheme == APIC_blend_RPIC) && interpolation_degree != 1)
          ? (&(particles.DataManager::get(C_name<TM>()).array))
          : NULL;
      ...
      for (int idx = particle_group[group_idx].first;
        ...
        if constexpr (USE_APIC_BLEND_RPIC) {
          if constexpr (!USE_MPM_DEGREE_ONE) {
            ...
            C = mass * (((apic_rpic_ratio + 1) * (T)0.5) * gradVp +
                        ((apic_rpic_ratio - 1) * (T)0.5) * gradVp.transpose());
          }
        }
        TM4 stress_density = TM4::Zero();
        TM &stress = scratch_stress[i];
        stress_density.template topLeftCorner<dim, dim>() =
            stress; // stress density
        const TV &fp = scratch_fp.col(i);
        stress_density.template topRightCorner<dim, 1>() =
            -fp; // fp (for meshed forces).
        ...
        if constexpr (USE_MLS_MPM) {
          grid.iterateKernel(
              ...
              [&](IV node, T w, TV dw, GridState<T, dim> &g) {
                ...
                xi_minus_xp.template topLeftCorner<dim, 1>() =
                    node.template cast<T>() * dx -
                    Xp; // top dim entries non-zero
                xi_minus_xp(3) = 1;
                TV4 force_delta =
                    ((stress_density * xi_minus_xp) * (w * D_inverse));
                TV4 velocity_delta = velocity_density * xi_minus_xp * w;
                g.m += velocity_delta(3);
                g.v += velocity_delta.template topLeftCorner<dim, 1>();
                g.new_v -= force_delta.template topLeftCorner<
                    dim, 1>(); // fi -= \sum_p (Ap (xi-xp)  - fp )w_ip Dp_inv
              });
        } else {
          grid.iterateKernel(
              spline, particle_base_offset[i],
      ...
      for (int idx = particle_group[group_idx].first;
        ...
        grid.iterateKernel(
            ...
            [&](const IV &node, T w, const TV &dw, GridState<T, dim> &g) {
              TV4 xi_minus_xp = TV4::Zero();
              xi_minus_xp.template topLeftCorner<dim, 1>() =
                  node.template cast<T>() * dx - Xp; // top dim entries non-zero
              xi_minus_xp(3) = 1;
              TV4 velocity_delta = velocity_density * xi_minus_xp * w;
              g.m += velocity_delta(3);
              g.v += velocity_delta.template topLeftCorner<dim, 1>();
...
void MpmSimulationBase<T, dim>::backwardEulerStep() {
  ...
  if (diff_test)
    runDiffTest<T, dim, Objective>(num_nodes, dv, objective,
    ...
    for (int idx = particle_group[group_idx].first;
      ...
      grid.iterateKernel(spline, particle_base_offset[i],
                         [&](IV node, T w, TV dw, GridState<T, dim> &g) {
                           ...
                           fp.col(i) +=
                               -st_tau * (g.m - mass * w) * w * xp_minus_xi;
                         });
    }
  });
}
...
void MpmSimulationBase<T, dim>::forcesUpdateParticleState() {
  ...
  tbb::parallel_for(tbb::blocked_range<size_t>(0, particles.count),
                    [&](const tbb::blocked_range<size_t> &range) {
                      for (size_t b = range.begin(), b_end = range.end();
                           b < b_end; ++b) {
...
void MpmSimulationBase<T, dim>::moveNodes(const TVStack &dv_in) {
  ...
  tbb::parallel_for(tbb::blocked_range<size_t>(0, num_nodes),
                    [&](const tbb::blocked_range<size_t> &range) {
                      for (size_t i = range.begin(), i_end = range.end();
                           i < i_end; ++i) {
...
void MpmSimulationBase<T, dim>::writeState(std::ostream &out) {
  ...
  if (write_partio) {
    std::string obj_filename =
        output_dir.absolutePath(outputFileName("partio", ".bgeo"));
    writePartio(obj_filename, particles);
  }

  scene.writeState(out, outputFileName("boundary", ".obj"),
                   outputFileName("segmesh", ".poly"), output_dir, binary_ver,
                   write_meshes);
...
/**
    ...
    */
template <class T, int dim> double MpmSimulationBase<T, dim>::calculateDt() {
  ZIRAN_QUIET_TIMER();
  TV p_min_corner, p_max_corner;
  Eigen::Array<T, 2, 1> max_speed =
      evalMaxParticleSpeed(p_min_corner, p_max_corner);
  ZIRAN_INFO("Particle Min Corner: ", p_min_corner.transpose());
  ZIRAN_INFO("Particle Max Corner: ", p_max_corner.transpose());
  p_min_corner.array() -=
      (interpolation_degree + 2) * dx; // expand by influenced region and buffer
  ...
  for (size_t k = 0; k < collision_objects.size(); ++k)
    max_collision_object_speed =
        std::max(collision_objects[k]->evalMaxSpeed(p_min_corner, p_max_corner),
                 max_collision_object_speed);
...
template <class T, int dim> void MpmSimulationBase<T, dim>::buildMassMatrix() {
  ZIRAN_QUIET_TIMER();
  mass_matrix.resize(num_nodes, 1);

  grid.iterateGrid(
      [&](IV node, GridState<T, dim> &g) { mass_matrix(g.idx) = g.m; });
}

template <class T, int dim>
void MpmSimulationBase<T, dim>::addScaledForces(
    const T scale,
    TVStack &f) // called BE Newton objective (for computing residual)
{
  for (auto &lf : forces)
...
template <class T, int dim>
void MpmSimulationBase<T, dim>::addScaledForceDifferentials(
    const T scale, const TVStack &x,
    TVStack &f) // called BE Newton objective (for computing residual)
{
  for (auto &lf : forces)
...
void MpmSimulationBase<T, dim>::gridVelocityExplicitUpdate(double dt) {
  ...
  if (explicit_velocity_field) {
    ...
    grid.iterateGrid([&](IV node, GridState<T, dim> &g) {
      ...
      bool collided = AnalyticCollisionObject<T, dim>::multiObjectCollision(
          collision_objects, xi, vi, normal_basis);
...
void MpmSimulationBase<T, dim>::constructNewVelocityFromNewtonResult() {
  ZIRAN_QUIET_TIMER();
  grid.iterateTouchedGrid(
      [&](IV node, GridState<T, dim> &g) { g.new_v = TV::Zero(); });
  grid.iterateGrid(
      [&](IV node, GridState<T, dim> &g) { g.new_v = g.v + dv.col(g.idx); });
}
...
void MpmSimulationBase<T, dim>::gridToParticlesHelper(double dt) {
  ...
  tbb::parallel_for(0, (int)particle_group.size(), [&](int group_idx) {
    for (int idx = particle_group[group_idx].first;
      ...
      particles.X[i] += increment; // particle position update is the same for
                                   // all transfer schemes
      T inc = increment.squaredNorm();
      T dx2 = dx * dx;
      local_faster_than_grid_cell = local_faster_than_grid_cell + (inc > dx2);
      local_faster_than_half_grid_cell = local_faster_than_half_grid_cell +
                                         (inc > dx2 * (T)0.25 * (cfl * cfl));
  ...
  if (faster_than_grid_cell && autorestart) {
    ...
    if (step.max_dt <= step.min_dt) {
      ...
      frame_to_restart--;
      ZIRAN_ASSERT(frame_to_restart >= start_frame,
                   "Unstable intial conditions detected, shrink min_dt or "
                   "change other parameters");
    } else {
      double new_max_dt = std::max(step.max_dt / 2, step.min_dt);
      ZIRAN_WARN("Shrinking max dt to ", new_max_dt);

      restart_callbacks.emplace_back(
          [new_max_dt, this](int frame) { step.max_dt = new_max_dt; });
    }
...
template <class T, int dim> void MpmSimulationBase<T, dim>::applyPlasticity() {
...
void MpmSimulationBase<T, dim>::sortParticlesAndPolluteGrid() {
  ...
  for (int i = 0; i < particles.count; ++i)
    if (i == particles.count - 1 ||
        (particle_sorter[i] >> 32) != (particle_sorter[i + 1] >> 32)) {
  ...
  for (int i = 0; i < particles.count; ++i) {
    ...
    uint64_t offset = (particle_sorter[i] >> index_bits)
  ...
  for (int b = 0; b < (int)blocks.second; ++b) {
    ...
    std::memset(&grid_array(base_offset), 0,
                (size_t)(1 << MpmGrid<T, dim>::log2_page));
    GridState<T, dim> *g =
        reinterpret_cast<GridState<T, dim> *>(&grid_array(base_offset));
...
void MpmSimulationBase<T, dim>::buildInitialDvAndVnForNewton() {
  ...
  grid.iterateGrid([&](IV node, GridState<T, dim> &g) {
    ...
    bool any_collision = AnalyticCollisionObject<T, dim>::multiObjectCollision(
        collision_objects, xi, vi, normal_basis);

    int node_id = g.idx;
    assert(node_id < num_nodes);
    if (any_collision) {
      CollisionNode<T, dim> Z{
          node_id, TM::Identity() - normal_basis * normal_basis.transpose()};
      collision_nodes.push_back(Z);

      // Newton initial guess for collided nodes.
      // Setting it this way so that the new velocity constructed after the
      // solve agrees with collision.
      dv.col(node_id) = vi - old_v;
    } else
      dv.col(node_id) =
          gravity * dt; // Newton initial guess for non-collided nodes
...
template <class T, int dim>
Eigen::Array<T, 2, 1>
MpmSimulationBase<T, dim>::evalMaxParticleSpeed(TV &min_corner,
                                                TV &max_corner) {
  ZIRAN_TIMER();
  ...
  result = particles.map_reduce(
      ...
      [](const TArray &a, const TArray &b) { return a.max(b); },
