...
template <int log2_struct, int log2_field, int D, int log2_page = 12>
class SPGrid_Mask;
...
class SPGrid_Mask_base<log2_struct, 3, log2_page> {
  static_assert(log2_page >= 12, "Please make sure that the page size is "
                                 "larger than a physical page(4096)");

public:
  enum {
    page_bits = log2_page // Bits needed for indexing individual bytes within a
                          // page (not necessarily a 4KB page)
  };

  enum {
    data_bits =
        log2_struct, // Bits needed for indexing individual bytes within type T
    block_bits =
        page_bits -
        data_bits, // Bits needed for indexing data elements within a block
    block_zbits = block_bits / 3 +
                  (block_bits % 3 > 0), // Bits needed for the z-coordinate of a
                                        // data elements within a block
    block_ybits = block_bits / 3 +
                  (block_bits % 3 > 1), // Bits needed for the y-coordinate of a
                                        // data elements within a block
    block_xbits =
        block_bits /
        3 // Bits needed for the x-coordinate of a data elements within a block
  };

  enum : uint64_t { // Bit masks for the upper 52 bits of memory addresses (page
                    // indices)
    // New mask structure, after allowing non-4KB pages
    page_zmask =
        (0x9249249249249249UL << (3 - block_bits % 3 + (page_bits - 12))) &
        (0xffffffffffffffffUL << page_bits),
    page_ymask =
        (0x2492492492492492UL << (3 - block_bits % 3 + (page_bits - 12))) &
        (0xffffffffffffffffUL << page_bits),
    page_xmask =
        (0x4924924924924924UL << (3 - block_bits % 3 + (page_bits - 12))) &
        (0xffffffffffffffffUL << page_bits)
...
class SPGrid_Mask<log2_struct, log2_field, 3, log2_page>
    : public SPGrid_Mask_base<log2_struct, 3, log2_page> {
public:
  enum { dim = 3 };
  enum { field_size = 1 << log2_field };
  typedef SPGrid_Mask_base<log2_struct, dim, log2_page> T_Mask_base;
  using T_Mask_base::block_bits;
  using T_Mask_base::block_xbits;
  using T_Mask_base::block_ybits;
  using T_Mask_base::block_zbits;
  using T_Mask_base::data_bits;
  using T_Mask_base::elements_per_block;
  using T_Mask_base::page_bits;

private:
  using T_Mask_base::page_xmask;
  using T_Mask_base::page_ymask;
  using T_Mask_base::page_zmask;
  enum { // Bit masks for the lower 12 bits of memory addresses (element indices
         // within a page)
    element_zmask = ((1 << block_zbits) - 1)
                    << log2_field, // Likely ok! (after !=4KB generalization)
    element_ymask = ((1 << block_ybits) - 1)
                    << (log2_field +
                        block_zbits), // Likely ok! (after !=4KB generalization)
    element_xmask = ((1 << block_xbits) - 1)
                    << (log2_field + block_zbits +
                        block_ybits) // Likely ok! (after !=4KB generalization)
  };

public:
  enum { // This needs to be carefully re-checked
    // Same as the corresponding element bit masks, but with the most
    // significant bit the respective coordinate zeroed out
    ...
    // Same as the corresponding element bit masks, but with the least
    // significant bit the respective coordinate zeroed out
    ...
    // Just the most significant bit of the element bit mask for the respective
    // coordinate
    ...
    downsample_lower_mask =
        element_z_lsbits | element_y_lsbits | element_x_lsbits,
    upsample_lower_mask =
        element_z_msbits | element_y_msbits | element_x_msbits,

    // "Left over bits" - lob=0 means page address starts with z-bit, lob=1 is
    // x-bit, lob=2 is y-bit
    lob = (3 - block_bits % 3) % 3,

    xloc = lob == 0 ? (log2_page + 3)
                    : (lob == 1 ? (log2_page + 1) : (log2_page + 2)),
    yloc = lob == 0 ? (log2_page + 2)
                    : (lob == 1 ? (log2_page + 3) : (log2_page + 1)),
    zloc = lob == 0 ? (log2_page + 1)
                    : (lob == 1 ? (log2_page + 2) : (log2_page + 3)),
    ...
    bit_log2_page_mask = lob == 0
                             ? element_z_msbit
                             : (lob == 1 ? element_x_msbit : element_y_msbit),
    bit_log2_page_plus_1_mask =
        lob == 0 ? element_y_msbit
                 : (lob == 1 ? element_z_msbit : element_x_msbit),
    bit_log2_page_plus_2_mask =
        lob == 0 ? element_x_msbit
                 : (lob == 1 ? element_y_msbit : element_z_msbit)
  ...
  enum {
    ODD_BITS = BitSpread<1, xmask>::value | BitSpread<1, ymask>::value |
               BitSpread<1, zmask>::value
  };

public:
  static unsigned int Bytes_Per_Element() { return 1u << data_bits; }

  static unsigned int Elements_Per_Block() { return elements_per_block; }

  template <int i, int j, int k> struct LinearOffset {
    static const uint64_t value = BitSpread<i, xmask>::value |
                                  BitSpread<j, ymask>::value |
                                  BitSpread<k, zmask>::value;
  };
  ...
  inline static uint64_t Linear_Offset(const std::array<int, 3> &coord) {
    ...
    return Bit_Spread(coord[0], xmask) | Bit_Spread(coord[1], ymask) |
           Bit_Spread(coord[2], zmask);
#else
    return Bit_Spread<xmask>(coord[0]) | Bit_Spread<ymask>(coord[1]) |
           Bit_Spread<zmask>(coord[2]);
#endif
  }
  inline static uint64_t
  Linear_Offset(const std::array<unsigned int, 3> &coord) {
#ifdef HASWELL
    return Bit_Spread(coord[0], xmask) | Bit_Spread(coord[1], ymask) |
           Bit_Spread(coord[2], zmask);
#else
  ...
  inline static uint64_t DownsampleOffset(uint64_t linear_offset) {
    return Packed_RightShift(
        Packed_Offset<origin, origin, origin>(linear_offset));
  }
  ...
  inline static uint64_t Packed_RightShift(uint64_t linear_offset) {
    ...
    uint64_t result =
        upper | lower | my_array[(linear_offset >> log2_page) & 0x7UL];
    return result;
  }
  ...
  inline static uint64_t UpsampleOffset(uint64_t linear_offset) {
    return Packed_Offset<-origin, -origin, -origin>(
        Packed_LeftShift(linear_offset));
  }
  ...
  inline static uint64_t Packed_Offset(const uint64_t pI) {
    ...
    uint64_t x_result =
        ((pI | MXADD_Xmask) + (pJ & ~MXADD_Xmask)) & ~MXADD_Xmask;
    uint64_t y_result =
        ((pI | MXADD_Ymask) + (pJ & ~MXADD_Ymask)) & ~MXADD_Ymask;
    uint64_t z_result =
        ((pI | MXADD_Zmask) + (pJ & ~MXADD_Zmask)) & ~MXADD_Zmask;
...
template <int log2_struct> class SPGrid_Mask_base<log2_struct, 2> {
protected:
  enum {
    data_bits =
        log2_struct, // Bits needed for indexing individual bytes within type T
    block_bits =
        12 - data_bits, // Bits needed for indexing data elements within a block
    block_ybits = block_bits / 2 +
                  (block_bits % 2 > 0), // Bits needed for the z-coordinate of a
                                        // data elements within a block
    block_xbits =
        block_bits /
        2 // Bits needed for the x-coordinate of a data elements within a block
  };

  enum : uint64_t { // Bit masks for the upper 52 bits of memory addresses (page
                    // indices)
    page_ymask =
        (0x5555555555555555UL << (2 - block_bits % 2)) & 0xfffffffffffff000UL,
    page_xmask =
        (0xaaaaaaaaaaaaaaaaUL << (2 - block_bits % 2)) & 0xfffffffffffff000UL
  };
...
class SPGrid_Mask<log2_struct, log2_field, 2>
    : public SPGrid_Mask_base<log2_struct, 2> {
public:
  enum { dim = 2 };
  enum { field_size = 1 << log2_field };
...
public:
  ...
  enum { ODD_BITS = BitSpread<1, xmask>::value | BitSpread<1, ymask>::value };

public:
  static unsigned int Bytes_Per_Element() { return 1u << data_bits; }

  static unsigned int Elements_Per_Block() { return 1u << block_bits; }

  template <int i, int j> struct LinearOffset {
    static const uint64_t value =
        BitSpread<i, xmask>::value | BitSpread<j, ymask>::value;
  };
  ...
  inline static uint64_t
  Linear_Offset(const std::array<unsigned int, 2> &coord) {
#ifdef HASWELL
    return Bit_Spread(coord[0], xmask) | Bit_Spread(coord[1], ymask);
#else
    return Bit_Spread<xmask>(coord[0]) | Bit_Spread<ymask>(coord[1]);
  ...
  inline static uint64_t Packed_RightShift(uint64_t linear_offset) {
    static uint64_t my_array[4] = {0, bit12_mask, bit13_mask,
