...
void Check_Compliance() {
  if (sysconf(_SC_PAGESIZE) != 4096)
    FATAL_ERROR("Page size different than 4KB detected");
  if (sizeof(unsigned long) != 8)
    FATAL_ERROR("unsigned long is not 64-bit integer");
  if (sizeof(size_t) != 8)
    FATAL_ERROR("size_t is not 64-bit long");
  if (sizeof(void *) != 8)
    FATAL_ERROR("void* is not 64-bit long");
  typedef enum { dummy = 0xffffffffffffffffUL } Long_Enum;
  if (sizeof(Long_Enum) != 8)
    FATAL_ERROR("Missing support for 64-bit enums");
}
...
void *Raw_Allocate(const size_t size) {
  ...
  if (ptr == MAP_FAILED)
    FATAL_ERROR("Failed to allocate " + Value_To_String(size) + " bytes");
  if (0xfffUL & (uint64_t)ptr)
    FATAL_ERROR("Allocated pointer value " + Value_To_String(ptr) +
                " is not page-aligned");
  return ptr;
}
...
void Raw_Deallocate(void *data, const size_t size) {
  if (munmap(data, size) != 0)
    FATAL_ERROR("Failed to deallocate " + Value_To_String(size) + " bytes");
}
//#####################################################################
...
void Deactivate_Page(void *data, const size_t size) {
  if (0xfffUL & (uint64_t)data)
    FATAL_ERROR("Allocated pointer value " + Value_To_String(data) +
                " is not page-aligned");
  int ret = madvise(data, size, MADV_DONTNEED);
  if (ret < 0) {
    char buffer[256];
    char *error_message = (char *)strerror_r(errno, buffer, 256);
    FATAL_ERROR("Failed to deallocate " + Value_To_String(size) +
                " bytes, ERROR: " + error_message);
  }
}
//#####################################################################
...
//#####################################################################
// WARNING: This validation function is very conservative. It can fail (without
// truly indicating a problem) when using transparent hugepages
