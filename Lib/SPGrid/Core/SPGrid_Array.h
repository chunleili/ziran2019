...
template <class T, class T_MASK> class SPGrid_Array {
public:
  enum { dim = T_MASK::dim };
  typedef T DATA;
  typedef T_MASK MASK;
...
public:
  ...
  SPGrid_Array(void *const data_ptr_input,
               const SPGrid_Geometry<dim> &geometry_input)
...
#endif
    return *reinterpret_cast<T *>(reinterpret_cast<uint64_t>(data_ptr) +
                                  T_MASK::Linear_Offset(coord));
  }
  ...
  inline T &operator()(const uint64_t offset) {
    static_assert(dim == 3, "Dimension mismatch");
    return *reinterpret_cast<T *>(reinterpret_cast<uint64_t>(data_ptr) +
                                  T_MASK::Packed_Offset<di, dj, dk>(offset));
  }

  template <int di, int dj> inline T &operator()(const uint64_t offset) {
    static_assert(dim == 2, "Dimension mismatch");
    return *reinterpret_cast<T *>(reinterpret_cast<uint64_t>(data_ptr) +
                                  T_MASK::Packed_Offset<di, dj>(offset));
  }

  // Debug_Get() functions operate like the operator parenthesis, but also check
  // if the memory address is resident
  T &Debug_Get(const ucoord_t i, const ucoord_t j, const ucoord_t k) {
    static_assert(dim == 3, "Dimension mismatch");
#ifdef SPGRID_CHECK_BOUNDS
    geometry.Check_Bounds(i, j, k);
#endif
    T *addr = reinterpret_cast<T *>(reinterpret_cast<uint64_t>(data_ptr) +
                                    T_MASK::Linear_Offset(i, j, k));
