...
class SPGrid_Allocator_Base
    : public SPGrid_Geometry<dim>,
      public SPGrid_Mask_base<log2_struct, dim, log2_page> {
...
public:
  ...
  SPGrid_Allocator_Base(const ucoord_t xsize_input, const ucoord_t ysize_input,
                        const ucoord_t zsize_input)
      : SPGrid_Geometry<dim>(xsize_input, ysize_input, zsize_input, block_xbits,
                             block_ybits, T_Mask_Base::block_zbits) {
  ...
  SPGrid_Allocator_Base(const ucoord_t xsize_input, const ucoord_t ysize_input)
      : SPGrid_Geometry<dim>(xsize_input, ysize_input, block_xbits,
                             block_ybits) {
  ...
  SPGrid_Allocator_Base(const std::array<ucoord_t, 3> size_in)
      : SPGrid_Geometry<dim>(size_in[0], size_in[1], size_in[2], block_xbits,
                             block_ybits, T_Mask_Base::block_zbits) {
    static_assert(dim == 3, "Dimension mismatch");
    Check_Compliance();
    data_ptr = Raw_Allocate(Padded_Volume() << log2_struct);
  ...
  void Validate(uint64_t *page_mask_array) const {
    Validate_Memory_Use((Padded_Volume() << log2_struct) >> 12, Get_Data_Ptr(),
                        page_mask_array);
  }
