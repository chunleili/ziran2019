...
class SPGrid_Allocator
    : public SPGrid_Allocator_Base<NextLogTwo<sizeof(Struct_type)>::value, dim,
                                   log2_page> {
public:
  using Base = SPGrid_Allocator_Base<NextLogTwo<sizeof(Struct_type)>::value,
                                     dim, log2_page>;
  ...
  using Array_mask =
      SPGrid_Mask<log2_struct, NextLogTwo<sizeof(Field_type)>::value, dim,
                  log2_page>;
  template <class Field_type = Struct_type>
  using Array_type = SPGrid_Array<Field_type, Array_mask<Field_type>>;

  template <class T1, class Field_type>
  typename std::enable_if<std::is_same<T1, Struct_type>::value,
                          Array_type<Field_type>>::type
  Get_Array(Field_type T1::*field) {
    size_t offset = OffsetOfMember(field) << block_bits;
    void *offset_ptr = reinterpret_cast<void *>(
        reinterpret_cast<uint64_t>(Get_Data_Ptr()) + offset);
    return Array_type<Field_type>(offset_ptr, *this);
  }

  template <class T1, class Field_type>
  typename std::enable_if<std::is_same<T1, Struct_type>::value,
                          Array_type<const Field_type>>::type
  ...
  Array_type<> Get_Array() { return Array_type<>(Get_Data_Ptr(), *this); }
