...
// Class SPGrid_Set
//#####################################################################
template <class T_ARRAY> class SPGrid_Set {
  enum { d = T_ARRAY::dim };
...
public:
  ...
  // Different threads can safely process entries of the same page, but cannot
  // safely mask different bits of the same entry
  void Mask(const std::array<ucoord_t, d> &coord, const T mask) {
    static_assert(std::is_integral<T>::value,
                  "Masking allowed only with integral types in SPGrid_Set");
