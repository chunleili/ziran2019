...
//#####################################################################
template <int log2_page = 12> class SPGrid_Page_Map {
protected:
  ...
  uint64_t *page_map; // The actual page map - a bitmap structured as an array
                      // of 64-bit entries.
  std::vector<uint64_t>
      block_offsets; // Alternative representation as a list of
                     // kernel_linearized offsets. Created on demand.
  bool dirty; // Indicates that block offsets are inconsistent with the page map
              // (perhaps for a good reason, if only one of them is used).
...
public:
  ...
  ~SPGrid_Page_Map() { Raw_Deallocate(page_map, map_size * sizeof(uint64_t)); }
  ...
  void Set_Page(const uint64_t offset) {
    ...
    if (mask & ~entry) {
      entry |= mask;
      block_offsets.push_back((offset >> log2_page) << log2_page);
    }
    if (!dirty)
...
#if 1
  void Update_Block_Offsets() { dirty = false; }
#else
  ...
  // entirety of the page map. It should perferably be implemented using
  // mincore() instead, to selectively query only resident pages.
