...
public:
  ...
  void writeState(std::ostream &out) {
    ...
    return;
    {
      std::string filename = SimulationBase::output_dir.absolutePath(
          SimulationBase::outputFileName("F_e", ".bgeo"));
      visualize_particles(Fn_pointer, filename);
    }
    {
      std::string filename = SimulationBase::output_dir.absolutePath(
          SimulationBase::outputFileName("F_ne", ".bgeo"));
      visualize_particles(FFn_pointer, filename);
    }
  }

  AdmmSimulation() : Base(), cg(10000) {
    ...
    cg_objective.setProjection([&](TVStack &r) {
      for (auto iter = Base::collision_nodes.begin();
           iter != Base::collision_nodes.end(); ++iter) {
    ...
    Base::parallel_for_updating_grid([&](int i) {
      ...
      grid.iterateKernel(
          ...
          [&](const IV &node, T w, const TV &dw, GridState<T, dim> &g) {
            g.new_v += dt * rho_scale * omega[i] * omega[i] * bFu[i] *
                       Fn.transpose() * dw;
          });
      if (enable_visco) {
        int pc = particles.count;
        ...
        grid.iterateKernel(
            spline, particle_base_offset[i],
            [&](const IV &node, T w, const TV &dw, GridState<T, dim> &g) {
              g.new_v += dt * rho_scale * omega[i + pc] * omega[i + pc] *
                         bFu[i + pc] * FFn.transpose() * dw;
            });
      }
    });
    grid.iterateGrid(
        [&](IV node, GridState<T, dim> &g) { rhs.col(g.idx) += g.new_v; });
  ...
  template <class TCONST, class TPCONST> void initOmega() {
    ZIRAN_TIMER();
    auto ranges = particles.X.ranges;
    auto constitutive_model_name = AttributeName<TCONST>(TCONST::name());
    auto plastic_name = AttributeName<TPCONST>(TPCONST::name());
    if (!particles.exist(constitutive_model_name) ||
        !particles.exist(plastic_name))
      return;
    tbb::parallel_for(ranges, [&](DisjointRanges &subrange) {
      DisjointRanges subset(
          subrange, particles.commonRanges(constitutive_model_name,
                                           element_measure_name<T>(),
                                           F_name<T, dim>(), plastic_name));
      for (auto iter = particles.subsetIter(subset, constitutive_model_name,
                                            element_measure_name<T>(),
                                            F_name<T, dim>(), plastic_name);
           iter; ++iter) {
        ...
        T clamp_value = 1e-6;
        total += (makePD2D(constitutive_model.Bij(sigma, 0, 1, clamp_value)))
                     .squaredNorm();
        if constexpr (dim == 3) {
          total += (makePD2D(constitutive_model.Bij(sigma, 0, 2, clamp_value)))
                       .squaredNorm();
          total += (makePD2D(constitutive_model.Bij(sigma, 1, 2, clamp_value)))
                       .squaredNorm();
        }
        // omega[i] = std::sqrt(vol *
        // ((T)2*constitutive_model.mu/3+constitutive_model.lambda));
        ...
        if constexpr (std::is_same<TPCONST, StvkWithHencky<T, dim>>::value) {
          ...
          total = A.squaredNorm();
          clamp_value = 1e-6;
          total += (makePD2D(plasticity_model.Bij(sigma, 0, 1, clamp_value)))
                       .squaredNorm();
          if constexpr (dim == 3) {
            total += (makePD2D(plasticity_model.Bij(sigma, 0, 2, clamp_value)))
                         .squaredNorm();
            total += (makePD2D(plasticity_model.Bij(sigma, 1, 2, clamp_value)))
                         .squaredNorm();
          }
          // omega[i + pc] = std::sqrt(vol *
          // ((T)2*plasticity_model.mu/3+plasticity_model.lambda));
  ...
  void updateCollisionNodes() {
    ...
    grid.iterateGrid([&](IV node, GridState<T, dim> &g) {
      ...
      TM normal_basis;
      bool any_collision =
          AnalyticCollisionObject<T, dim>::multiObjectCollisionWithFakeSeparate(
              collision_objects, xi, vi, normal_basis);
      if (any_collision) {
        CollisionNode<T, dim> Z{(int)g.idx,
                                TM::Identity() -
                                    normal_basis * normal_basis.transpose()};
        mtx.lock();
  ...
  void initStep() {
    ...
    if (!enable_visco) {
      ...
      tbb::parallel_for(0, y_old_size / 2, [&](int i) {
        y[i] = y_old[i] * omega_old[i] / omega[i];
        y[i + pc] = y_old[i + y_old_size / 2] * omega_old[i + y_old_size / 2] /
                    omega[i + pc];
      });
    }
    // init mass_matrix
  ...
  void initMultiplierAndPreconditioner() {
    ...
    if constexpr (build_matrix) {
      ...
      Base::parallel_for_updating_grid([&](int i) {
        ...
        for (int p = 0; p < cached_size; ++p) {
          ...
          for (int q = 0; q < cached_size; ++q) {
            int offset = 0;
            if constexpr (dim == 2) {
              offset = 5 * (cached_nodes[q](0) - cached_nodes[p](0) + 2) +
                       cached_nodes[q](1) - cached_nodes[p](1) + 2;
            } else {
              offset = 25 * (cached_nodes[q](0) - cached_nodes[p](0) + 2) +
                       5 * (cached_nodes[q](1) - cached_nodes[p](1) + 2) +
                       cached_nodes[q](2) - cached_nodes[p](2) + 2;
            }
      ...
      grid.iterateGrid(
          [&](IV node, GridState<T, dim> &g) { g.new_v = g.m * TV::Ones(); });
      Base::parallel_for_updating_grid([&](int i) {
        TV &Xp = Xarray[i];
        TM &Fn = (*Fn_pointer)[i];
        BSplineWeights<T, dim> spline(Xp, dx);
        grid.iterateKernel(spline, particle_base_offset[i],
                           [&](IV node, T w, TV dw, GridState<T, dim> &g) {
                             T coeff =
                                 rho_scale * omega[i] * omega[i] * dt * dt;
                             TM tmp =
                                 coeff * (TV::Ones() * dw.transpose() * Fn);
  ...
  template <class TCONST, class TPCONST> void FStepSolve(const T &localTol) {
    auto &Xarray = particles.X.array;
    auto constitutive_model_name = AttributeName<TCONST>(TCONST::name());
    auto plastic_name = AttributeName<TPCONST>(TPCONST::name());
    auto ranges = particles.X.ranges;
    ...
    tbb::parallel_for(ranges, [&](DisjointRanges &subrange) {
      DisjointRanges subset(
          subrange,
          particles.commonRanges(constitutive_model_name, plastic_name,
                                 element_measure_name<T>(), F_name<T, dim>()));
      for (auto iter = particles.subsetIter(
               subset, constitutive_model_name, plastic_name,
               element_measure_name<T>(), F_name<T, dim>());
           iter; ++iter) {
        ...
        if constexpr (std::is_same<TPCONST, StvkWithHencky<T, dim>>::value) {
          ...
          for (int j = 0; j < 20; j++) {
            ...
            TM zi_pd;
            {
              ...
              T beta = (T)2.0 * ((T)2.0 * mu + lambda * dim) /
                           ((T)9.0 * viscosity_v) -
                       (T)2.0 * mu / (viscosity_d * dim);

              TV epsilon_trial = zi.array().abs().log();
              TV epsilon = (T)1.0 / ((T)1.0 + dt * alpha) *
                           (epsilon_trial -
                            dt * beta / ((T)1.0 + dt * (alpha + dim * beta)) *
                                epsilon_trial.trace() * TV::Ones());
              ...
              zi_pd =
                  zi_p_m * ((T)1.0 / ((T)1.0 + dt * alpha)) *
                  (zi_inv_m - dt * beta / ((T)1.0 + dt * (alpha + dim * beta)) *
                                  TV::Ones() * zi_inv.transpose());
            }
  ...
  void FStep(T localTol) {
    ...
    if (use_elasticity_plasticity) {
      ...
      FStepSolve<StvkWithHenckyIsotropic<T, dim>, StvkWithHencky<T, dim>>(
          localTol);
    }
  }
  ...
  T admmResidual() {
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, residual.cols(), 256),
                      [&](const tbb::blocked_range<int> &range) {
                        ...
                        residual.middleCols(start, length) =
                            dtg *
                            mass_matrix.segment(start, length).transpose();
                      });
    ...
    T norm_sq = tbb::parallel_reduce(
        ...
        [&](const tbb::blocked_range<int> &range, T ns) -> T {
          ...
          const auto &mass_block =
              mass_matrix.segment(start, length).transpose();
          return ns + ((r_block.colwise().squaredNorm()).array() /
                       mass_block.array())
                          .sum();
        },
    ...
    ZIRAN_INFO("admm residual ", residual_current,
               ", total cg iterations = ", total_cg_iterations,
               ", rho_scale = ", rho_scale);
  ...
  void updateRhoScaleAbsolute() {
    ...
    grid.iterateTouchedGrid(
        [&](IV node, GridState<T, dim> &g) { g.new_v = TV::Zero(); });
    grid.iterateGrid(
        [&](IV node, GridState<T, dim> &g) { g.new_v = dv.col(g.idx); });
    ...
    T dual_residual = 0;
    grid.iterateGrid(
        [&](IV node, GridState<T, dim> &g) { g.new_v -= old_dv.col(g.idx); });
    tbb::parallel_for(0, (int)particle_group.size(), [&](int group_idx) {
      for (int idx = particle_group[group_idx].first;
           idx <= particle_group[group_idx].second; ++idx) {
        int i = particle_order[idx];
    ...
    ZIRAN_INFO("prime residual : ", prime_residual,
  ...
  void admmStep() {
    ...
    ZIRAN_ASSERT(over_relaxation_alpha == (T)1,
                 "please use AdmmSimulationFull.h");
    ...
    for (int tim = 0; tim < admm_max_iterations; ++tim) {
      ...
      dvStep(global_tolerance,
             std::min((T)0.5,
                      (T)1 * std::sqrt(std::max(residual_current, (T)1e-6))));
      uStep();
    }
    // same with BE method
