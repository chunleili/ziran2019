...
public:
  ...
  void setMaxIteration(const int max_iterations_input) {
  ...
  int solve(const TM &A, TV &x, const TV &b, const bool verbose = false) {
    ...
    T goal_tolerance =
        std::min(tolerance, convergence_norm * relative_tolerance);
    printf("   INFO  CG starts with convergence_norm = %.10lf ",
