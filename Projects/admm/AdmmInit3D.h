...
template <class T, int dim> class AdmmInitBase;

template <class T> class AdmmInit3D : public AdmmInitBase<T, 3> {
public:
  ...
  void reload() override {
    if (test_number == 1) {
      ...
      {
        AxisAlignedAnalyticBox<T, dim> box_out(TV(4.25, 4.9, 4.9),
                                               TV(5.75, 5.1, 5.1));
        AxisAlignedAnalyticBox<T, dim> box_in(TV(4.25, 4.925, 4.925),
                                              TV(5.75, 5.075, 5.075));
        ...
        MpmParticleHandleBase<T, dim> particles_handle =
            init_helper.sampleInAnalyticLevelSetWithExistingPoints(
                meshed_points, materialRegionLevelSet, 2, 8);
        //// Elasticity
      ...
      AnalyticCollisionObject<T, dim> sphere_object1(
          sphere_transform1, sphere1, AnalyticCollisionObject<T, dim>::STICKY);
      init_helper.addAnalyticCollisionObject(sphere_object1);
      TV sphere_center2(0, 0, 0);
      Sphere<T, dim> sphere2(sphere_center2, 0.2);
      AnalyticCollisionObject<T, dim> sphere_object2(
          sphere_transform2, sphere2, AnalyticCollisionObject<T, dim>::STICKY);
    ...
    if (test_number == 3) {
      ...
      MpmParticleHandleBase<T, dim> particles_handle =
          init_helper.sampleInAnalyticLevelSet(cylinder, rho, 8);
    ...
    if (test_number == 4) {
      ...
      MpmParticleHandleBase<T, dim> particles_handle =
          init_helper.sampleFromVdbFileWithExistingPoints(
              meshed_points, "LevelSets/lion.vdb", rho, ppc);
      auto initial_translation = [=](int index, Ref<T> mass, TV &X, TV &V) {
        X = X + TV(5, 5, 5);
      };
      particles_handle.transform(initial_translation);
      ...
      // MpmParticleHandleBase<T, dim> particles_handle =
      // init_helper.sampleInAnalyticLevelSet(cylinder, rho, ppc);
      ...
      particles_handle.addFElasticNonequilibratedBasedMpmForce(
          nonequilibrated_model, (T)10, (T)10);

#endif

      TV translation_freeze =
          TV(5, 5.345, 5) + TV(0, 24 * 0.0025, 0) * 50. / 24.;
      T rotation_freeze = (T)24 * 5 / 180 * M_PI * 50. / 24.;

      auto cylinder_transform =
          [translation_freeze,
           rotation_freeze](T time, AnalyticCollisionObject<T, dim> &object) {
            T frame_dt = 1. / 24.;
            T frame_move = 50;
            T frame_freeze = 100;
            if (time < frame_move * frame_dt) {
              ...
              Vector<T, 4> rotation(std::cos(theta * time / 2), 0,
                                    std::sin(theta * time / 2), 0);
              ...
              Vector<T, 4> rotation(std::cos(rotation_freeze / 2), 0,
                                    std::sin(rotation_freeze / 2), 0);
              object.setRotation(rotation);
              TV omega(0, 0, 0);
              object.setAngularVelocity(omega);
      ...
      AnalyticCollisionObject<T, dim> box_object(
          box, AnalyticCollisionObject<T, dim>::STICKY);
    ...
    if (test_number == 5) {
      ...
      MpmParticleHandleBase<T, dim> particles_handle =
          init_helper.sampleInAnalyticLevelSetWithExistingPoints(
              meshed_points, cylinder, rho, ppc);
