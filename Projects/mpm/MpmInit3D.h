...
template <class T, int dim> class MpmInitBase;

template <class T> class MpmInit3D : public MpmInitBase<T, 3> {
public:
  ...
  void reload() override {
    // Watermelon smash
    // ./mpm -test 1 --3d -Jp 0.99 -Jp2 0.97 -beta 2 -beta2 1 -nu 0.4 -nu2 0.35
    // -E 2000 -E2 1000 -rho 2 -vy -6.5
    if (test_number == 1) {
      //            std::string Jp_str = "Jp_" + std::to_string(Minchen::p_Jp);
      //            Jp_str.erase(Jp_str.find_last_not_of('0') + 1,
      //            std::string::npos); Jp_str += "_" +
      //            std::to_string(Minchen::p_Jp2);
      //            Jp_str.erase(Jp_str.find_last_not_of('0') + 1,
      //            std::string::npos);
      //
      //            std::string beta_str = "beta_" +
      //            std::to_string(Minchen::p_beta);
      //            beta_str.erase(beta_str.find_last_not_of('0') + 1,
      //            std::string::npos); beta_str += "_" +
      //            std::to_string(Minchen::p_beta2);
      //            beta_str.erase(beta_str.find_last_not_of('0') + 1,
      //            std::string::npos);
      //
      //            std::string nu_str = "nu_" + std::to_string(Minchen::p_nu);
      //            nu_str.erase(nu_str.find_last_not_of('0') + 1,
      //            std::string::npos); nu_str += "_" +
      //            std::to_string(Minchen::p_nu2);
      //            nu_str.erase(nu_str.find_last_not_of('0') + 1,
      //            std::string::npos);
      //
      //            std::string E_str = "E_" + std::to_string(Minchen::p_E);
      //            E_str.erase(E_str.find_last_not_of('0') + 1,
      //            std::string::npos); E_str += "_" +
      //            std::to_string(Minchen::p_E2);
      //            E_str.erase(E_str.find_last_not_of('0') + 1,
      //            std::string::npos);
      //
      //            std::string rho_str = "rho_" +
      //            std::to_string(Minchen::p_rho2);
      //            rho_str.erase(rho_str.find_last_not_of('0') + 1,
      //            std::string::npos);
      //
      //            std::string vy_str = "vy_" + std::to_string(Minchen::p_vy);
      //            vy_str.erase(vy_str.find_last_not_of('0') + 1,
      //            std::string::npos);
      //
      //            std::cout << "Jp = " << Minchen::p_Jp << ", beta = " <<
      //            Minchen::p_beta << std::endl; std::cout << "nu = " <<
      //            Minchen::p_nu << ", E = " << Minchen::p_E << std::endl;
      //            std::cout << "Jp2 = " << Minchen::p_Jp2 << ", beta2 = " <<
      //            Minchen::p_beta2 << std::endl; std::cout << "nu2 = " <<
      //            Minchen::p_nu2 << ", E2 = " << Minchen::p_E2 << std::endl;
      //            std::cout << "rho2 = " << Minchen::p_rho2 << std::endl;
      //            std::cout << "vy = " << Minchen::p_vy << std::endl;
      //
      //            std::string path("output/watermelon_smash_3d_" + Jp_str +
      //            "_" + beta_str + "_" + nu_str + "_" + E_str + "_" + rho_str
      //            + "_" + vy_str); if (Minchen::p_noPlasticity) {
      ...
      //                    [&](int frame) {
      //                        std::string filename =
      //                        sim.output_dir.absolutePath(sim.outputFileName("logJp",
      //                        ".bgeo")); writePartioAttribute(filename,
      //                        sim.particles,
      //                        AttributeName<NonAssociativeCamClay<T>>(NonAssociativeCamClay<T>::name()),
      //                        (T)-1);
      //                    });
      //            }
      ...
      // Add watermelon shell modeled with pumpkin parameters and add watermelon
      // innards modeled with cam clay snow params
      T rho = 2;
      T suggested_dt =
          evaluateTimestepLinearElasticityAnalysis(E, nu, rho, sim.dx, sim.cfl);
      if (sim.symplectic) {
        sim.step.max_dt = suggested_dt * 0.6;
      }

      int particlesPerCell = 8;
      MpmParticleHandleBase<T, dim> particles_handle_outside =
          init_helper.sampleFromVdbFile("LevelSets/watermelon_outside.vdb", rho,
                                        particlesPerCell);
      ...
      NonAssociativeCamClay<T> pOutside(
          /*logJp*/ std::log(Jp), /*friction_angle*/ 45, /*beta*/ beta,
          /*xi*/ 3, dim); // test 19 pumpkin params
      ...
      MpmParticleHandleBase<T, dim> particles_handle_inside =
          init_helper.sampleFromVdbFile("LevelSets/watermelon_inside.vdb",
                                        Minchen::p_rho2, particlesPerCell);
      particles_handle_inside.transform(initial_translation);

      NeoHookeanBorden<T, dim> modelInside(E2, nu2);
      NonAssociativeCamClay<T> pInside(
          /*logJp*/ std::log(Jp2), /*friction_angle*/ 45, /*beta*/ beta2,
          /*xi*/ 3,
          dim); // test 9 cam clay snow params (M = 0.6 --> phi = 15.83 deg)
    ...
    if (test_number == 2) {
      ...
      //            Jp_str.erase(Jp_str.find_last_not_of('0') + 1,
      //            std::string::npos); std::string beta_str =
      //            std::to_string(Minchen::p_beta);
      //            beta_str.erase(beta_str.find_last_not_of('0') + 1,
      //            std::string::npos); std::string nu_str =
      //            std::to_string(Minchen::p_nu);
      //            nu_str.erase(nu_str.find_last_not_of('0') + 1,
      //            std::string::npos); std::string E_str =
      //            std::to_string(Minchen::p_E);
      //            E_str.erase(E_str.find_last_not_of('0') + 1,
      //            std::string::npos); std::cout << "Jp = " << Minchen::p_Jp <<
      //            ", beta = " << Minchen::p_beta << std::endl; std::cout <<
      //            "nu = " << Minchen::p_nu << ", E = " << Minchen::p_E <<
      //            std::endl; std::string path("output/pumpkin_smash_3d_" +
      //            Jp_str + "_" + beta_str + "_" + nu_str + "_" + E_str); if
      //            (Minchen::p_noPlasticity) {
      //                std::cout << "pure elasticity" << std::endl;
      ...
      int particlesPerCell = 8;
      MpmParticleHandleBase<T, dim> particles_handle_pumpkin1 =
          init_helper.sampleFromVdbFile("LevelSets/pumpkinShiftedUp.vdb", rho,
                                        particlesPerCell);
      MpmParticleHandleBase<T, dim> particles_handle_pumpkin2 =
          init_helper.sampleFromVdbFile("LevelSets/pumpkinShiftedUp.vdb", rho,
                                        particlesPerCell);

      auto initial_translation_pumpkin1 = [](int index, Ref<T> mass, TV &X,
                                             TV &V) {
        X[1] *= 2;
        X = X + TV(4, 0.2, 4);
        V = TV(0, 0, 0);
      ...
      auto initial_translation_pumpkin2 = [](int index, Ref<T> mass, TV &X,
                                             TV &V) {
      ...
      NonAssociativeCamClay<T> p(/*logJp*/ std::log(Jp), /*friction_angle*/ 45,
                                 /*beta*/ beta, /*xi*/ 3,
                                 dim); // .97, 45, .8, 3
      ...
      // Add ground, and a wall to prevent negative position in each direction
      // (x and z)!
      T wallFriction = 0.1;
    ...
    if (test_number == 3) {
      ...
      T suggested_dt = evaluateTimestepLinearElasticityAnalysis(
          youngs_crust, nu, rho, sim.dx, sim.cfl);
      sim.step.max_dt = suggested_dt * 0.7;
      if (sim.symplectic) {
        ZIRAN_ASSERT(sim.step.max_dt <= suggested_dt, suggested_dt);
      }

      { // crust
        MpmParticleHandleBase<T, dim> particles_handle =
            init_helper.sampleFromVdbFile("LevelSets/oreo_crust.vdb", rho);
        auto initial_translation = [=](int index, Ref<T> mass, TV &X, TV &V) {
        ...
        NonAssociativeCamClay<T> p(/*logJp*/ std::log((T)1.01),
                                   /*friction_angle*/ 60, /*beta*/ 3, /*xi*/ 3,
                                   dim); // .97, 45, .8, 3
      ...
      { // heart
        ...
        T height = 0.02;
        T theta = M_PI / 2;
        Vector<T, 4> cylinder_rotation(std::cos(theta / 2), 0, 0,
                                       std::sin(theta / 2));
        TV translation(0, 0, 0);
        CappedCylinder<T, dim> cylinder(radius, height, cylinder_rotation,
                                        translation);
        MpmParticleHandleBase<T, dim> particles_handle =
            init_helper.sampleInAnalyticLevelSet(cylinder, rho_heart, 8);
        auto initial_translation = [=](int index, Ref<T> mass, TV &X, TV &V) {
          Rotation<T, dim> rotation(TV(0, 1, 0), TV(0.1, 1, 0.2));
          X = rotation.rotation * X + TV(.0, .9, .0) + domain_center;
          V = initial_V;
        ...
        NonAssociativeCamClay<T> p(/*logJp*/ std::log((T).99),
                                   /*friction_angle*/ 45, /*beta*/ .7, /*xi*/ 1,
                                   dim); // .97, 45, .8, 3
        particles_handle.addFBasedMpmForce(model);
        particles_handle.addPlasticity(model, p, "F");
      }
