...
template <class T> class FractureInit3D : public FractureInitBase<T, 3> {
public:
  ...
  void reload() override {
    ...
    // ./mpm -test 24 --3d --usePhaseField -nu 0.4 -E 20 -rho 0.1 -v 10 -dx
    // 0.012 -M 15 -sigmaPercent 0.015 -bulletFriction 0.1
    // ./mpm -test 24 --3d --usePhaseField -nu 0.4 -E 20 -rho 0.1 -v 10 -dx
    // 0.012 -M 10 -sigmaPercent 0.02 -bulletFriction 0.1
    // ./mpm -test 24 --3d -nu 0.4 -E 20 -rho 0.1 -v 10 -dx 0.012
    // -bulletFriction 0.1
    if (test_number == 1) {
      //            std::string name = "Dino";
      //
      //            std::string nu_str = "nu" + std::to_string(Minchen::nu);
      //            nu_str.erase(nu_str.find_last_not_of('0') + 1,
      //            std::string::npos); std::string E_str = "E" +
      //            std::to_string(Minchen::E);
      //            E_str.erase(E_str.find_last_not_of('0') + 1,
      //            std::string::npos); std::string rho_str = "rho" +
      //            std::to_string(Minchen::rho);
      //            rho_str.erase(rho_str.find_last_not_of('0') + 1,
      //            std::string::npos); std::string v_str = "v" +
      //            std::to_string(Minchen::v);
      //            v_str.erase(v_str.find_last_not_of('0') + 1,
      //            std::string::npos); std::string dx_str = "dx" +
      //            std::to_string(Minchen::dx);
      //            dx_str.erase(dx_str.find_last_not_of('0') + 1,
      //            std::string::npos); std::string bulletFriction_str =
      //            "bulletFric" + std::to_string(Minchen::bulletFriction);
      //            bulletFriction_str.erase(bulletFriction_str.find_last_not_of('0')
      //            + 1, std::string::npos); if (Minchen::usePhaseField) {
      //                std::string M_str = "M" + std::to_string(Minchen::M);
      //                M_str.erase(M_str.find_last_not_of('0') + 1,
      //                std::string::npos); std::string sigmaPercent_str =
      //                "sigma" + std::to_string(Minchen::sigmaPercent);
      //                sigmaPercent_str.erase(sigmaPercent_str.find_last_not_of('0')
      //                + 1, std::string::npos);
      //
      //                sim.output_dir.path = "output/bullet_jello" + name +
      //                "_pf_" + nu_str + "_" + E_str + "_" + rho_str + "_" +
      //                v_str + "_" + dx_str + "_" + bulletFriction_str + "_" +
      //                M_str + "_" + sigmaPercent_str;
      //            }
      //            else {
      //                sim.output_dir.path = "output/bullet_jello" + name + "_"
      //                + nu_str + "_" + E_str + "_" + rho_str + "_" + v_str +
      //                "_" + dx_str + "_" + bulletFriction_str;
      //            }
      ...
      MpmParticleHandleBase<T, dim> particles_handle_top =
          init_helper.sampleFromVdbFile("LevelSets/dino.vdb", jelloRho,
                                        particlesPerCellPane);
      ...
      particles_handle_top.addFBasedMpmForceWithPhaseField(
          percent, (T)sim.dx * 0.5, model);
...
#define COLLISION_OBJECT_BULLET
      ...
      auto bulletTransform = [](T time,
        ...
        //                TV translation(4 + Minchen::v * time, 0.45, 5.5);
        //                //multiply each velocity by dt to get dx!
        TV translation(4 + 10 * time, 0.975,
                       5.5); // multiply each velocity by dt to get dx!
        ...
        //                T cosine = std::cos(angle_rad / 2), sine =
        //                std::sin(angle_rad / 2); Vector<T, 4> rotation(cosine,
        //                sine * rotAxis[0], sine * rotAxis[1], sine *
        //                rotAxis[2]); object.setRotation(rotation);
        Vector<T, 3> omega(0, 0, 0);
        object.setAngularVelocity(omega);
      };
      AnalyticCollisionObject<T, dim> bullet_object(
          bulletTransform, bullet_ls, AnalyticCollisionObject<T, dim>::STICKY);
      ...
      MpmParticleHandleBase<T, dim> particles_handle_bullet =
          init_helper.sampleFromVdbFile(
              "/home/joshwolper/Desktop/Fracture Media/Models/bullet.vdb",
              bulletRho, particlesPerCellPane);
      auto initial_translation_bullet = [](int index, Ref<T> mass, TV &X,
                                           TV &V) {
      ...
      particles_handle_bullet.addFBasedMpmForceWithPhaseField(
          /* resistence to break */ 0.0075, (T)sim.dx * 0.5, bulletModel,
          false);
#endif
      ...
      AnalyticCollisionObject<T, dim> ground_object(
          ground_ls, AnalyticCollisionObject<T, dim>::STICKY);
      ...
      AnalyticCollisionObject<T, dim> xWall_object(
          xWall_ls, AnalyticCollisionObject<T, dim>::STICKY);
      ...
      AnalyticCollisionObject<T, dim> zWall_object(
          zWall_ls, AnalyticCollisionObject<T, dim>::STICKY);
    ...
    if (test_number == 2) {
      ...
      bool allowDamage = true;
      {
        ...
        AnalyticCollisionObject<T, dim> ground_object(
            ballLevelSet, AnalyticCollisionObject<T, dim>::STICKY);
      ...
      MpmParticleHandleBase<T, dim> particles_handle =
          init_helper.sampleFromVdbFile("LevelSets/breadxxx.vdb", rho, ppc);
      ...
      NeoHookeanBorden<T, dim> model(youngs, nu); // neohookean borden
                                                  // elasticity
      // NonAssociativeCamClay<T> p(/*logJp*/ 0.99, /*friction_angle*/ 45,
      // /*beta*/ 60, /*xi*/ 6, dim);
      //            NonAssociativeVonMises<T> p(/*tauY*/ 1000 * 1.0, /*alpha*/
      //            0, /*hardeningCoefficient*/ 0, dim); //von mises plasticity

      particles_handle.addFBasedMpmForceWithPhaseField(pfPercent, l0, model,
                                                       allowDamage);
      //            particles_handle.addPlasticity(model, p, "F");

      for (auto iter = particles_handle.particles.subsetIter(
               DisjointRanges{particles_handle.particle_range},
      ...
      auto leftSphereTransform = [](T time,
                                    AnalyticCollisionObject<T, dim> &object) {
        TV translation_velocity(-0.125, 0., 0.);
        TV translation(-0.125 * time, 0, 0.);
        object.setTranslation(translation, translation_velocity);
      };
      auto rightSphereTransform = [](T time,
                                     AnalyticCollisionObject<T, dim> &object) {
        TV translation_velocity(0.125, 0., 0.);
        TV translation(0.125 * time, 0, 0.);
        object.setTranslation(translation, translation_velocity);
    ...
    if (test_number == 3) {
      ...
      // Phase Field Params
      sim.use_phase_field = true;
      sim.parabolic_M = 1; // was -1 before to match physbam, changed to tune
                           // pure elasticity with PF
      sim.lumping = true;
      {
        sim.delete_particle_threshold = -1; // 0.0075 looks decent, but lose
                                            // detail at the fracture interface
      ...
      int ppc = 40;
      MpmParticleHandleBase<T, dim> particles_handle =
          init_helper.sampleFromVdbFile("LevelSets/armadillo.vdb", rho, ppc);

      // Elasticity Handler
      NeoHookeanBorden<T, dim> model(Youngs, nu);
      particles_handle.addFBasedMpmForceWithPhaseField(percentage, l0, model,
                                                       allowDamage);

      // Sigma C Voronoi Initialization Routine
      if (sim.use_phase_field) {
        // std::string filename = "/home/joshwolper/Desktop/Fracture
        // Media/Models/Voronoi Files/armadilloVoro.obj"; T radius = 0.1; //0.1
        // seems good T magnitude = 10; //magnitude = 1 keeps the function
        // continuous T minP = -0.9999999; //should not go below -1 T maxP =
        // 3000; T zeta = 75; particles_handle.voronoiSigmaC(filename, radius);
      }
      ...
      // SPHERE A
      AnalyticCollisionObject<T, dim> sphereObjectA(
          sphereA,
          AnalyticCollisionObject<T, dim>::STICKY); // this sphere does not move
      init_helper.addAnalyticCollisionObject(sphereObjectA);

      // SPHERE B
      auto sphereTransformB = [](T time,
                                 AnalyticCollisionObject<T, dim> &object) {
        T sphereSpeed = 0.2;
        TV translation_velocity(-1 * sphereSpeed, sphereSpeed, 0);
        TV translation(-1 * sphereSpeed * time, sphereSpeed * time,
                       0); // multiply each velocity by dt to get dx!
        object.setTranslation(translation, translation_velocity);
        Vector<T, 3> omega(0, 0, 0);
        object.setAngularVelocity(omega);
      };
      AnalyticCollisionObject<T, dim> sphereObjectB(
          sphereTransformB, sphereB,
          AnalyticCollisionObject<T, dim>::STICKY); // this sphere moves
      init_helper.addAnalyticCollisionObject(sphereObjectB);

      // SPHERE C
      auto sphereTransformC = [](T time,
                                 AnalyticCollisionObject<T, dim> &object) {
        T sphereSpeed = 0.2;
        TV translation_velocity(sphereSpeed, sphereSpeed, 0);
        TV translation(sphereSpeed * time, sphereSpeed * time,
                       0); // multiply each velocity by dt to get dx!
        object.setTranslation(translation, translation_velocity);
        Vector<T, 3> omega(0, 0, 0);
        object.setAngularVelocity(omega);
      };
      AnalyticCollisionObject<T, dim> sphereObjectC(
          sphereTransformC, sphereC,
          AnalyticCollisionObject<T, dim>::STICKY); // this sphere moves
      init_helper.addAnalyticCollisionObject(sphereObjectC);

      // SPHERE D
      auto sphereTransformD = [](T time,
                                 AnalyticCollisionObject<T, dim> &object) {
        T sphereSpeed = 0.2;
        TV translation_velocity(-1 * sphereSpeed, -1 * sphereSpeed, 0);
        TV translation(-1 * sphereSpeed * time, -1 * sphereSpeed * time,
                       0); // multiply each velocity by dt to get dx!
        object.setTranslation(translation, translation_velocity);
        Vector<T, 3> omega(0, 0, 0);
        object.setAngularVelocity(omega);
      };
      AnalyticCollisionObject<T, dim> sphereObjectD(
          sphereTransformD, sphereD,
          AnalyticCollisionObject<T, dim>::STICKY); // this sphere moves
      init_helper.addAnalyticCollisionObject(sphereObjectD);

      // SPHERE E
      auto sphereTransformE = [](T time,
                                 AnalyticCollisionObject<T, dim> &object) {
        T sphereSpeed = 0.2;
        TV translation_velocity(sphereSpeed, -1 * sphereSpeed, 0);
        TV translation(sphereSpeed * time, -1 * sphereSpeed * time,
                       0); // multiply each velocity by dt to get dx!
        object.setTranslation(translation, translation_velocity);
        Vector<T, 3> omega(0, 0, 0);
        object.setAngularVelocity(omega);
      };
      AnalyticCollisionObject<T, dim> sphereObjectE(
          sphereTransformE, sphereE,
          AnalyticCollisionObject<T, dim>::STICKY); // this sphere moves
