...
public:
  ...
  ConjugateGradient(const int max_it_input) : max_iterations(max_it_input) {
  ...
  void setTolerance(
      const T tolerance_input = 16 * std::numeric_limits<T>::epsilon()) {
  ...
  int solve(const TM &A, TV &x, const TV &b, const bool verbose = false) {
    ...
    T relative_tolerance =
        convergence_norm * (std::is_same<T, float>::value ? (T)1e-6 : (T)1e-12);
    ...
    for (iterations = 0;; iterations++) {
      ...
      if (convergence_norm < tolerance ||
          convergence_norm < relative_tolerance ||
          iterations == max_iterations) {
        ZIRAN_INFO("CG iterattion ends at ", iterations,
                   " and convergence_norm = ", convergence_norm);
        return iterations;
      }
    }
#else
    ...
    ZIRAN_VERB_IF(verbose, "ConjugateGradient max iterations reached ",
                  max_iterations);
