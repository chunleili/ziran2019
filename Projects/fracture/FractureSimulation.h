...
public:
  ...
  FractureSimulation() : cg_objective(*this), cg(100) {}
  ...
  inline static AttributeName<NeoHookeanBorden<T, dim>>
  neohookean_borden_range() {
    return AttributeName<NeoHookeanBorden<T, dim>>(
  ...
  virtual void initialize() {
    ...
    //                for (auto iter = particles.subsetIter(subset,
    //                phase_field_range()); iter; ++iter) {
    //                    auto& phase_field = iter.template get<0>();
    //                    if (unique_one_over_sigma_c == 0) {
    //                        unique_one_over_sigma_c =
    //                        phase_field.one_over_sigma_c;
    //                    }
    //                    else {
    //                        ZIRAN_ASSERT(unique_one_over_sigma_c ==
    //                        phase_field.one_over_sigma_c, "all phase field
    //                        particle should own same one_over_sigma_c");
    //                    }
    //                }
    //            });
    //        ZIRAN_INFO("Particles with phase field, one_over_sigma_c = ",
    //        unique_one_over_sigma_c);

    SimulationBase::end_frame_callbacks.emplace_back([&](int frame) {
      std::string filename = SimulationBase::output_dir.absolutePath(
          SimulationBase::outputFileName("phaseField", ".bgeo"));
      ...
      sigmaCH =
          parts->addAttribute("sigmaC", Partio::VECTOR,
                              1); // NOTE: this is NOT the inverse of sigmaC!!
      ...
      for (int k = 0; k < particles.count; k++) {
        ...
        sigmaCP[0] =
            (T)1 / (float)sigmaCData[k]; // store the inverse of the inverse so
                                         // we have the actual sigmaC!
      }
  ...
  void deleteParticles() {
    ...
    tbb::parallel_for(particles.X.ranges, [&](DisjointRanges &subrange) {
      DisjointRanges subset(
          subrange, particles.commonRanges(phase_field_range(), F_range()));
      for (auto iter =
               particles.subsetIter(subset, phase_field_range(), F_range());
           iter; ++iter) {
        auto &phase_field = iter.template get<0>();
        auto &F = iter.template get<1>();
        ZIRAN_ASSERT(!((phase_field.c < delete_particle_threshold) &&
                       (F.determinant() != (T)1)),
                     "set F as identity wrongly");
      }
    });
  }
  ...
  void solvePhaseFieldSystem() {
    ...
    /*if (parabolic_M > 0) {
        for (uint64_t color = 0; color < (1 << dim); ++color) {
            tbb::parallel_for(0, (int) particle_group.size(), [&](int group_idx)
    { if ((block_offset[group_idx] & ((1 << dim) - 1)) != color) return; for
    (int idx = particle_group[group_idx].first; idx <=
    particle_group[group_idx].second; ++idx) { int i = particle_order[idx]; TV
    &Xp = Xarray[i]; BSplineWeights<T, dim> spline(Xp, dx);
                    grid.iterateKernel(spline, particle_base_offset[i],
                                       [&](const IV &node, T w, const TV &dw,
    GridState<T, dim> &g) { g.phase_field_multiplier += w; g.phase_field +=
    (*pf_pointer)[i].c * w;
                                       });
                }
            });
                      ...
                      [&](DisjointRanges &subrange) {
                          DisjointRanges subset(subrange,
                                                particles.commonRanges(phase_field_range(),
                                                                       ...
                                                                       F_range()));
                          for (auto iter = particles.subsetIter(subset,
    phase_field_range(), element_measure_range(), F_range()); iter; ++iter) {
                              auto &phase_field = iter.template get<0>();
                              auto &vol0 = iter.template get<1>();
                              auto &F = iter.template get<2>();
                              phase_field.vol = vol0 * F.determinant();
            ...
            grid.iterateKernel(
                ...
                [&](IV node, T w, TV dw, GridState<T, dim> &g) {
                  ...
                  if (!lumping) {
                    ...
                    if (parabolic_M > 0) {
                      b(node_id) += vol *
                                    (phase.pf_Fp * parabolic_M + (T)1 / dt) *
                                    w * x(node_id);
                    } else {
          ...
          for (int idx = particle_group[group_idx].first;
            ...
            grid.iterateKernel(
                ...
                [&](IV node, T w, TV dw, GridState<T, dim> &g) {
                  ...
                  if (parabolic_M > 0) {
                    out(node_id) +=
                        vol * (parabolic_M * phase.pf_Fp + (T)1 / dt) * w;
                  } else {
    ...
    if (parabolic_M > 0) {
      ...
      grid.iterateGrid(
          [&](IV node, GridState<T, dim> &g) { g.phase_field = x(g.idx); });
    }
    ...
    tbb::parallel_for(ranges, [&](DisjointRanges &subrange) {
      DisjointRanges subset(subrange,
                            particles.commonRanges(phase_field_range(),
                                                   ...
                                                   F_range()));
      for (auto iter =
               particles.subsetIter(subset, phase_field_range(),
                                    neohookean_borden_range(), F_range());
           iter; ++iter) {
        auto &phase_field = iter.template get<0>();
  ...
  void solvePhaseFieldSystemNaive() {
    ...
    tbb::parallel_for(ranges, [&](DisjointRanges &subrange) {
      ...
      for (auto iter =
        ...
        // Now, compute c based on naive damage assumption (using sigmaF as our
        // defined threshold)
        T J = F.determinant();
        TM tau;
        ...
        // set model.g to 1 so we compute based on the undegraded principal
        // stress
        model.g = 1;
        ...
        Eigen::VectorXcd eigenVals =
            cauchy.eigenvalues(); // TODO: NEED TO CHANGE THIS WHEN SWITCHING
                                  // BETWEEN FLOAT AND DOUBLE!!!!
        T sigmaMax = (T)eigenVals[0]
                         .real(); // get the real part of the first eigen value
                                  // (which shouldnt have be complex anyway lol)
        T newC = c;
        if (sigmaMax > sigmaF) {
