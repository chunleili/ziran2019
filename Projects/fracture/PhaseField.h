...
template <class T, int dim> class PhaseField {
public:
  T residual_phase;
  ...
  PhaseField(const T residual_phase, const T c, const T H, const T l0,
             const T one_over_sigma_c, const T pf_Fp, const T H_max,
             const T vol, const bool allow_damage)
      : residual_phase(residual_phase), c(c), H(H), l0(l0),
        one_over_sigma_c(one_over_sigma_c), pf_Fp(pf_Fp), H_max(H_max),
        vol(vol), allow_damage(allow_damage) {}
  ...
  static T Get_Sigma_C_Based_On_Max_Deformation(const T percentage,
                                                const TCONST &model) {
  ...
  static const char *name() { return "PhaseField"; }
};
