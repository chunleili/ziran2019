#ifndef FRACTURE_INIT_H
#define FRACTURE_INIT_H
...
template <class T, int dim> class FractureInitBase {
public:
  using TV = Vector<T, dim>;
  using TVI = Vector<int, dim>;

  FractureSimulation<T, dim> &sim;   // This contains the real scene data.
  ...
  FractureInitBase(FractureSimulation<T, dim> &sim, const int test_number)
      : sim(sim), scene(sim.scene), init_helper(sim), test_number(test_number) {
  }
